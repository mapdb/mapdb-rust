...
#[derive(Debug, Clone)]
pub struct BoolBoolListMultimap {
    inner: HashMap<bool, Vec<bool>>,
    total_size: usize,
}

impl BoolBoolListMultimap {
    pub fn new() -> Self {
        BoolBoolListMultimap {
            inner: HashMap::new(),
            total_size: 0,
        }
    }
    ...
    pub fn put(&mut self, key: bool, value: bool) {
    ...
    pub fn get_all(&self, key: bool) -> Vec<bool> {
        self.get(key).to_vec()
    }

    /// Removes all values for the given key. Returns the removed values.
    pub fn remove_all(&mut self, key: bool) -> Vec<bool> {
        let internal_key = key;
    ...
    pub fn contains_key_value(&self, key: bool, value: bool) -> bool {
    ...
    pub fn for_each_key_value(&self, mut f: impl FnMut(bool, &[bool])) {
        for (k, vals) in &self.inner {
            f(*k, vals);
        }
    }
...
impl Default for BoolBoolListMultimap {
    fn default() -> Self {
        Self::new()
    }
}

impl PartialEq for BoolBoolListMultimap {
...
impl fmt::Display for BoolBoolListMultimap {
...
mod tests {
    ...
    #[test]
    fn test_put_get() {
        let mut m = BoolBoolListMultimap::new();
        m.put(true, true);
        m.put(true, false);
        m.put(false, true);
    ...
    fn test_contains_key_value() {
        ...
        m.put(true, true);
        m.put(true, false);
        assert!(m.contains_key_value(true, true));
        assert!(m.contains_key_value(true, false));
    }
    ...
    #[test]
    fn test_equals() {
        let mut m1 = BoolBoolListMultimap::new();
        m1.put(true, true);
        m1.put(true, false);
        let mut m2 = BoolBoolListMultimap::new();
        m2.put(true, true);
        m2.put(true, false);
        assert_eq!(m1, m2);
    }
    ...
    fn test_with_key_value_fluent() {
        let m = BoolBoolListMultimap::new()
            .with_key_value(true, true)
            .with_key_value(true, false)
            .with_key_value(false, true);
        assert_eq!(m.size(), 3);
        assert_eq!(m.keys_count(), 2);
    }
