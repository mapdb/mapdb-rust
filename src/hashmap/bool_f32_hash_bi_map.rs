...
use crate::hashmap::f32_bool_hash_bi_map::F32BoolHashBiMap;
use std::fmt;
...
impl BoolF32HashBiMap {
    ...
    pub fn insert(&mut self, key: bool, value: f32) -> Option<f32> {
    ...
    pub fn remove_value(&mut self, value: f32) -> Option<bool> {
    ...
    pub fn inverse(&self) -> F32BoolHashBiMap {
        F32BoolHashBiMap::from_maps(self.reverse.clone(), self.forward.clone())
    }
    ...
    pub(crate) fn from_maps(
        ...
        BoolF32HashBiMap { forward, reverse }
    }
}

impl Default for BoolF32HashBiMap {
    fn default() -> Self {
        Self::new()
    }
}

impl PartialEq for BoolF32HashBiMap {
...
impl fmt::Display for BoolF32HashBiMap {
...
mod tests {
    ...
    #[test]
    fn test_insert_get() {
        let mut m = BoolF32HashBiMap::new();
        m.insert(true, 1.0f32);
        m.insert(false, 2.0f32);
        assert_eq!(m.get(true), Some(1.0f32));
        assert_eq!(m.get(false), Some(2.0f32));
        assert_eq!(m.len(), 2);
    }
    ...
    fn test_get_key() {
        ...
        m.insert(true, 1.0f32);
        m.insert(false, 2.0f32);
        assert_eq!(m.get_key(1.0f32), Some(true));
        assert_eq!(m.get_key(2.0f32), Some(false));
    ...
    fn test_insert_overwrite() {
        let mut m = BoolF32HashBiMap::new();
        m.insert(true, 1.0f32);
        let old = m.insert(true, 2.0f32);
        assert_eq!(old, Some(1.0f32));
        assert_eq!(m.get(true), Some(2.0f32));
        // Reverse should reflect the new mapping
        assert_eq!(m.get_key(2.0f32), Some(true));
    ...
    fn test_remove() {
        ...
        m.insert(true, 1.0f32);
        m.insert(false, 2.0f32);
        assert_eq!(m.remove(true), Some(1.0f32));
        assert_eq!(m.len(), 1);
    ...
    fn test_remove_value() {
        ...
        m.insert(true, 1.0f32);
        m.insert(false, 2.0f32);
        assert_eq!(m.remove_value(1.0f32), Some(true));
        assert_eq!(m.len(), 1);
        assert!(!m.contains_key(true));
        assert!(!m.contains_value(1.0f32));
    }
    ...
    fn test_inverse() {
        ...
        assert_eq!(inv.get(1.0f32), Some(true));
        assert_eq!(inv.get(2.0f32), Some(false));
        // inverse reverse lookup: old key -> old value
        assert_eq!(inv.get_key(true), Some(1.0f32));
        assert_eq!(inv.len(), 2);
    }
...
impl crate::traits::bool_f32_map::BoolF32Map for BoolF32HashBiMap {
    fn len(&self) -> usize {
        self.len()
    }
...
impl crate::traits::bool_f32_map::BoolF32MutableMap for BoolF32HashBiMap {
