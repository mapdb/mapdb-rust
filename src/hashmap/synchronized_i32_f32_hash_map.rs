...
use super::i32_f32_hash_map::I32F32HashMap;

/// Thread-safe wrapper around [`I32F32HashMap`]. Reads are concurrent;
/// mutations are exclusive. Lock poisoning panics (parity with Go).
#[derive(Debug, Default)]
pub struct SynchronizedI32F32HashMap {
    inner: RwLock<I32F32HashMap>,
}

impl SynchronizedI32F32HashMap {
    pub fn new() -> Self {
        SynchronizedI32F32HashMap {
            inner: RwLock::new(I32F32HashMap::new()),
        }
    }

    pub fn with_capacity(capacity: usize) -> Self {
        SynchronizedI32F32HashMap {
            inner: RwLock::new(I32F32HashMap::with_capacity(capacity)),
        }
    }

    pub fn insert(&self, key: i32, value: f32) -> Option<f32> {
    ...
    pub fn remove(&self, key: i32) -> Option<f32> {
...
mod tests {
    ...
    #[test]
    fn test_basic_ops() {
        let m = SynchronizedI32F32HashMap::new();
        assert!(m.is_empty());
        assert_eq!(m.insert(1, 1.0f32), None);
        assert_eq!(m.insert(2, 2.0f32), None);
        assert_eq!(m.insert(3, 3.0f32), None);
        assert_eq!(m.get(1), Some(1.0f32));
        assert!(m.contains_key(1));
        assert!(m.contains_value(2.0f32));
        let old = m.insert(1, 2.0f32);
        assert_eq!(old, Some(1.0f32));
        assert_eq!(m.remove(1), Some(2.0f32));
        assert!(!m.contains_key(1));
        m.clear();
        assert!(m.is_empty());
    }
    ...
    #[test]
    fn test_concurrent() {
        let m = Arc::new(SynchronizedI32F32HashMap::new());
