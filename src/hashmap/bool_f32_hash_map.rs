...
impl BoolF32HashMap {
    ...
    pub fn reject(&self, predicate: impl Fn(bool, f32) -> bool) -> Self {
        let mut result = Self::new();
        for (k, v) in self.iter() {
            if !predicate(k, v) {
                result.insert(k, v);
    ...
    pub fn detect(&self, predicate: impl Fn(bool, f32) -> bool) -> Option<(bool, f32)> {
    ...
    pub fn none_satisfy(&self, predicate: impl Fn(bool, f32) -> bool) -> bool {
        !self.iter().any(|(k, v)| predicate(k, v))
    }

    pub fn count(&self, predicate: impl Fn(bool, f32) -> bool) -> usize {
        self.iter().filter(|&(k, v)| predicate(k, v)).count()
    }

    pub fn inject_into<R>(&self, initial: R, mut f: impl FnMut(R, bool, f32) -> R) -> R {
        let mut acc = initial;
        for (k, v) in self.iter() {
            acc = f(acc, k, v);
        }
    ...
    pub fn with_key_value(mut self, key: bool, value: f32) -> Self {
...
impl Default for BoolF32HashMap {
    fn default() -> Self {
        Self::new()
    }
}

impl PartialEq for BoolF32HashMap {
...
impl fmt::Display for BoolF32HashMap {
...
mod tests {
    ...
    fn test_get_or_default() {
        let mut m = BoolF32HashMap::new();
        m.insert(true, 1.0f32);
        assert_eq!(m.get_or_default(true, 3.0f32), 1.0f32);
        assert_eq!(m.get_or_default(false, 3.0f32), 3.0f32);
    }
    ...
    fn test_any_all_none() {
        let mut m = BoolF32HashMap::new();
        m.insert(true, 1.0f32);
        assert!(m.any_satisfy(|_k, v| v == 1.0f32));
    }

    #[test]
    fn test_equals() {
        let mut m1 = BoolF32HashMap::new();
        m1.insert(true, 1.0f32);
        m1.insert(false, 2.0f32);
        let mut m2 = BoolF32HashMap::new();
        m2.insert(false, 2.0f32);
        m2.insert(true, 1.0f32);
        assert_eq!(m1, m2);
    }
...
impl crate::traits::bool_f32_map::BoolF32Map for BoolF32HashMap {
    fn len(&self) -> usize {
        self.len()
    }
...
impl crate::traits::bool_f32_map::BoolF32MutableMap for BoolF32HashMap {
