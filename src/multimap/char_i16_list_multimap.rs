...
/// List multimap from `char` keys to `i16` values.
...
#[derive(Debug, Clone)]
pub struct CharI16ListMultimap {
    inner: HashMap<char, Vec<i16>>,
    total_size: usize,
}

impl CharI16ListMultimap {
    pub fn new() -> Self {
        CharI16ListMultimap {
            inner: HashMap::new(),
            total_size: 0,
    ...
    pub fn put(&mut self, key: char, value: i16) {
    ...
    pub fn get_all(&self, key: char) -> Vec<i16> {
        self.get(key).to_vec()
    }

    /// Removes all values for the given key. Returns the removed values.
    pub fn remove_all(&mut self, key: char) -> Vec<i16> {
        let internal_key = key;
    ...
    pub fn contains_key_value(&self, key: char, value: i16) -> bool {
    ...
    pub fn for_each_key_value(&self, mut f: impl FnMut(char, &[i16])) {
        for (k, vals) in &self.inner {
            f(*k, vals);
        }
    }
    ...
    pub fn to_vec(&self) -> Vec<(char, i16)> {
...
impl Default for CharI16ListMultimap {
    fn default() -> Self {
        Self::new()
    }
}

impl PartialEq for CharI16ListMultimap {
...
impl fmt::Display for CharI16ListMultimap {
...
mod tests {
    ...
    #[test]
    fn test_put_get() {
        let mut m = CharI16ListMultimap::new();
        m.put('a', 1);
        m.put('a', 2);
        m.put('b', 3);
    ...
    fn test_contains_key_value() {
        ...
        assert!(!m.contains_key_value('a', 3));
    }
    ...
    #[test]
    fn test_equals() {
        let mut m1 = CharI16ListMultimap::new();
        m1.put('a', 1);
        m1.put('a', 2);
        let mut m2 = CharI16ListMultimap::new();
        m2.put('a', 1);
        m2.put('a', 2);
        assert_eq!(m1, m2);
    }
