...
use super::bool_char_hash_map::BoolCharHashMap;

/// Thread-safe wrapper around [`BoolCharHashMap`]. Reads are concurrent;
/// mutations are exclusive. Lock poisoning panics (parity with Go).
#[derive(Debug, Default)]
pub struct SynchronizedBoolCharHashMap {
    inner: RwLock<BoolCharHashMap>,
}

impl SynchronizedBoolCharHashMap {
    pub fn new() -> Self {
        SynchronizedBoolCharHashMap {
            inner: RwLock::new(BoolCharHashMap::new()),
        }
    }

    pub fn with_capacity(capacity: usize) -> Self {
        SynchronizedBoolCharHashMap {
            inner: RwLock::new(BoolCharHashMap::with_capacity(capacity)),
        }
    }

    pub fn insert(&self, key: bool, value: char) -> Option<char> {
    ...
    pub fn remove(&self, key: bool) -> Option<char> {
...
mod tests {
    ...
    #[test]
    fn test_basic_ops() {
        let m = SynchronizedBoolCharHashMap::new();
        assert!(m.is_empty());
        assert_eq!(m.insert(true, 'a'), None);
        assert_eq!(m.insert(false, 'b'), None);
        ...
        assert_eq!(m.remove(true), Some('b'));
        assert!(!m.contains_key(true));
        m.clear();
        assert!(m.is_empty());
    }
    ...
    #[test]
    fn test_concurrent() {
        let m = Arc::new(SynchronizedBoolCharHashMap::new());
