...
#[derive(Debug, Clone)]
pub struct I16I32ListMultimap {
    inner: HashMap<i16, Vec<i32>>,
    total_size: usize,
}

impl I16I32ListMultimap {
    pub fn new() -> Self {
        I16I32ListMultimap {
            inner: HashMap::new(),
            total_size: 0,
    ...
    pub fn put(&mut self, key: i16, value: i32) {
    ...
    pub fn get_all(&self, key: i16) -> Vec<i32> {
        self.get(key).to_vec()
    }

    /// Removes all values for the given key. Returns the removed values.
    pub fn remove_all(&mut self, key: i16) -> Vec<i32> {
        let internal_key = key;
    ...
    pub fn contains_key_value(&self, key: i16, value: i32) -> bool {
    ...
    pub fn for_each_key_value(&self, mut f: impl FnMut(i16, &[i32])) {
        for (k, vals) in &self.inner {
            f(*k, vals);
        }
    }
    ...
    pub fn to_vec(&self) -> Vec<(i16, i32)> {
...
impl Default for I16I32ListMultimap {
    fn default() -> Self {
        Self::new()
    }
}

impl PartialEq for I16I32ListMultimap {
...
impl fmt::Display for I16I32ListMultimap {
...
mod tests {
    ...
    #[test]
    fn test_put_get() {
        let mut m = I16I32ListMultimap::new();
        m.put(1, 1);
        m.put(1, 2);
        m.put(2, 3);
    ...
    #[test]
    fn test_equals() {
        let mut m1 = I16I32ListMultimap::new();
        m1.put(1, 1);
        m1.put(1, 2);
        let mut m2 = I16I32ListMultimap::new();
        m2.put(1, 1);
        m2.put(1, 2);
        assert_eq!(m1, m2);
    }
