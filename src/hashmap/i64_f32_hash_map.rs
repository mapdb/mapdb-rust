...
impl I64F32HashMap {
    ...
    pub fn insert(&mut self, key: i64, value: f32) -> Option<f32> {
    ...
    pub fn get_or_default(&self, key: i64, default: f32) -> f32 {
        self.inner.get(key).unwrap_or(default)
    }

    /// Removes the key. Returns the old value if present.
    pub fn remove(&mut self, key: i64) -> Option<f32> {
    ...
    pub fn for_each(&self, mut f: impl FnMut(i64, f32)) {
        for (k, v) in self.iter() {
            f(k, v);
        }
    }

    pub fn select(&self, predicate: impl Fn(i64, f32) -> bool) -> Self {
    ...
    pub fn reject(&self, predicate: impl Fn(i64, f32) -> bool) -> Self {
        let mut result = Self::new();
        for (k, v) in self.iter() {
            if !predicate(k, v) {
                result.insert(k, v);
    ...
    pub fn detect(&self, predicate: impl Fn(i64, f32) -> bool) -> Option<(i64, f32)> {
        self.iter().find(|&(k, v)| predicate(k, v))
    }

    pub fn any_satisfy(&self, predicate: impl Fn(i64, f32) -> bool) -> bool {
        self.iter().any(|(k, v)| predicate(k, v))
    }

    pub fn all_satisfy(&self, predicate: impl Fn(i64, f32) -> bool) -> bool {
        self.iter().all(|(k, v)| predicate(k, v))
    }

    pub fn none_satisfy(&self, predicate: impl Fn(i64, f32) -> bool) -> bool {
        !self.iter().any(|(k, v)| predicate(k, v))
    }

    pub fn count(&self, predicate: impl Fn(i64, f32) -> bool) -> usize {
        self.iter().filter(|&(k, v)| predicate(k, v)).count()
    }

    pub fn inject_into<R>(&self, initial: R, mut f: impl FnMut(R, i64, f32) -> R) -> R {
        let mut acc = initial;
        for (k, v) in self.iter() {
            acc = f(acc, k, v);
        }
    ...
    pub fn with_key_value(mut self, key: i64, value: f32) -> Self {
...
impl Default for I64F32HashMap {
    fn default() -> Self {
        Self::new()
    }
}

impl PartialEq for I64F32HashMap {
...
impl fmt::Display for I64F32HashMap {
...
mod tests {
    ...
    #[test]
    fn test_equals() {
        let mut m1 = I64F32HashMap::new();
        m1.insert(1, 1.0f32);
        m1.insert(2, 2.0f32);
        let mut m2 = I64F32HashMap::new();
        m2.insert(2, 2.0f32);
        m2.insert(1, 1.0f32);
        assert_eq!(m1, m2);
    }
...
impl crate::traits::i64_f32_map::I64F32Map for I64F32HashMap {
    fn len(&self) -> usize {
        self.len()
    }
    ...
    fn iter(&self) -> impl Iterator<Item = (i64, f32)> + '_ {
        self.iter()
    }
}

impl crate::traits::i64_f32_map::I64F32MutableMap for I64F32HashMap {
    fn insert(&mut self, key: i64, value: f32) -> Option<f32> {
        self.insert(key, value)
    }
    fn remove(&mut self, key: i64) -> Option<f32> {
        self.remove(key)
    }
    fn clear(&mut self) {
        self.clear()
