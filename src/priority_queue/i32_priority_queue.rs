...
/// Primitive min-heap priority queue of `i32` values, backed by a `Vec`.
/// O(log n) push/pop, O(1) peek. Duplicates are allowed.
#[derive(Debug, Clone)]
pub struct I32PriorityQueue {
    items: Vec<i32>,
}

impl I32PriorityQueue {
    pub fn new() -> Self {
        I32PriorityQueue { items: Vec::new() }
    }
    ...
    pub fn drain_sorted(mut self) -> Vec<i32> {
...
impl Default for I32PriorityQueue {
    fn default() -> Self {
        Self::new()
    }
}

impl fmt::Display for I32PriorityQueue {
...
mod tests {
    ...
    #[test]
    fn test_of_heapify() {
        let q = I32PriorityQueue::of(&[3, 1, 2]);
        assert_eq!(q.len(), 3);
        assert_eq!(q.peek(), Some(1));
    }
