...
use super::f64_char_hash_map::F64CharHashMap;

/// Thread-safe wrapper around [`F64CharHashMap`]. Reads are concurrent;
/// mutations are exclusive. Lock poisoning panics (parity with Go).
#[derive(Debug, Default)]
pub struct SynchronizedF64CharHashMap {
    inner: RwLock<F64CharHashMap>,
}

impl SynchronizedF64CharHashMap {
    pub fn new() -> Self {
        SynchronizedF64CharHashMap {
            inner: RwLock::new(F64CharHashMap::new()),
        }
    }

    pub fn with_capacity(capacity: usize) -> Self {
        SynchronizedF64CharHashMap {
            inner: RwLock::new(F64CharHashMap::with_capacity(capacity)),
        }
    }

    pub fn insert(&self, key: f64, value: char) -> Option<char> {
    ...
    pub fn remove(&self, key: f64) -> Option<char> {
...
mod tests {
    ...
    #[test]
    fn test_basic_ops() {
        let m = SynchronizedF64CharHashMap::new();
        assert!(m.is_empty());
        assert_eq!(m.insert(1.0f64, 'a'), None);
        assert_eq!(m.insert(2.0f64, 'b'), None);
        assert_eq!(m.insert(3.0f64, 'c'), None);
        ...
        assert_eq!(m.remove(1.0f64), Some('b'));
        assert!(!m.contains_key(1.0f64));
        m.clear();
        assert!(m.is_empty());
    }
    ...
    #[test]
    fn test_concurrent() {
        let m = Arc::new(SynchronizedF64CharHashMap::new());
