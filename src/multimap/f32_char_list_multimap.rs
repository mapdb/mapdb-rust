...
/// List multimap from `f32` keys to `char` values.
...
#[derive(Debug, Clone)]
pub struct F32CharListMultimap {
    inner: HashMap<u32, Vec<char>>,
    total_size: usize,
}

impl F32CharListMultimap {
    pub fn new() -> Self {
        F32CharListMultimap {
            inner: HashMap::new(),
            total_size: 0,
    ...
    pub fn put(&mut self, key: f32, value: char) {
        let internal_key = f32::to_bits(key);
    ...
    pub fn get_all(&self, key: f32) -> Vec<char> {
        self.get(key).to_vec()
    }

    /// Removes all values for the given key. Returns the removed values.
    pub fn remove_all(&mut self, key: f32) -> Vec<char> {
        let internal_key = f32::to_bits(key);
    ...
    pub fn contains_key_value(&self, key: f32, value: char) -> bool {
    ...
    pub fn for_each_key_value(&self, mut f: impl FnMut(f32, &[char])) {
        for (k, vals) in &self.inner {
            f(f32::from_bits(*k), vals);
        }
    }
...
impl Default for F32CharListMultimap {
    fn default() -> Self {
        Self::new()
    }
}

impl PartialEq for F32CharListMultimap {
...
impl fmt::Display for F32CharListMultimap {
...
mod tests {
    ...
    #[test]
    fn test_put_get() {
        let mut m = F32CharListMultimap::new();
        m.put(1.0f32, 'a');
        m.put(1.0f32, 'b');
        m.put(2.0f32, 'c');
        assert_eq!(m.get(1.0f32).len(), 2);
        assert_eq!(m.get(2.0f32).len(), 1);
        assert_eq!(m.get(99.0f32).len(), 0);
        assert_eq!(m.size(), 3);
    ...
    fn test_remove_all() {
        ...
        m.put(1.0f32, 'b');
        m.put(2.0f32, 'c');
        let removed = m.remove_all(1.0f32);
    ...
    fn test_contains_key_value() {
        ...
        m.put(1.0f32, 'a');
        m.put(1.0f32, 'b');
        assert!(m.contains_key_value(1.0f32, 'a'));
        assert!(m.contains_key_value(1.0f32, 'b'));
        assert!(!m.contains_key_value(1.0f32, 'c'));
    }
    ...
    #[test]
    fn test_equals() {
        let mut m1 = F32CharListMultimap::new();
        m1.put(1.0f32, 'a');
        m1.put(1.0f32, 'b');
        let mut m2 = F32CharListMultimap::new();
        m2.put(1.0f32, 'a');
        m2.put(1.0f32, 'b');
        assert_eq!(m1, m2);
    }
    ...
    fn test_with_key_value_fluent() {
        let m = F32CharListMultimap::new()
            .with_key_value(1.0f32, 'a')
            .with_key_value(1.0f32, 'b')
            .with_key_value(2.0f32, 'c');
        assert_eq!(m.size(), 3);
        assert_eq!(m.keys_count(), 2);
    }
