...
/// Read-only trait for any map from `char` keys to `i16` values.
///
...
pub trait CharI16Map {
    ...
    fn for_each_key_value(&self, mut f: impl FnMut(char, i16)) {
...
/// Mutable map trait extending CharI16Map.
pub trait CharI16MutableMap: CharI16Map {
...
mod verify {
    use super::*;
    fn _assert_map<T: CharI16Map>() {}
    fn _assert_mut<T: CharI16MutableMap>() {}

    /// Compile-time verification: every concrete `char` → `i16` map type
    /// implements the read-only and (where applicable) mutable map trait.
    #[test]
    fn types_implement_traits() {
        _assert_map::<crate::hashmap::char_i16_hash_map::CharI16HashMap>();
        _assert_mut::<crate::hashmap::char_i16_hash_map::CharI16HashMap>();
        _assert_map::<crate::treemap::char_i16_tree_map::CharI16TreeMap>();
        _assert_mut::<crate::treemap::char_i16_tree_map::CharI16TreeMap>();
        _assert_map::<crate::hashmap::char_i16_hash_bi_map::CharI16HashBiMap>();
        _assert_mut::<crate::hashmap::char_i16_hash_bi_map::CharI16HashBiMap>();
        _assert_map::<crate::immutable::immutable_char_i16_hash_map::ImmutableCharI16HashMap>();
    }
}
