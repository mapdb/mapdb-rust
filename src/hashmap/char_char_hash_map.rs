...
/// Hash map from `char` keys to `char` values.
/// Open-addressing with linear probing and Robin Hood backward-shift deletion.
#[derive(Debug, Clone)]
pub struct CharCharHashMap {
    inner: OpenHashMap<char, char>,
}

impl CharCharHashMap {
    pub fn new() -> Self {
        CharCharHashMap {
    ...
    pub fn detect(&self, predicate: impl Fn(char, char) -> bool) -> Option<(char, char)> {
        self.iter().find(|&(k, v)| predicate(k, v))
    }

    pub fn any_satisfy(&self, predicate: impl Fn(char, char) -> bool) -> bool {
        self.iter().any(|(k, v)| predicate(k, v))
    }

    pub fn all_satisfy(&self, predicate: impl Fn(char, char) -> bool) -> bool {
        self.iter().all(|(k, v)| predicate(k, v))
    }

    pub fn none_satisfy(&self, predicate: impl Fn(char, char) -> bool) -> bool {
        !self.iter().any(|(k, v)| predicate(k, v))
    }

    pub fn count(&self, predicate: impl Fn(char, char) -> bool) -> usize {
        self.iter().filter(|&(k, v)| predicate(k, v)).count()
    }

    pub fn inject_into<R>(&self, initial: R, mut f: impl FnMut(R, char, char) -> R) -> R {
        let mut acc = initial;
        for (k, v) in self.iter() {
            acc = f(acc, k, v);
        }
...
impl Default for CharCharHashMap {
    fn default() -> Self {
        Self::new()
    }
}

impl PartialEq for CharCharHashMap {
...
impl fmt::Display for CharCharHashMap {
...
mod tests {
    ...
    #[test]
    fn test_equals() {
        let mut m1 = CharCharHashMap::new();
        m1.insert('a', 'a');
        m1.insert('b', 'b');
        let mut m2 = CharCharHashMap::new();
        m2.insert('b', 'b');
        m2.insert('a', 'a');
        assert_eq!(m1, m2);
    }
...
impl crate::traits::char_char_map::CharCharMap for CharCharHashMap {
    fn len(&self) -> usize {
        self.len()
    }
...
impl crate::traits::char_char_map::CharCharMutableMap for CharCharHashMap {
