...
/// List multimap from `bool` keys to `char` values.
...
#[derive(Debug, Clone)]
pub struct BoolCharListMultimap {
    inner: HashMap<bool, Vec<char>>,
    total_size: usize,
}

impl BoolCharListMultimap {
    pub fn new() -> Self {
        BoolCharListMultimap {
            inner: HashMap::new(),
            total_size: 0,
        }
    }
    ...
    pub fn put(&mut self, key: bool, value: char) {
    ...
    pub fn get_all(&self, key: bool) -> Vec<char> {
        self.get(key).to_vec()
    }

    /// Removes all values for the given key. Returns the removed values.
    pub fn remove_all(&mut self, key: bool) -> Vec<char> {
        let internal_key = key;
    ...
    pub fn contains_key_value(&self, key: bool, value: char) -> bool {
    ...
    pub fn for_each_key_value(&self, mut f: impl FnMut(bool, &[char])) {
        for (k, vals) in &self.inner {
            f(*k, vals);
        }
    }
    ...
    pub fn to_vec(&self) -> Vec<(bool, char)> {
...
impl Default for BoolCharListMultimap {
    fn default() -> Self {
        Self::new()
    }
}

impl PartialEq for BoolCharListMultimap {
...
impl fmt::Display for BoolCharListMultimap {
...
mod tests {
    ...
    #[test]
    fn test_put_get() {
        let mut m = BoolCharListMultimap::new();
        m.put(true, 'a');
        m.put(true, 'b');
        m.put(false, 'c');
        assert_eq!(m.get(true).len(), 2);
        assert_eq!(m.get(false).len(), 1);
    ...
    fn test_contains_key_value() {
        ...
        m.put(true, 'a');
        m.put(true, 'b');
        assert!(m.contains_key_value(true, 'a'));
        assert!(m.contains_key_value(true, 'b'));
        assert!(!m.contains_key_value(true, 'c'));
    }
    ...
    #[test]
    fn test_equals() {
        let mut m1 = BoolCharListMultimap::new();
        m1.put(true, 'a');
        m1.put(true, 'b');
        let mut m2 = BoolCharListMultimap::new();
        m2.put(true, 'a');
        m2.put(true, 'b');
        assert_eq!(m1, m2);
    }
    ...
    fn test_with_key_value_fluent() {
        let m = BoolCharListMultimap::new()
            .with_key_value(true, 'a')
            .with_key_value(true, 'b')
            .with_key_value(false, 'c');
        assert_eq!(m.size(), 3);
        assert_eq!(m.keys_count(), 2);
    }
