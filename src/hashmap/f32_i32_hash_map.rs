// AUTO-GENERATED. DO NOT EDIT.

use crate::hash_table::OpenHashMap;
use std::fmt;

/// Hash map from `f32` keys to `i32` values.
/// Open-addressing with linear probing and Robin Hood backward-shift deletion.
#[derive(Debug, Clone)]
pub struct F32I32HashMap {
    inner: OpenHashMap<f32, i32>,
}

impl F32I32HashMap {
    pub fn new() -> Self {
        F32I32HashMap {
            inner: OpenHashMap::new(),
        }
    }

    pub fn with_capacity(capacity: usize) -> Self {
        F32I32HashMap {
            inner: OpenHashMap::with_capacity(capacity),
        }
    }

    /// Inserts a key-value pair. Returns the old value if the key was already present.
    pub fn insert(&mut self, key: f32, value: i32) -> Option<i32> {
        self.inner.insert(key, value)
    }

    /// Returns the value for the key, or None.
    pub fn get(&self, key: f32) -> Option<i32> {
        self.inner.get(key)
    }

    /// Returns the value for the key, or the default.
    pub fn get_or_default(&self, key: f32, default: i32) -> i32 {
        self.inner.get(key).unwrap_or(default)
    }

    /// Removes the key. Returns the old value if present.
    pub fn remove(&mut self, key: f32) -> Option<i32> {
        self.inner.remove(key)
    }

    pub fn contains_key(&self, key: f32) -> bool {
        self.inner.contains_key(key)
    }

    pub fn contains_value(&self, value: i32) -> bool {
        self.inner.iter().any(|(_, v)| v == value)
    }

    pub fn len(&self) -> usize {
        self.inner.len()
    }
    pub fn is_empty(&self) -> bool {
        self.inner.is_empty()
    }
    pub fn clear(&mut self) {
        self.inner.clear();
    }

    /// Ensures the map can accept `additional` more entries without a
    /// rehash. Returns `TryReserveError` on allocator failure. See
    /// `docs/rust/error-handling.md`.
    pub fn try_reserve(
        &mut self,
        additional: usize,
    ) -> Result<(), std::collections::TryReserveError> {
        self.inner.try_reserve(additional)
    }

    /// Iterates over (key, value) pairs.
    pub fn iter(&self) -> impl Iterator<Item = (f32, i32)> + '_ {
        self.inner.iter()
    }

    pub fn keys(&self) -> impl Iterator<Item = f32> + '_ {
        self.inner.iter().map(|(k, _)| k)
    }

    pub fn values(&self) -> impl Iterator<Item = i32> + '_ {
        self.inner.iter().map(|(_, v)| v)
    }

    pub fn for_each(&self, mut f: impl FnMut(f32, i32)) {
        for (k, v) in self.iter() {
            f(k, v);
        }
    }

    pub fn select(&self, predicate: impl Fn(f32, i32) -> bool) -> Self {
        let mut result = Self::new();
        for (k, v) in self.iter() {
            if predicate(k, v) {
                result.insert(k, v);
            }
        }
        result
    }

    pub fn reject(&self, predicate: impl Fn(f32, i32) -> bool) -> Self {
        let mut result = Self::new();
        for (k, v) in self.iter() {
            if !predicate(k, v) {
                result.insert(k, v);
            }
        }
        result
    }

    pub fn detect(&self, predicate: impl Fn(f32, i32) -> bool) -> Option<(f32, i32)> {
        self.iter().find(|&(k, v)| predicate(k, v))
    }

    pub fn any_satisfy(&self, predicate: impl Fn(f32, i32) -> bool) -> bool {
        self.iter().any(|(k, v)| predicate(k, v))
    }

    pub fn all_satisfy(&self, predicate: impl Fn(f32, i32) -> bool) -> bool {
        self.iter().all(|(k, v)| predicate(k, v))
    }

    pub fn none_satisfy(&self, predicate: impl Fn(f32, i32) -> bool) -> bool {
        !self.iter().any(|(k, v)| predicate(k, v))
    }

    pub fn count(&self, predicate: impl Fn(f32, i32) -> bool) -> usize {
        self.iter().filter(|&(k, v)| predicate(k, v)).count()
    }

    pub fn inject_into<R>(&self, initial: R, mut f: impl FnMut(R, f32, i32) -> R) -> R {
        let mut acc = initial;
        for (k, v) in self.iter() {
            acc = f(acc, k, v);
        }
        acc
    }

    pub fn keys_to_vec(&self) -> Vec<f32> {
        self.keys().collect()
    }
    pub fn values_to_vec(&self) -> Vec<i32> {
        self.values().collect()
    }

    pub fn with_key_value(mut self, key: f32, value: i32) -> Self {
        self.insert(key, value);
        self
    }

    pub fn without_key(mut self, key: f32) -> Self {
        self.remove(key);
        self
    }
}

impl Default for F32I32HashMap {
    fn default() -> Self {
        Self::new()
    }
}

impl PartialEq for F32I32HashMap {
    fn eq(&self, other: &Self) -> bool {
        if self.len() != other.len() {
            return false;
        }
        self.iter()
            .all(|(k, v)| other.get(k).is_some_and(|ov| v == ov))
    }
}

impl fmt::Display for F32I32HashMap {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "{{")?;
        let mut first = true;
        for (k, v) in self.iter() {
            if !first {
                write!(f, ", ")?;
            }
            write!(f, "{}={}", k, v)?;
            first = false;
        }
        write!(f, "}}")
    }
}

#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn test_insert_get() {
        let mut m = F32I32HashMap::new();
        m.insert(1.0f32, 1);
        m.insert(2.0f32, 2);
        m.insert(3.0f32, 3);
        assert_eq!(m.get(1.0f32), Some(1));
        assert_eq!(m.get(99.0f32), None);
        assert_eq!(m.len(), 3);
    }

    #[test]
    fn test_insert_overwrite() {
        let mut m = F32I32HashMap::new();
        m.insert(1.0f32, 1);
        let old = m.insert(1.0f32, 2);
        assert_eq!(old, Some(1));
        assert_eq!(m.get(1.0f32), Some(2));
    }

    #[test]
    fn test_remove() {
        let mut m = F32I32HashMap::new();
        m.insert(1.0f32, 1);
        m.insert(2.0f32, 2);
        assert_eq!(m.remove(1.0f32), Some(1));
        assert_eq!(m.len(), 1);
        assert!(!m.contains_key(1.0f32));
    }

    #[test]
    fn test_contains_key() {
        let mut m = F32I32HashMap::new();
        m.insert(1.0f32, 1);
        assert!(m.contains_key(1.0f32));
        assert!(!m.contains_key(99.0f32));
    }

    #[test]
    fn test_get_or_default() {
        let mut m = F32I32HashMap::new();
        m.insert(1.0f32, 1);
        assert_eq!(m.get_or_default(1.0f32, 3), 1);
        assert_eq!(m.get_or_default(99.0f32, 3), 3);
    }

    #[test]
    fn test_clear() {
        let mut m = F32I32HashMap::new();
        m.insert(1.0f32, 1);
        m.clear();
        assert!(m.is_empty());
    }

    #[test]
    fn test_select_reject() {
        let mut m = F32I32HashMap::new();
        m.insert(1.0f32, 1);
        m.insert(2.0f32, 2);
        m.insert(3.0f32, 3);
        assert_eq!(m.select(|_k, v| v > 1).len(), 2);
        assert_eq!(m.reject(|_k, v| v > 1).len(), 1);
    }

    #[test]
    fn test_any_all_none() {
        let mut m = F32I32HashMap::new();
        m.insert(1.0f32, 1);
        m.insert(2.0f32, 2);
        assert!(m.any_satisfy(|_k, v| v == 2));
        assert!(m.all_satisfy(|_k, v| v > 0));
        assert!(m.none_satisfy(|_k, v| v == 99));
    }

    #[test]
    fn test_equals() {
        let mut m1 = F32I32HashMap::new();
        m1.insert(1.0f32, 1);
        m1.insert(2.0f32, 2);
        let mut m2 = F32I32HashMap::new();
        m2.insert(2.0f32, 2);
        m2.insert(1.0f32, 1);
        assert_eq!(m1, m2);
    }

    #[test]
    fn test_display() {
        let mut m = F32I32HashMap::new();
        m.insert(1.0f32, 1);
        assert!(!m.to_string().is_empty());
    }

    #[test]
    fn test_resize() {
        let mut m = F32I32HashMap::new();
        for i in 0..100 {
            m.insert(i as f32, (i * 10) as i32);
        }
        assert!(m.len() > 0);
    }

    #[test]
    fn test_try_reserve_happy_path() {
        let mut m = F32I32HashMap::new();
        m.try_reserve(100).unwrap();
    }

    #[test]
    fn test_try_reserve_propagates_capacity_overflow() {
        let mut m = F32I32HashMap::new();
        assert!(m.try_reserve(usize::MAX / 2).is_err());
    }

    // -----------------------------------------------------------------------
    // IEEE 754 edge cases — float key semantics
    // -----------------------------------------------------------------------
    // Locks in bit-level equality:  NaN keys are findable, ±0.0 are distinct,
    // ±Inf round-trip cleanly. See docs/float-nan-semantics-audit.md.

    #[test]
    fn test_nan_key_findable() {
        let mut m = F32I32HashMap::new();
        m.insert(f32::NAN, 1);
        assert!(m.contains_key(f32::NAN));
        assert_eq!(m.get(f32::NAN), Some(1));
    }

    #[test]
    fn test_nan_key_replaces_does_not_duplicate() {
        let mut m = F32I32HashMap::new();
        m.insert(f32::NAN, 1);
        m.insert(f32::NAN, 2);
        m.insert(f32::NAN, 3);
        assert_eq!(m.len(), 1);
        assert_eq!(m.get(f32::NAN), Some(3));
    }

    #[test]
    fn test_nan_key_remove() {
        let mut m = F32I32HashMap::new();
        m.insert(f32::NAN, 1);
        let removed = m.remove(f32::NAN);
        assert_eq!(removed, Some(1));
        assert_eq!(m.len(), 0);
        assert!(!m.contains_key(f32::NAN));
    }

    #[test]
    fn test_negative_zero_distinct_from_positive_zero() {
        let mut m = F32I32HashMap::new();
        m.insert(0.0f32, 1);
        m.insert(-0.0f32, 2);
        assert_eq!(m.len(), 2);
        assert_eq!(m.get(0.0f32), Some(1));
        assert_eq!(m.get(-0.0f32), Some(2));
    }

    #[test]
    fn test_infinity_keys() {
        let mut m = F32I32HashMap::new();
        m.insert(f32::INFINITY, 1);
        m.insert(f32::NEG_INFINITY, 2);
        assert_eq!(m.len(), 2);
        assert_eq!(m.get(f32::INFINITY), Some(1));
        assert_eq!(m.get(f32::NEG_INFINITY), Some(2));
        assert!(m.contains_key(f32::INFINITY));
        assert!(m.contains_key(f32::NEG_INFINITY));
    }
}

impl crate::traits::f32_i32_map::F32I32Map for F32I32HashMap {
    fn len(&self) -> usize {
        self.len()
    }
    fn contains_key(&self, key: f32) -> bool {
        self.contains_key(key)
    }
    fn get(&self, key: f32) -> Option<i32> {
        self.get(key)
    }
    fn iter(&self) -> impl Iterator<Item = (f32, i32)> + '_ {
        self.iter()
    }
}

impl crate::traits::f32_i32_map::F32I32MutableMap for F32I32HashMap {
    fn insert(&mut self, key: f32, value: i32) -> Option<i32> {
        self.insert(key, value)
    }
    fn remove(&mut self, key: f32) -> Option<i32> {
        self.remove(key)
    }
    fn clear(&mut self) {
        self.clear()
    }
}
