...
/// Read-only trait for any map from `char` keys to `bool` values.
///
...
pub trait CharBoolMap {
    ...
    fn for_each_key_value(&self, mut f: impl FnMut(char, bool)) {
...
/// Mutable map trait extending CharBoolMap.
pub trait CharBoolMutableMap: CharBoolMap {
...
mod verify {
    use super::*;
    fn _assert_map<T: CharBoolMap>() {}
    fn _assert_mut<T: CharBoolMutableMap>() {}

    /// Compile-time verification: every concrete `char` → `bool` map type
    /// implements the read-only and (where applicable) mutable map trait.
    #[test]
    fn types_implement_traits() {
        _assert_map::<crate::hashmap::char_bool_hash_map::CharBoolHashMap>();
        _assert_mut::<crate::hashmap::char_bool_hash_map::CharBoolHashMap>();
        _assert_map::<crate::treemap::char_bool_tree_map::CharBoolTreeMap>();
        _assert_mut::<crate::treemap::char_bool_tree_map::CharBoolTreeMap>();
        _assert_map::<crate::hashmap::char_bool_hash_bi_map::CharBoolHashBiMap>();
        _assert_mut::<crate::hashmap::char_bool_hash_bi_map::CharBoolHashBiMap>();
        _assert_map::<crate::immutable::immutable_char_bool_hash_map::ImmutableCharBoolHashMap>();
    }
}
