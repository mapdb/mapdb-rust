...
impl I16F32HashMap {
    ...
    pub fn get_or_default(&self, key: i16, default: f32) -> f32 {
    ...
    pub fn detect(&self, predicate: impl Fn(i16, f32) -> bool) -> Option<(i16, f32)> {
        self.iter().find(|&(k, v)| predicate(k, v))
    }

    pub fn any_satisfy(&self, predicate: impl Fn(i16, f32) -> bool) -> bool {
        self.iter().any(|(k, v)| predicate(k, v))
    }

    pub fn all_satisfy(&self, predicate: impl Fn(i16, f32) -> bool) -> bool {
        self.iter().all(|(k, v)| predicate(k, v))
    }

    pub fn none_satisfy(&self, predicate: impl Fn(i16, f32) -> bool) -> bool {
        !self.iter().any(|(k, v)| predicate(k, v))
    }

    pub fn count(&self, predicate: impl Fn(i16, f32) -> bool) -> usize {
        self.iter().filter(|&(k, v)| predicate(k, v)).count()
    }

    pub fn inject_into<R>(&self, initial: R, mut f: impl FnMut(R, i16, f32) -> R) -> R {
        let mut acc = initial;
        for (k, v) in self.iter() {
            acc = f(acc, k, v);
        }
...
impl Default for I16F32HashMap {
    fn default() -> Self {
        Self::new()
    }
}

impl PartialEq for I16F32HashMap {
...
impl fmt::Display for I16F32HashMap {
...
mod tests {
    ...
    #[test]
    fn test_equals() {
        let mut m1 = I16F32HashMap::new();
        m1.insert(1, 1.0f32);
        m1.insert(2, 2.0f32);
        let mut m2 = I16F32HashMap::new();
        m2.insert(2, 2.0f32);
        m2.insert(1, 1.0f32);
        assert_eq!(m1, m2);
    }
...
impl crate::traits::i16_f32_map::I16F32Map for I16F32HashMap {
    fn len(&self) -> usize {
        self.len()
    }
...
impl crate::traits::i16_f32_map::I16F32MutableMap for I16F32HashMap {
