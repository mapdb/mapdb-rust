...
/// List multimap from `i8` keys to `char` values.
...
#[derive(Debug, Clone)]
pub struct I8CharListMultimap {
    inner: HashMap<i8, Vec<char>>,
    total_size: usize,
}

impl I8CharListMultimap {
    pub fn new() -> Self {
        I8CharListMultimap {
            inner: HashMap::new(),
            total_size: 0,
    ...
    pub fn put(&mut self, key: i8, value: char) {
    ...
    pub fn get_all(&self, key: i8) -> Vec<char> {
        self.get(key).to_vec()
    }

    /// Removes all values for the given key. Returns the removed values.
    pub fn remove_all(&mut self, key: i8) -> Vec<char> {
        let internal_key = key;
    ...
    pub fn contains_key_value(&self, key: i8, value: char) -> bool {
    ...
    pub fn for_each_key_value(&self, mut f: impl FnMut(i8, &[char])) {
        for (k, vals) in &self.inner {
            f(*k, vals);
        }
    }
...
impl Default for I8CharListMultimap {
    fn default() -> Self {
        Self::new()
    }
}

impl PartialEq for I8CharListMultimap {
...
impl fmt::Display for I8CharListMultimap {
...
mod tests {
    ...
    #[test]
    fn test_put_get() {
        let mut m = I8CharListMultimap::new();
        m.put(1, 'a');
        m.put(1, 'b');
        m.put(2, 'c');
    ...
    #[test]
    fn test_equals() {
        let mut m1 = I8CharListMultimap::new();
        m1.put(1, 'a');
        m1.put(1, 'b');
        let mut m2 = I8CharListMultimap::new();
        m2.put(1, 'a');
        m2.put(1, 'b');
        assert_eq!(m1, m2);
    }
