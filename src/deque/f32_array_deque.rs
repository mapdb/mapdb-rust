...
/// Double-ended queue of `f32` values, backed by `VecDeque`.
/// O(1) amortized push/pop at both ends.
#[derive(Debug, Clone)]
pub struct F32ArrayDeque {
    inner: VecDeque<f32>,
}
...
impl F32ArrayDeque {
    ...
    pub fn add_first(&mut self, value: f32) {
    ...
    pub fn remove_first(&mut self) -> Option<f32> {
        self.inner.pop_front()
    }

    /// Removes and returns the back element, or None if empty.
    pub fn remove_last(&mut self) -> Option<f32> {
        self.inner.pop_back()
    }
    ...
    pub fn contains(&self, value: f32) -> bool {
        self.inner.iter().any(|&v| v.to_bits() == value.to_bits())
    }
...
impl Default for F32ArrayDeque {
    fn default() -> Self {
        Self::new()
    }
}

impl PartialEq for F32ArrayDeque {
...
impl fmt::Display for F32ArrayDeque {
...
mod tests {
    ...
    fn test_add_last_remove_first_fifo() {
        let mut d = F32ArrayDeque::new();
        d.add_last(1.0f32);
        d.add_last(2.0f32);
        d.add_last(3.0f32);
        assert_eq!(d.len(), 3);
        assert_eq!(d.remove_first(), Some(1.0f32));
        assert_eq!(d.remove_first(), Some(2.0f32));
    ...
    fn test_add_first_remove_last() {
        ...
        assert_eq!(d.peek_last(), Some(1.0f32));
        assert_eq!(d.remove_last(), Some(1.0f32));
        assert_eq!(d.remove_last(), Some(2.0f32));
        assert_eq!(d.remove_last(), Some(3.0f32));
    }
    ...
    fn test_contains() {
        ...
        assert!(!d.contains(3.0f32));
    }

    #[test]
    fn test_clear() {
        let mut d = F32ArrayDeque::of(&[1.0f32, 2.0f32]);
        d.clear();
        assert!(d.is_empty());
    }
    ...
    fn test_iter_front_to_back() {
        let d = F32ArrayDeque::of(&[1.0f32, 2.0f32, 3.0f32]);
