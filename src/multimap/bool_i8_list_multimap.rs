...
#[derive(Debug, Clone)]
pub struct BoolI8ListMultimap {
    inner: HashMap<bool, Vec<i8>>,
    total_size: usize,
}

impl BoolI8ListMultimap {
    pub fn new() -> Self {
        BoolI8ListMultimap {
            inner: HashMap::new(),
            total_size: 0,
    ...
    pub fn put(&mut self, key: bool, value: i8) {
    ...
    pub fn get_all(&self, key: bool) -> Vec<i8> {
        self.get(key).to_vec()
    }

    /// Removes all values for the given key. Returns the removed values.
    pub fn remove_all(&mut self, key: bool) -> Vec<i8> {
        let internal_key = key;
    ...
    pub fn contains_key_value(&self, key: bool, value: i8) -> bool {
    ...
    pub fn for_each_key_value(&self, mut f: impl FnMut(bool, &[i8])) {
        for (k, vals) in &self.inner {
            f(*k, vals);
        }
    }
...
impl Default for BoolI8ListMultimap {
    fn default() -> Self {
        Self::new()
    }
}

impl PartialEq for BoolI8ListMultimap {
...
impl fmt::Display for BoolI8ListMultimap {
...
mod tests {
    ...
    #[test]
    fn test_put_get() {
        let mut m = BoolI8ListMultimap::new();
        m.put(true, 1);
        m.put(true, 2);
        m.put(false, 3);
    ...
    #[test]
    fn test_equals() {
        let mut m1 = BoolI8ListMultimap::new();
        m1.put(true, 1);
        m1.put(true, 2);
        let mut m2 = BoolI8ListMultimap::new();
        m2.put(true, 1);
        m2.put(true, 2);
        assert_eq!(m1, m2);
    }
