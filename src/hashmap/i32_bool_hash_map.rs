...
impl I32BoolHashMap {
    ...
    pub fn reject(&self, predicate: impl Fn(i32, bool) -> bool) -> Self {
        let mut result = Self::new();
        for (k, v) in self.iter() {
            if !predicate(k, v) {
                result.insert(k, v);
    ...
    pub fn detect(&self, predicate: impl Fn(i32, bool) -> bool) -> Option<(i32, bool)> {
    ...
    pub fn none_satisfy(&self, predicate: impl Fn(i32, bool) -> bool) -> bool {
        !self.iter().any(|(k, v)| predicate(k, v))
    }

    pub fn count(&self, predicate: impl Fn(i32, bool) -> bool) -> usize {
        self.iter().filter(|&(k, v)| predicate(k, v)).count()
    }

    pub fn inject_into<R>(&self, initial: R, mut f: impl FnMut(R, i32, bool) -> R) -> R {
        let mut acc = initial;
        for (k, v) in self.iter() {
            acc = f(acc, k, v);
        }
    ...
    pub fn with_key_value(mut self, key: i32, value: bool) -> Self {
...
impl Default for I32BoolHashMap {
    fn default() -> Self {
        Self::new()
    }
}

impl PartialEq for I32BoolHashMap {
...
impl fmt::Display for I32BoolHashMap {
...
mod tests {
    ...
    #[test]
    fn test_equals() {
        let mut m1 = I32BoolHashMap::new();
        m1.insert(1, true);
        m1.insert(2, false);
        let mut m2 = I32BoolHashMap::new();
        m2.insert(2, false);
        m2.insert(1, true);
        assert_eq!(m1, m2);
    }
...
impl crate::traits::i32_bool_map::I32BoolMap for I32BoolHashMap {
    fn len(&self) -> usize {
        self.len()
    }
...
impl crate::traits::i32_bool_map::I32BoolMutableMap for I32BoolHashMap {
