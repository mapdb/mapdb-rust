...
#[derive(Debug, Clone)]
pub struct F64I16ListMultimap {
    inner: HashMap<u64, Vec<i16>>,
    total_size: usize,
}

impl F64I16ListMultimap {
    pub fn new() -> Self {
        F64I16ListMultimap {
            inner: HashMap::new(),
            total_size: 0,
    ...
    pub fn put(&mut self, key: f64, value: i16) {
    ...
    pub fn get_all(&self, key: f64) -> Vec<i16> {
        self.get(key).to_vec()
    }

    /// Removes all values for the given key. Returns the removed values.
    pub fn remove_all(&mut self, key: f64) -> Vec<i16> {
        let internal_key = f64::to_bits(key);
    ...
    pub fn contains_key_value(&self, key: f64, value: i16) -> bool {
    ...
    pub fn for_each_key_value(&self, mut f: impl FnMut(f64, &[i16])) {
        for (k, vals) in &self.inner {
    ...
    pub fn to_vec(&self) -> Vec<(f64, i16)> {
...
impl Default for F64I16ListMultimap {
    fn default() -> Self {
        Self::new()
    }
}

impl PartialEq for F64I16ListMultimap {
...
impl fmt::Display for F64I16ListMultimap {
...
mod tests {
    ...
    #[test]
    fn test_put_get() {
        let mut m = F64I16ListMultimap::new();
        m.put(1.0f64, 1);
        m.put(1.0f64, 2);
        m.put(2.0f64, 3);
    ...
    #[test]
    fn test_equals() {
        let mut m1 = F64I16ListMultimap::new();
        m1.put(1.0f64, 1);
        m1.put(1.0f64, 2);
        let mut m2 = F64I16ListMultimap::new();
        m2.put(1.0f64, 1);
        m2.put(1.0f64, 2);
        assert_eq!(m1, m2);
    }
