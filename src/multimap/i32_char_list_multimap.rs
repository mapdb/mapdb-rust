...
/// List multimap from `i32` keys to `char` values.
...
#[derive(Debug, Clone)]
pub struct I32CharListMultimap {
    inner: HashMap<i32, Vec<char>>,
    total_size: usize,
}

impl I32CharListMultimap {
    pub fn new() -> Self {
        I32CharListMultimap {
            inner: HashMap::new(),
            total_size: 0,
    ...
    pub fn put(&mut self, key: i32, value: char) {
    ...
    pub fn get_all(&self, key: i32) -> Vec<char> {
        self.get(key).to_vec()
    }

    /// Removes all values for the given key. Returns the removed values.
    pub fn remove_all(&mut self, key: i32) -> Vec<char> {
        let internal_key = key;
    ...
    pub fn contains_key_value(&self, key: i32, value: char) -> bool {
    ...
    pub fn for_each_key_value(&self, mut f: impl FnMut(i32, &[char])) {
        for (k, vals) in &self.inner {
            f(*k, vals);
        }
    }
    ...
    pub fn to_vec(&self) -> Vec<(i32, char)> {
...
impl Default for I32CharListMultimap {
    fn default() -> Self {
        Self::new()
    }
}

impl PartialEq for I32CharListMultimap {
...
impl fmt::Display for I32CharListMultimap {
...
mod tests {
    ...
    #[test]
    fn test_put_get() {
        let mut m = I32CharListMultimap::new();
        m.put(1, 'a');
        m.put(1, 'b');
        m.put(2, 'c');
    ...
    fn test_contains_key_value() {
        ...
        m.put(1, 'a');
        m.put(1, 'b');
        assert!(m.contains_key_value(1, 'a'));
        assert!(m.contains_key_value(1, 'b'));
        assert!(!m.contains_key_value(1, 'c'));
    }
    ...
    #[test]
    fn test_equals() {
        let mut m1 = I32CharListMultimap::new();
        m1.put(1, 'a');
        m1.put(1, 'b');
        let mut m2 = I32CharListMultimap::new();
        m2.put(1, 'a');
        m2.put(1, 'b');
        assert_eq!(m1, m2);
    }
