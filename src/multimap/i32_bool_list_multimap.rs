...
#[derive(Debug, Clone)]
pub struct I32BoolListMultimap {
    inner: HashMap<i32, Vec<bool>>,
    total_size: usize,
}

impl I32BoolListMultimap {
    pub fn new() -> Self {
        I32BoolListMultimap {
            inner: HashMap::new(),
            total_size: 0,
    ...
    pub fn get_all(&self, key: i32) -> Vec<bool> {
        self.get(key).to_vec()
    }

    /// Removes all values for the given key. Returns the removed values.
    pub fn remove_all(&mut self, key: i32) -> Vec<bool> {
        let internal_key = key;
    ...
    pub fn contains_key_value(&self, key: i32, value: bool) -> bool {
    ...
    pub fn for_each_key_value(&self, mut f: impl FnMut(i32, &[bool])) {
        for (k, vals) in &self.inner {
            f(*k, vals);
        }
    }
...
impl Default for I32BoolListMultimap {
    fn default() -> Self {
        Self::new()
    }
}

impl PartialEq for I32BoolListMultimap {
...
impl fmt::Display for I32BoolListMultimap {
...
mod tests {
    ...
    #[test]
    fn test_put_get() {
        let mut m = I32BoolListMultimap::new();
        m.put(1, true);
        m.put(1, false);
        m.put(2, true);
    ...
    #[test]
    fn test_equals() {
        let mut m1 = I32BoolListMultimap::new();
        m1.put(1, true);
        m1.put(1, false);
        let mut m2 = I32BoolListMultimap::new();
        m2.put(1, true);
        m2.put(1, false);
        assert_eq!(m1, m2);
    }
