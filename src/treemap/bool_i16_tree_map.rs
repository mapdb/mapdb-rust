...
/// Sorted map from `bool` keys to `i16` values.
#[derive(Debug, Clone)]
pub struct BoolI16TreeMap {
    inner: BTreeMap<bool, i16>,
}
...
impl BoolI16TreeMap {
    ...
    pub fn min(&self) -> Option<(bool, i16)> {
        self.inner.iter().next().map(|(&k, &v)| (k, v))
    }

    pub fn max(&self) -> Option<(bool, i16)> {
...
impl Default for BoolI16TreeMap {
    fn default() -> Self {
        Self::new()
    }
}

impl PartialEq for BoolI16TreeMap {
...
impl fmt::Display for BoolI16TreeMap {
...
impl crate::traits::bool_i16_map::BoolI16Map for BoolI16TreeMap {
    fn len(&self) -> usize {
        self.len()
    }
...
impl crate::traits::bool_i16_map::BoolI16MutableMap for BoolI16TreeMap {
