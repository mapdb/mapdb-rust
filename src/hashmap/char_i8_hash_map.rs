...
/// Hash map from `char` keys to `i8` values.
/// Open-addressing with linear probing and Robin Hood backward-shift deletion.
#[derive(Debug, Clone)]
pub struct CharI8HashMap {
    inner: OpenHashMap<char, i8>,
}
...
impl CharI8HashMap {
    ...
    pub fn insert(&mut self, key: char, value: i8) -> Option<i8> {
    ...
    pub fn remove(&mut self, key: char) -> Option<i8> {
    ...
    pub fn reject(&self, predicate: impl Fn(char, i8) -> bool) -> Self {
        let mut result = Self::new();
        for (k, v) in self.iter() {
            if !predicate(k, v) {
                result.insert(k, v);
    ...
    pub fn detect(&self, predicate: impl Fn(char, i8) -> bool) -> Option<(char, i8)> {
    ...
    pub fn none_satisfy(&self, predicate: impl Fn(char, i8) -> bool) -> bool {
        !self.iter().any(|(k, v)| predicate(k, v))
    }

    pub fn count(&self, predicate: impl Fn(char, i8) -> bool) -> usize {
        self.iter().filter(|&(k, v)| predicate(k, v)).count()
    }

    pub fn inject_into<R>(&self, initial: R, mut f: impl FnMut(R, char, i8) -> R) -> R {
        let mut acc = initial;
        for (k, v) in self.iter() {
            acc = f(acc, k, v);
        }
    ...
    pub fn with_key_value(mut self, key: char, value: i8) -> Self {
...
impl Default for CharI8HashMap {
    fn default() -> Self {
        Self::new()
    }
}

impl PartialEq for CharI8HashMap {
...
impl fmt::Display for CharI8HashMap {
...
mod tests {
    ...
    #[test]
    fn test_equals() {
        let mut m1 = CharI8HashMap::new();
        m1.insert('a', 1);
        m1.insert('b', 2);
        let mut m2 = CharI8HashMap::new();
        m2.insert('b', 2);
        m2.insert('a', 1);
        assert_eq!(m1, m2);
    }
    ...
    fn test_resize() {
        let mut m = CharI8HashMap::new();
        for i in 0..100 {
            m.insert(char::from(b'a' + (i % 26) as u8), (i * 10) as i8);
        }
        assert!(m.len() > 0);
    }
...
impl crate::traits::char_i8_map::CharI8Map for CharI8HashMap {
    fn len(&self) -> usize {
        self.len()
    }
...
impl crate::traits::char_i8_map::CharI8MutableMap for CharI8HashMap {
