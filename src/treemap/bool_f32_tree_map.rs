...
/// Sorted map from `bool` keys to `f32` values.
#[derive(Debug, Clone)]
pub struct BoolF32TreeMap {
    inner: BTreeMap<bool, f32>,
}
...
impl BoolF32TreeMap {
    ...
    pub fn min(&self) -> Option<(bool, f32)> {
        self.inner.iter().next().map(|(&k, &v)| (k, v))
    }

    pub fn max(&self) -> Option<(bool, f32)> {
...
impl Default for BoolF32TreeMap {
    fn default() -> Self {
        Self::new()
    }
}

impl PartialEq for BoolF32TreeMap {
...
impl fmt::Display for BoolF32TreeMap {
...
impl crate::traits::bool_f32_map::BoolF32Map for BoolF32TreeMap {
    fn len(&self) -> usize {
        self.len()
    }
...
impl crate::traits::bool_f32_map::BoolF32MutableMap for BoolF32TreeMap {
