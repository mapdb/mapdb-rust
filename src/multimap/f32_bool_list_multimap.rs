...
#[derive(Debug, Clone)]
pub struct F32BoolListMultimap {
    inner: HashMap<u32, Vec<bool>>,
    total_size: usize,
}

impl F32BoolListMultimap {
    pub fn new() -> Self {
        F32BoolListMultimap {
            inner: HashMap::new(),
            total_size: 0,
    ...
    pub fn put(&mut self, key: f32, value: bool) {
    ...
    pub fn get_all(&self, key: f32) -> Vec<bool> {
        self.get(key).to_vec()
    }

    /// Removes all values for the given key. Returns the removed values.
    pub fn remove_all(&mut self, key: f32) -> Vec<bool> {
        let internal_key = f32::to_bits(key);
    ...
    pub fn contains_key_value(&self, key: f32, value: bool) -> bool {
    ...
    pub fn for_each_key_value(&self, mut f: impl FnMut(f32, &[bool])) {
        for (k, vals) in &self.inner {
...
impl Default for F32BoolListMultimap {
    fn default() -> Self {
        Self::new()
    }
}

impl PartialEq for F32BoolListMultimap {
...
impl fmt::Display for F32BoolListMultimap {
...
mod tests {
    ...
    #[test]
    fn test_put_get() {
        let mut m = F32BoolListMultimap::new();
        m.put(1.0f32, true);
        m.put(1.0f32, false);
        m.put(2.0f32, true);
    ...
    fn test_contains_key_value() {
        ...
        m.put(1.0f32, true);
        m.put(1.0f32, false);
        assert!(m.contains_key_value(1.0f32, true));
        assert!(m.contains_key_value(1.0f32, false));
    }
    ...
    #[test]
    fn test_equals() {
        let mut m1 = F32BoolListMultimap::new();
        m1.put(1.0f32, true);
        m1.put(1.0f32, false);
        let mut m2 = F32BoolListMultimap::new();
        m2.put(1.0f32, true);
        m2.put(1.0f32, false);
        assert_eq!(m1, m2);
    }
    ...
    fn test_with_key_value_fluent() {
        let m = F32BoolListMultimap::new()
            .with_key_value(1.0f32, true)
            .with_key_value(1.0f32, false)
            .with_key_value(2.0f32, true);
        assert_eq!(m.size(), 3);
        assert_eq!(m.keys_count(), 2);
    }
