...
impl I8I16HashMap {
    ...
    pub fn insert(&mut self, key: i8, value: i16) -> Option<i16> {
        self.inner.insert(key, value)
    }

    /// Returns the value for the key, or None.
    pub fn get(&self, key: i8) -> Option<i16> {
        self.inner.get(key)
    }

    /// Returns the value for the key, or the default.
    pub fn get_or_default(&self, key: i8, default: i16) -> i16 {
        self.inner.get(key).unwrap_or(default)
    }

    /// Removes the key. Returns the old value if present.
    pub fn remove(&mut self, key: i8) -> Option<i16> {
    ...
    pub fn iter(&self) -> impl Iterator<Item = (i8, i16)> + '_ {
        self.inner.iter()
    }
    ...
    pub fn for_each(&self, mut f: impl FnMut(i8, i16)) {
        for (k, v) in self.iter() {
            f(k, v);
        }
    }

    pub fn select(&self, predicate: impl Fn(i8, i16) -> bool) -> Self {
    ...
    pub fn reject(&self, predicate: impl Fn(i8, i16) -> bool) -> Self {
        let mut result = Self::new();
        for (k, v) in self.iter() {
            if !predicate(k, v) {
                result.insert(k, v);
    ...
    pub fn detect(&self, predicate: impl Fn(i8, i16) -> bool) -> Option<(i8, i16)> {
        self.iter().find(|&(k, v)| predicate(k, v))
    }

    pub fn any_satisfy(&self, predicate: impl Fn(i8, i16) -> bool) -> bool {
        self.iter().any(|(k, v)| predicate(k, v))
    }

    pub fn all_satisfy(&self, predicate: impl Fn(i8, i16) -> bool) -> bool {
        self.iter().all(|(k, v)| predicate(k, v))
    }

    pub fn none_satisfy(&self, predicate: impl Fn(i8, i16) -> bool) -> bool {
        !self.iter().any(|(k, v)| predicate(k, v))
    }

    pub fn count(&self, predicate: impl Fn(i8, i16) -> bool) -> usize {
        self.iter().filter(|&(k, v)| predicate(k, v)).count()
    }

    pub fn inject_into<R>(&self, initial: R, mut f: impl FnMut(R, i8, i16) -> R) -> R {
        let mut acc = initial;
        for (k, v) in self.iter() {
            acc = f(acc, k, v);
        }
    ...
    pub fn with_key_value(mut self, key: i8, value: i16) -> Self {
...
impl Default for I8I16HashMap {
    fn default() -> Self {
        Self::new()
    }
}

impl PartialEq for I8I16HashMap {
...
impl fmt::Display for I8I16HashMap {
...
mod tests {
    ...
    #[test]
    fn test_equals() {
        let mut m1 = I8I16HashMap::new();
        m1.insert(1, 1);
        m1.insert(2, 2);
        let mut m2 = I8I16HashMap::new();
        m2.insert(2, 2);
        m2.insert(1, 1);
        assert_eq!(m1, m2);
    }
...
impl crate::traits::i8_i16_map::I8I16Map for I8I16HashMap {
    fn len(&self) -> usize {
        self.len()
    }
    ...
    fn get(&self, key: i8) -> Option<i16> {
        self.get(key)
    }
    fn iter(&self) -> impl Iterator<Item = (i8, i16)> + '_ {
        self.iter()
    }
}

impl crate::traits::i8_i16_map::I8I16MutableMap for I8I16HashMap {
    fn insert(&mut self, key: i8, value: i16) -> Option<i16> {
        self.insert(key, value)
    }
    fn remove(&mut self, key: i8) -> Option<i16> {
        self.remove(key)
    }
    fn clear(&mut self) {
        self.clear()
