...
/// List multimap from `char` keys to `i8` values.
...
#[derive(Debug, Clone)]
pub struct CharI8ListMultimap {
    inner: HashMap<char, Vec<i8>>,
    total_size: usize,
}

impl CharI8ListMultimap {
    pub fn new() -> Self {
        CharI8ListMultimap {
            inner: HashMap::new(),
            total_size: 0,
    ...
    pub fn put(&mut self, key: char, value: i8) {
    ...
    pub fn get_all(&self, key: char) -> Vec<i8> {
        self.get(key).to_vec()
    }

    /// Removes all values for the given key. Returns the removed values.
    pub fn remove_all(&mut self, key: char) -> Vec<i8> {
        let internal_key = key;
    ...
    pub fn contains_key_value(&self, key: char, value: i8) -> bool {
    ...
    pub fn for_each_key_value(&self, mut f: impl FnMut(char, &[i8])) {
        for (k, vals) in &self.inner {
            f(*k, vals);
        }
    }
...
impl Default for CharI8ListMultimap {
    fn default() -> Self {
        Self::new()
    }
}

impl PartialEq for CharI8ListMultimap {
...
impl fmt::Display for CharI8ListMultimap {
...
mod tests {
    ...
    #[test]
    fn test_put_get() {
        let mut m = CharI8ListMultimap::new();
        m.put('a', 1);
        m.put('a', 2);
        m.put('b', 3);
    ...
    #[test]
    fn test_equals() {
        let mut m1 = CharI8ListMultimap::new();
        m1.put('a', 1);
        m1.put('a', 2);
        let mut m2 = CharI8ListMultimap::new();
        m2.put('a', 1);
        m2.put('a', 2);
        assert_eq!(m1, m2);
    }
