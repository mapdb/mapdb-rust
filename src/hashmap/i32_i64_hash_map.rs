...
impl I32I64HashMap {
    ...
    pub fn reject(&self, predicate: impl Fn(i32, i64) -> bool) -> Self {
        let mut result = Self::new();
        for (k, v) in self.iter() {
            if !predicate(k, v) {
                result.insert(k, v);
    ...
    pub fn detect(&self, predicate: impl Fn(i32, i64) -> bool) -> Option<(i32, i64)> {
    ...
    pub fn none_satisfy(&self, predicate: impl Fn(i32, i64) -> bool) -> bool {
        !self.iter().any(|(k, v)| predicate(k, v))
    }

    pub fn count(&self, predicate: impl Fn(i32, i64) -> bool) -> usize {
        self.iter().filter(|&(k, v)| predicate(k, v)).count()
    }

    pub fn inject_into<R>(&self, initial: R, mut f: impl FnMut(R, i32, i64) -> R) -> R {
        let mut acc = initial;
        for (k, v) in self.iter() {
            acc = f(acc, k, v);
        }
    ...
    pub fn with_key_value(mut self, key: i32, value: i64) -> Self {
...
impl Default for I32I64HashMap {
    fn default() -> Self {
        Self::new()
    }
}

impl PartialEq for I32I64HashMap {
...
impl fmt::Display for I32I64HashMap {
...
mod tests {
    ...
    #[test]
    fn test_equals() {
        let mut m1 = I32I64HashMap::new();
        m1.insert(1, 1);
        m1.insert(2, 2);
        let mut m2 = I32I64HashMap::new();
        m2.insert(2, 2);
        m2.insert(1, 1);
        assert_eq!(m1, m2);
    }
...
impl crate::traits::i32_i64_map::I32I64Map for I32I64HashMap {
    fn len(&self) -> usize {
        self.len()
    }
...
impl crate::traits::i32_i64_map::I32I64MutableMap for I32I64HashMap {
