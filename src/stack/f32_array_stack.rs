...
/// LIFO stack of `f32` values, backed by a Vec.
#[derive(Debug, Clone)]
pub struct F32ArrayStack {
    items: Vec<f32>,
}

impl F32ArrayStack {
    pub fn new() -> Self {
        F32ArrayStack { items: Vec::new() }
    }
...
impl Default for F32ArrayStack {
    fn default() -> Self {
        Self::new()
    }
}

impl PartialEq for F32ArrayStack {
...
impl fmt::Display for F32ArrayStack {
...
mod tests {
    ...
    fn test_push_peek_pop() {
        let mut s = F32ArrayStack::new();
        s.push(1.0f32);
        s.push(2.0f32);
        s.push(3.0f32);
        assert_eq!(s.len(), 3);
        assert_eq!(s.peek(), Some(3.0f32));
        assert_eq!(s.pop(), Some(3.0f32));
        assert_eq!(s.len(), 2);
    }
    ...
    fn test_contains() {
        ...
        assert!(!s.contains(3.0f32));
    }

    #[test]
    fn test_clear() {
        let mut s = F32ArrayStack::of(&[1.0f32]);
        s.clear();
        assert!(s.is_empty());
    }
    ...
    fn test_iter_top_to_bottom() {
        ...
        assert_eq!(vals[0], 3.0f32); // top first
    }
...
impl crate::traits::f32_collection::F32Collection for F32ArrayStack {
    fn len(&self) -> usize {
        self.len()
    }
...
impl crate::traits::f32_collection::F32MutableCollection for F32ArrayStack {
    fn clear(&mut self) {
        self.clear()
    }
}

impl crate::traits::f32_collection::F32Stack for F32ArrayStack {
    fn peek(&self) -> Option<f32> {
        self.peek()
    }
}

impl crate::traits::f32_collection::F32MutableStack for F32ArrayStack {
