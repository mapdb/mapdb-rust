...
#[derive(Debug, Clone)]
pub struct I64F32ListMultimap {
    inner: HashMap<i64, Vec<f32>>,
    total_size: usize,
}

impl I64F32ListMultimap {
    pub fn new() -> Self {
        I64F32ListMultimap {
            inner: HashMap::new(),
            total_size: 0,
    ...
    pub fn put(&mut self, key: i64, value: f32) {
    ...
    pub fn get_all(&self, key: i64) -> Vec<f32> {
        self.get(key).to_vec()
    }

    /// Removes all values for the given key. Returns the removed values.
    pub fn remove_all(&mut self, key: i64) -> Vec<f32> {
        let internal_key = key;
    ...
    pub fn contains_key_value(&self, key: i64, value: f32) -> bool {
        self.get(key)
            .iter()
    ...
    pub fn for_each_key_value(&self, mut f: impl FnMut(i64, &[f32])) {
        for (k, vals) in &self.inner {
            f(*k, vals);
        }
    }
    ...
    pub fn to_vec(&self) -> Vec<(i64, f32)> {
...
impl Default for I64F32ListMultimap {
    fn default() -> Self {
        Self::new()
    }
}

impl PartialEq for I64F32ListMultimap {
...
impl fmt::Display for I64F32ListMultimap {
...
mod tests {
    ...
    #[test]
    fn test_put_get() {
        let mut m = I64F32ListMultimap::new();
        m.put(1, 1.0f32);
        m.put(1, 2.0f32);
        m.put(2, 3.0f32);
    ...
    #[test]
    fn test_equals() {
        let mut m1 = I64F32ListMultimap::new();
        m1.put(1, 1.0f32);
        m1.put(1, 2.0f32);
        let mut m2 = I64F32ListMultimap::new();
        m2.put(1, 1.0f32);
        m2.put(1, 2.0f32);
        assert_eq!(m1, m2);
    }
