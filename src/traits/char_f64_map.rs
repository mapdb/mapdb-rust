...
/// Read-only trait for any map from `char` keys to `f64` values.
///
...
pub trait CharF64Map {
    ...
    fn for_each_key_value(&self, mut f: impl FnMut(char, f64)) {
...
/// Mutable map trait extending CharF64Map.
pub trait CharF64MutableMap: CharF64Map {
...
mod verify {
    use super::*;
    fn _assert_map<T: CharF64Map>() {}
    fn _assert_mut<T: CharF64MutableMap>() {}

    /// Compile-time verification: every concrete `char` → `f64` map type
    /// implements the read-only and (where applicable) mutable map trait.
    #[test]
    fn types_implement_traits() {
        _assert_map::<crate::hashmap::char_f64_hash_map::CharF64HashMap>();
        _assert_mut::<crate::hashmap::char_f64_hash_map::CharF64HashMap>();
        _assert_map::<crate::treemap::char_f64_tree_map::CharF64TreeMap>();
        _assert_mut::<crate::treemap::char_f64_tree_map::CharF64TreeMap>();
        _assert_map::<crate::hashmap::char_f64_hash_bi_map::CharF64HashBiMap>();
        _assert_mut::<crate::hashmap::char_f64_hash_bi_map::CharF64HashBiMap>();
        _assert_map::<crate::immutable::immutable_char_f64_hash_map::ImmutableCharF64HashMap>();
    }
}
