...
/// Sorted map from `f64` keys to `bool` values.
#[derive(Debug, Clone)]
pub struct F64BoolTreeMap {
...
impl F64BoolTreeMap {
    ...
    pub fn min(&self) -> Option<(f64, bool)> {
        self.entries.first().copied()
    }

    pub fn max(&self) -> Option<(f64, bool)> {
...
impl Default for F64BoolTreeMap {
    fn default() -> Self {
        Self::new()
    }
}

impl PartialEq for F64BoolTreeMap {
...
impl fmt::Display for F64BoolTreeMap {
...
impl crate::traits::f64_bool_map::F64BoolMap for F64BoolTreeMap {
    fn len(&self) -> usize {
        self.len()
    }
...
impl crate::traits::f64_bool_map::F64BoolMutableMap for F64BoolTreeMap {
