...
use crate::hashmap::i8_f32_hash_map::I8F32HashMap;
use std::collections::HashMap;
use std::fmt;

/// Immutable hash map from `i8` keys to `f32` values. Clone is cheap (shared data).
#[derive(Debug, Clone)]
pub struct ImmutableI8F32HashMap {
    inner: HashMap<i8, f32>,
}

impl ImmutableI8F32HashMap {
    pub fn from_mutable(m: &I8F32HashMap) -> Self {
        let mut inner = HashMap::new();
        m.for_each(|k, v| {
            inner.insert(k, v);
        });
        ImmutableI8F32HashMap { inner }
    }
    ...
    pub fn to_mutable(&self) -> I8F32HashMap {
...
impl PartialEq for ImmutableI8F32HashMap {
...
impl fmt::Display for ImmutableI8F32HashMap {
...
mod tests {
    ...
    fn test_get() {
        ...
        m.insert(1, 1.0f32);
        m.insert(2, 2.0f32);
        let im = ImmutableI8F32HashMap::from_mutable(&m);
    ...
    fn test_display() {
        let mut m = I8F32HashMap::new();
        m.insert(1, 1.0f32);
        assert!(!ImmutableI8F32HashMap::from_mutable(&m)
            .to_string()
            .is_empty());
    }
}

impl crate::traits::i8_f32_map::I8F32Map for ImmutableI8F32HashMap {
    fn len(&self) -> usize {
        self.len()
    }
