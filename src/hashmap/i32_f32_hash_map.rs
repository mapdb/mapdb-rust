...
impl I32F32HashMap {
    ...
    pub fn insert(&mut self, key: i32, value: f32) -> Option<f32> {
        self.inner.insert(key, value)
    }

    /// Returns the value for the key, or None.
    pub fn get(&self, key: i32) -> Option<f32> {
        self.inner.get(key)
    }

    /// Returns the value for the key, or the default.
    pub fn get_or_default(&self, key: i32, default: f32) -> f32 {
        self.inner.get(key).unwrap_or(default)
    }

    /// Removes the key. Returns the old value if present.
    pub fn remove(&mut self, key: i32) -> Option<f32> {
    ...
    pub fn for_each(&self, mut f: impl FnMut(i32, f32)) {
        for (k, v) in self.iter() {
            f(k, v);
        }
    }

    pub fn select(&self, predicate: impl Fn(i32, f32) -> bool) -> Self {
    ...
    pub fn reject(&self, predicate: impl Fn(i32, f32) -> bool) -> Self {
        let mut result = Self::new();
        for (k, v) in self.iter() {
            if !predicate(k, v) {
                result.insert(k, v);
    ...
    pub fn detect(&self, predicate: impl Fn(i32, f32) -> bool) -> Option<(i32, f32)> {
        self.iter().find(|&(k, v)| predicate(k, v))
    }

    pub fn any_satisfy(&self, predicate: impl Fn(i32, f32) -> bool) -> bool {
        self.iter().any(|(k, v)| predicate(k, v))
    }

    pub fn all_satisfy(&self, predicate: impl Fn(i32, f32) -> bool) -> bool {
        self.iter().all(|(k, v)| predicate(k, v))
    }

    pub fn none_satisfy(&self, predicate: impl Fn(i32, f32) -> bool) -> bool {
        !self.iter().any(|(k, v)| predicate(k, v))
    }

    pub fn count(&self, predicate: impl Fn(i32, f32) -> bool) -> usize {
        self.iter().filter(|&(k, v)| predicate(k, v)).count()
    }

    pub fn inject_into<R>(&self, initial: R, mut f: impl FnMut(R, i32, f32) -> R) -> R {
        let mut acc = initial;
        for (k, v) in self.iter() {
            acc = f(acc, k, v);
        }
    ...
    pub fn with_key_value(mut self, key: i32, value: f32) -> Self {
...
impl Default for I32F32HashMap {
    fn default() -> Self {
        Self::new()
    }
}

impl PartialEq for I32F32HashMap {
...
impl fmt::Display for I32F32HashMap {
...
mod tests {
    ...
    #[test]
    fn test_equals() {
        let mut m1 = I32F32HashMap::new();
        m1.insert(1, 1.0f32);
        m1.insert(2, 2.0f32);
        let mut m2 = I32F32HashMap::new();
        m2.insert(2, 2.0f32);
        m2.insert(1, 1.0f32);
        assert_eq!(m1, m2);
    }
...
impl crate::traits::i32_f32_map::I32F32Map for I32F32HashMap {
    fn len(&self) -> usize {
        self.len()
    }
...
impl crate::traits::i32_f32_map::I32F32MutableMap for I32F32HashMap {
