...
use crate::hashmap::f64_i8_hash_bi_map::F64I8HashBiMap;
use std::fmt;
...
impl I8F64HashBiMap {
    ...
    pub fn get_key(&self, value: f64) -> Option<i8> {
    ...
    pub fn remove_value(&mut self, value: f64) -> Option<i8> {
    ...
    pub fn inverse(&self) -> F64I8HashBiMap {
        F64I8HashBiMap::from_maps(self.reverse.clone(), self.forward.clone())
    }

    /// Internal constructor from pre-built maps (used by the inverse type's `inverse()` call).
    pub(crate) fn from_maps(forward: OpenHashMap<i8, f64>, reverse: OpenHashMap<f64, i8>) -> Self {
        I8F64HashBiMap { forward, reverse }
    }
}

impl Default for I8F64HashBiMap {
    fn default() -> Self {
        Self::new()
    }
}

impl PartialEq for I8F64HashBiMap {
...
impl fmt::Display for I8F64HashBiMap {
...
mod tests {
    ...
    #[test]
    fn test_insert_get() {
        let mut m = I8F64HashBiMap::new();
        m.insert(1, 1.0f64);
        m.insert(2, 2.0f64);
        assert_eq!(m.get(1), Some(1.0f64));
        assert_eq!(m.get(2), Some(2.0f64));
        assert_eq!(m.len(), 2);
    }
    ...
    fn test_get_key() {
        ...
        m.insert(1, 1.0f64);
        m.insert(2, 2.0f64);
        assert_eq!(m.get_key(1.0f64), Some(1));
        assert_eq!(m.get_key(2.0f64), Some(2));
    ...
    fn test_insert_overwrite() {
        ...
        assert_eq!(m.get_key(2.0f64), Some(1));
    ...
    fn test_remove_value() {
        ...
        m.insert(1, 1.0f64);
        m.insert(2, 2.0f64);
        assert_eq!(m.remove_value(1.0f64), Some(1));
        assert_eq!(m.len(), 1);
        assert!(!m.contains_key(1));
        assert!(!m.contains_value(1.0f64));
    }
    ...
    fn test_inverse() {
        ...
        assert_eq!(inv.get(1.0f64), Some(1));
        assert_eq!(inv.get(2.0f64), Some(2));
        // inverse reverse lookup: old key -> old value
        assert_eq!(inv.get_key(1), Some(1.0f64));
        assert_eq!(inv.len(), 2);
    }
...
impl crate::traits::i8_f64_map::I8F64Map for I8F64HashBiMap {
    fn len(&self) -> usize {
        self.len()
    }
...
impl crate::traits::i8_f64_map::I8F64MutableMap for I8F64HashBiMap {
