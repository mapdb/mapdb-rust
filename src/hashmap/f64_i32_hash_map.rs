...
impl F64I32HashMap {
    ...
    pub fn insert(&mut self, key: f64, value: i32) -> Option<i32> {
    ...
    pub fn remove(&mut self, key: f64) -> Option<i32> {
    ...
    pub fn reject(&self, predicate: impl Fn(f64, i32) -> bool) -> Self {
        let mut result = Self::new();
        for (k, v) in self.iter() {
            if !predicate(k, v) {
                result.insert(k, v);
    ...
    pub fn detect(&self, predicate: impl Fn(f64, i32) -> bool) -> Option<(f64, i32)> {
    ...
    pub fn none_satisfy(&self, predicate: impl Fn(f64, i32) -> bool) -> bool {
        !self.iter().any(|(k, v)| predicate(k, v))
    }

    pub fn count(&self, predicate: impl Fn(f64, i32) -> bool) -> usize {
        self.iter().filter(|&(k, v)| predicate(k, v)).count()
    }

    pub fn inject_into<R>(&self, initial: R, mut f: impl FnMut(R, f64, i32) -> R) -> R {
        let mut acc = initial;
        for (k, v) in self.iter() {
            acc = f(acc, k, v);
        }
    ...
    pub fn with_key_value(mut self, key: f64, value: i32) -> Self {
...
impl Default for F64I32HashMap {
    fn default() -> Self {
        Self::new()
    }
}

impl PartialEq for F64I32HashMap {
...
impl fmt::Display for F64I32HashMap {
...
mod tests {
    ...
    #[test]
    fn test_equals() {
        let mut m1 = F64I32HashMap::new();
        m1.insert(1.0f64, 1);
        m1.insert(2.0f64, 2);
        let mut m2 = F64I32HashMap::new();
        m2.insert(2.0f64, 2);
        m2.insert(1.0f64, 1);
        assert_eq!(m1, m2);
    }
...
impl crate::traits::f64_i32_map::F64I32Map for F64I32HashMap {
    fn len(&self) -> usize {
        self.len()
    }
...
impl crate::traits::f64_i32_map::F64I32MutableMap for F64I32HashMap {
    fn insert(&mut self, key: f64, value: i32) -> Option<i32> {
        self.insert(key, value)
    }
    fn remove(&mut self, key: f64) -> Option<i32> {
        self.remove(key)
    }
    fn clear(&mut self) {
        self.clear()
