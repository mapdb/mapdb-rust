...
impl F64I64HashMap {
    ...
    pub fn insert(&mut self, key: f64, value: i64) -> Option<i64> {
    ...
    pub fn remove(&mut self, key: f64) -> Option<i64> {
    ...
    pub fn detect(&self, predicate: impl Fn(f64, i64) -> bool) -> Option<(f64, i64)> {
    ...
    pub fn none_satisfy(&self, predicate: impl Fn(f64, i64) -> bool) -> bool {
        !self.iter().any(|(k, v)| predicate(k, v))
    }

    pub fn count(&self, predicate: impl Fn(f64, i64) -> bool) -> usize {
        self.iter().filter(|&(k, v)| predicate(k, v)).count()
    }

    pub fn inject_into<R>(&self, initial: R, mut f: impl FnMut(R, f64, i64) -> R) -> R {
        let mut acc = initial;
        for (k, v) in self.iter() {
            acc = f(acc, k, v);
        }
...
impl Default for F64I64HashMap {
    fn default() -> Self {
        Self::new()
    }
}

impl PartialEq for F64I64HashMap {
...
impl fmt::Display for F64I64HashMap {
...
mod tests {
    ...
    #[test]
    fn test_equals() {
        let mut m1 = F64I64HashMap::new();
        m1.insert(1.0f64, 1);
        m1.insert(2.0f64, 2);
        let mut m2 = F64I64HashMap::new();
        m2.insert(2.0f64, 2);
        m2.insert(1.0f64, 1);
        assert_eq!(m1, m2);
    }
...
impl crate::traits::f64_i64_map::F64I64Map for F64I64HashMap {
    fn len(&self) -> usize {
        self.len()
    }
...
impl crate::traits::f64_i64_map::F64I64MutableMap for F64I64HashMap {
    fn insert(&mut self, key: f64, value: i64) -> Option<i64> {
        self.insert(key, value)
    }
    fn remove(&mut self, key: f64) -> Option<i64> {
        self.remove(key)
    }
    fn clear(&mut self) {
        self.clear()
