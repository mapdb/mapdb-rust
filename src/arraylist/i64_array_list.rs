...
impl I64ArrayList {
    ...
    /// # Panics
    /// Panics if index is out of bounds.
    pub fn remove_at_index(&mut self, index: usize) -> i64 {
    ...
    pub fn reject(&self, predicate: impl Fn(i64) -> bool) -> Self {
        I64ArrayList {
            items: self
                .items
                .iter()
    ...
    pub fn detect(&self, predicate: impl Fn(i64) -> bool) -> Option<i64> {
        self.items.iter().copied().find(|&v| predicate(v))
    }

    /// Returns true if any element satisfies the predicate.
    pub fn any_satisfy(&self, predicate: impl Fn(i64) -> bool) -> bool {
        self.items.iter().any(|&v| predicate(v))
    }

    /// Returns true if all elements satisfy the predicate.
    pub fn all_satisfy(&self, predicate: impl Fn(i64) -> bool) -> bool {
        self.items.iter().all(|&v| predicate(v))
    }

    /// Returns true if no element satisfies the predicate.
    pub fn none_satisfy(&self, predicate: impl Fn(i64) -> bool) -> bool {
        !self.items.iter().any(|&v| predicate(v))
    }

    /// Returns the count of elements satisfying the predicate.
    pub fn count(&self, predicate: impl Fn(i64) -> bool) -> usize {
    ...
    pub fn inject_into<R>(&self, initial: R, mut f: impl FnMut(R, i64) -> R) -> R {
        let mut acc = initial;
        for &v in &self.items {
            acc = f(acc, v);
        }
    ...
    pub fn with(mut self, value: i64) -> Self {
        self.push(value);
        self
    }

    /// Returns the list after removing a value (fluent API).
    pub fn without(mut self, value: i64) -> Self {
        self.remove(value);
        self
    }
}

impl Default for I64ArrayList {
    fn default() -> Self {
        Self::new()
    }
}

impl PartialEq for I64ArrayList {
...
impl Eq for I64ArrayList {}

impl fmt::Display for I64ArrayList {
...
impl IntoIterator for I64ArrayList {
    type Item = i64;
...
impl<'a> IntoIterator for &'a I64ArrayList {
    type Item = &'a i64;
...
impl std::ops::Index<usize> for I64ArrayList {
    type Output = i64;
...
mod tests {
    ...
    #[test]
    fn test_of() {
        let l = I64ArrayList::of(&[1, 2, 3]);
        assert_eq!(l.len(), 3);
    }

    #[test]
    fn test_set() {
        let mut l = I64ArrayList::of(&[1, 2]);
        let old = l.set(0, 3);
        assert_eq!(old, 1);
    ...
    fn test_remove_at_index() {
        let mut l = I64ArrayList::of(&[1, 2, 3]);
    ...
    fn test_select_reject() {
        let l = I64ArrayList::of(&[1, 2, 3, 4, 5]);
    ...
    #[test]
    fn test_sort() {
        let mut l = I64ArrayList::of(&[3, 1, 2]);
        l.sort();
    ...
    #[test]
    fn test_equals() {
        let l1 = I64ArrayList::of(&[1, 2]);
        let l2 = I64ArrayList::of(&[1, 2]);
        let l3 = I64ArrayList::of(&[1]);
        assert_eq!(l1, l2);
        assert_ne!(l1, l3);
    }
    ...
    fn test_into_iter() {
        let l = I64ArrayList::of(&[1, 2, 3]);
        let sum: i64 = l.into_iter().map(|v| v).sum();
        assert!(sum > 0);
    }
...
impl crate::traits::i64_collection::I64Collection for I64ArrayList {
    fn len(&self) -> usize {
        self.len()
    }
...
impl crate::traits::i64_collection::I64MutableCollection for I64ArrayList {
    fn clear(&mut self) {
        self.clear()
    }
}

impl crate::traits::i64_collection::I64List for I64ArrayList {
...
impl crate::traits::i64_collection::I64MutableList for I64ArrayList {
