...
impl I8Interval {
    ...
    pub fn from_to_by(from: i8, to: i8, step: i8) -> Self {
        assert!(step != 0, "I8Interval: step must not be zero");
        if from < to {
            assert!(step > 0, "I8Interval: step must be positive when from < to");
        } else if from > to {
            assert!(step < 0, "I8Interval: step must be negative when from > to");
        }
        I8Interval { from, to, step }
    }

    /// Creates an interval from `from` to `to` (inclusive) with step 1 or -1.
    pub fn from_to(from: i8, to: i8) -> Self {
        let step: i8 = if from <= to { 1 } else { -1 };
        I8Interval { from, to, step }
    }

    /// Creates an interval from 1 to `to` (inclusive).
    pub fn one_to(to: i8) -> Self {
        Self::from_to(1, to)
    }

    /// Creates an interval from 0 to `to - 1` (inclusive).
    pub fn zero_to(to: i8) -> Self {
        Self::from_to(0, to)
    }
    ...
    pub fn get(&self, index: usize) -> Option<i8> {
        if index >= self.len() {
            ...
            Some(self.from + self.step * index as i8)
        }
    }

    /// Returns an iterator over the elements.
    pub fn iter(&self) -> I8IntervalIter {
        I8IntervalIter {
            current: self.from,
            to: self.to,
            step: self.step,
...
/// Iterator for `I8Interval`.
pub struct I8IntervalIter {
    current: i8,
    to: i8,
    step: i8,
    done: bool,
...
impl Iterator for I8IntervalIter {
    type Item = i8;
...
impl ExactSizeIterator for I8IntervalIter {}

impl fmt::Display for I8Interval {
...
mod tests {
    ...
    #[test]
    fn test_from_to_by() {
        let iv = I8Interval::from_to_by(0, 10, 2);
        assert_eq!(iv.len(), 6);
    ...
    fn test_from_to_by_negative_step() {
        let iv = I8Interval::from_to_by(10, 0, -3);
        assert_eq!(iv.to_vec(), vec![10, 7, 4, 1]);
    }

    #[test]
    fn test_single_element() {
        let iv = I8Interval::from_to(3, 3);
        assert_eq!(iv.len(), 1);
    ...
    #[test]
    fn test_one_to() {
        let iv = I8Interval::one_to(3);
        assert_eq!(iv.to_vec(), vec![1, 2, 3]);
    }

    #[test]
    fn test_zero_to() {
        let iv = I8Interval::zero_to(3);
...
impl crate::traits::i8_collection::I8Collection for I8Interval {
    fn len(&self) -> usize {
        self.len()
    }
