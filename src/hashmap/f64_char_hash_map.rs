...
/// Hash map from `f64` keys to `char` values.
/// Open-addressing with linear probing and Robin Hood backward-shift deletion.
#[derive(Debug, Clone)]
pub struct F64CharHashMap {
    inner: OpenHashMap<f64, char>,
}
...
impl F64CharHashMap {
    ...
    pub fn detect(&self, predicate: impl Fn(f64, char) -> bool) -> Option<(f64, char)> {
    ...
    pub fn none_satisfy(&self, predicate: impl Fn(f64, char) -> bool) -> bool {
        !self.iter().any(|(k, v)| predicate(k, v))
    }

    pub fn count(&self, predicate: impl Fn(f64, char) -> bool) -> usize {
        self.iter().filter(|&(k, v)| predicate(k, v)).count()
    }

    pub fn inject_into<R>(&self, initial: R, mut f: impl FnMut(R, f64, char) -> R) -> R {
        let mut acc = initial;
        for (k, v) in self.iter() {
            acc = f(acc, k, v);
        }
...
impl Default for F64CharHashMap {
    fn default() -> Self {
        Self::new()
    }
}

impl PartialEq for F64CharHashMap {
...
impl fmt::Display for F64CharHashMap {
...
mod tests {
    ...
    fn test_get_or_default() {
        let mut m = F64CharHashMap::new();
        m.insert(1.0f64, 'a');
        assert_eq!(m.get_or_default(1.0f64, 'c'), 'a');
        assert_eq!(m.get_or_default(99.0f64, 'c'), 'c');
    }
    ...
    #[test]
    fn test_equals() {
        let mut m1 = F64CharHashMap::new();
        m1.insert(1.0f64, 'a');
        m1.insert(2.0f64, 'b');
        let mut m2 = F64CharHashMap::new();
        m2.insert(2.0f64, 'b');
        m2.insert(1.0f64, 'a');
        assert_eq!(m1, m2);
    }
    ...
    fn test_resize() {
        let mut m = F64CharHashMap::new();
        for i in 0..100 {
            m.insert(i as f64, char::from(b'A' + (i % 26) as u8));
        }
        assert!(m.len() > 0);
    }
    ...
    fn test_nan_key_findable() {
        ...
        assert_eq!(m.get(f64::NAN), Some('a'));
    }
    ...
    fn test_nan_key_replaces_does_not_duplicate() {
        ...
        m.insert(f64::NAN, 'c');
        assert_eq!(m.len(), 1);
        assert_eq!(m.get(f64::NAN), Some('c'));
    }
    ...
    fn test_negative_zero_distinct_from_positive_zero() {
        ...
        m.insert(-0.0f64, 'b');
        assert_eq!(m.len(), 2);
        assert_eq!(m.get(0.0f64), Some('a'));
        assert_eq!(m.get(-0.0f64), Some('b'));
    }

    #[test]
    fn test_infinity_keys() {
        let mut m = F64CharHashMap::new();
        m.insert(f64::INFINITY, 'a');
        m.insert(f64::NEG_INFINITY, 'b');
        assert_eq!(m.len(), 2);
        assert_eq!(m.get(f64::INFINITY), Some('a'));
        assert_eq!(m.get(f64::NEG_INFINITY), Some('b'));
        assert!(m.contains_key(f64::INFINITY));
        assert!(m.contains_key(f64::NEG_INFINITY));
    }
}

impl crate::traits::f64_char_map::F64CharMap for F64CharHashMap {
    fn len(&self) -> usize {
        self.len()
    }
...
impl crate::traits::f64_char_map::F64CharMutableMap for F64CharHashMap {
