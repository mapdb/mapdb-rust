...
#[derive(Debug, Clone)]
pub struct I16F64ListMultimap {
    inner: HashMap<i16, Vec<f64>>,
    total_size: usize,
}

impl I16F64ListMultimap {
    pub fn new() -> Self {
        I16F64ListMultimap {
            inner: HashMap::new(),
            total_size: 0,
    ...
    pub fn put(&mut self, key: i16, value: f64) {
    ...
    pub fn get_all(&self, key: i16) -> Vec<f64> {
        self.get(key).to_vec()
    }

    /// Removes all values for the given key. Returns the removed values.
    pub fn remove_all(&mut self, key: i16) -> Vec<f64> {
        let internal_key = key;
    ...
    pub fn contains_key_value(&self, key: i16, value: f64) -> bool {
        self.get(key)
            .iter()
    ...
    pub fn for_each_key_value(&self, mut f: impl FnMut(i16, &[f64])) {
        for (k, vals) in &self.inner {
            f(*k, vals);
        }
    }
    ...
    pub fn to_vec(&self) -> Vec<(i16, f64)> {
...
impl Default for I16F64ListMultimap {
    fn default() -> Self {
        Self::new()
    }
}

impl PartialEq for I16F64ListMultimap {
...
impl fmt::Display for I16F64ListMultimap {
...
mod tests {
    ...
    #[test]
    fn test_put_get() {
        let mut m = I16F64ListMultimap::new();
        m.put(1, 1.0f64);
        m.put(1, 2.0f64);
        m.put(2, 3.0f64);
    ...
    fn test_contains_key_value() {
        ...
        m.put(1, 1.0f64);
        m.put(1, 2.0f64);
        assert!(m.contains_key_value(1, 1.0f64));
        assert!(m.contains_key_value(1, 2.0f64));
        assert!(!m.contains_key_value(1, 3.0f64));
    }
    ...
    #[test]
    fn test_equals() {
        let mut m1 = I16F64ListMultimap::new();
        m1.put(1, 1.0f64);
        m1.put(1, 2.0f64);
        let mut m2 = I16F64ListMultimap::new();
        m2.put(1, 1.0f64);
        m2.put(1, 2.0f64);
        assert_eq!(m1, m2);
    }
    ...
    fn test_with_key_value_fluent() {
        let m = I16F64ListMultimap::new()
            .with_key_value(1, 1.0f64)
            .with_key_value(1, 2.0f64)
            .with_key_value(2, 3.0f64);
        assert_eq!(m.size(), 3);
        assert_eq!(m.keys_count(), 2);
    }
