...
#[derive(Debug, Clone)]
pub struct F32I32ListMultimap {
    inner: HashMap<u32, Vec<i32>>,
    total_size: usize,
}

impl F32I32ListMultimap {
    pub fn new() -> Self {
        F32I32ListMultimap {
            inner: HashMap::new(),
            total_size: 0,
    ...
    pub fn put(&mut self, key: f32, value: i32) {
    ...
    pub fn get_all(&self, key: f32) -> Vec<i32> {
        self.get(key).to_vec()
    }

    /// Removes all values for the given key. Returns the removed values.
    pub fn remove_all(&mut self, key: f32) -> Vec<i32> {
        let internal_key = f32::to_bits(key);
    ...
    pub fn contains_key_value(&self, key: f32, value: i32) -> bool {
    ...
    pub fn for_each_key_value(&self, mut f: impl FnMut(f32, &[i32])) {
        for (k, vals) in &self.inner {
    ...
    pub fn to_vec(&self) -> Vec<(f32, i32)> {
...
impl Default for F32I32ListMultimap {
    fn default() -> Self {
        Self::new()
    }
}

impl PartialEq for F32I32ListMultimap {
...
impl fmt::Display for F32I32ListMultimap {
...
mod tests {
    ...
    #[test]
    fn test_put_get() {
        let mut m = F32I32ListMultimap::new();
        m.put(1.0f32, 1);
        m.put(1.0f32, 2);
        m.put(2.0f32, 3);
    ...
    #[test]
    fn test_equals() {
        let mut m1 = F32I32ListMultimap::new();
        m1.put(1.0f32, 1);
        m1.put(1.0f32, 2);
        let mut m2 = F32I32ListMultimap::new();
        m2.put(1.0f32, 1);
        m2.put(1.0f32, 2);
        assert_eq!(m1, m2);
    }
