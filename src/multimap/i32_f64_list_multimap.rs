...
#[derive(Debug, Clone)]
pub struct I32F64ListMultimap {
    inner: HashMap<i32, Vec<f64>>,
    total_size: usize,
}

impl I32F64ListMultimap {
    pub fn new() -> Self {
        I32F64ListMultimap {
            inner: HashMap::new(),
            total_size: 0,
    ...
    pub fn put(&mut self, key: i32, value: f64) {
    ...
    pub fn get_all(&self, key: i32) -> Vec<f64> {
        self.get(key).to_vec()
    }

    /// Removes all values for the given key. Returns the removed values.
    pub fn remove_all(&mut self, key: i32) -> Vec<f64> {
        let internal_key = key;
    ...
    pub fn contains_key_value(&self, key: i32, value: f64) -> bool {
        self.get(key)
            .iter()
    ...
    pub fn for_each_key_value(&self, mut f: impl FnMut(i32, &[f64])) {
        for (k, vals) in &self.inner {
            f(*k, vals);
        }
    }
    ...
    pub fn to_vec(&self) -> Vec<(i32, f64)> {
...
impl Default for I32F64ListMultimap {
    fn default() -> Self {
        Self::new()
    }
}

impl PartialEq for I32F64ListMultimap {
...
impl fmt::Display for I32F64ListMultimap {
...
mod tests {
    ...
    #[test]
    fn test_put_get() {
        let mut m = I32F64ListMultimap::new();
        m.put(1, 1.0f64);
        m.put(1, 2.0f64);
        m.put(2, 3.0f64);
    ...
    #[test]
    fn test_equals() {
        let mut m1 = I32F64ListMultimap::new();
        m1.put(1, 1.0f64);
        m1.put(1, 2.0f64);
        let mut m2 = I32F64ListMultimap::new();
        m2.put(1, 1.0f64);
        m2.put(1, 2.0f64);
        assert_eq!(m1, m2);
    }
