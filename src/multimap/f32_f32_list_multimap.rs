...
#[derive(Debug, Clone)]
pub struct F32F32ListMultimap {
    inner: HashMap<u32, Vec<f32>>,
    total_size: usize,
}

impl F32F32ListMultimap {
    pub fn new() -> Self {
        F32F32ListMultimap {
            inner: HashMap::new(),
            total_size: 0,
    ...
    pub fn put(&mut self, key: f32, value: f32) {
    ...
    pub fn get_all(&self, key: f32) -> Vec<f32> {
        self.get(key).to_vec()
    }

    /// Removes all values for the given key. Returns the removed values.
    pub fn remove_all(&mut self, key: f32) -> Vec<f32> {
        let internal_key = f32::to_bits(key);
    ...
    pub fn contains_key_value(&self, key: f32, value: f32) -> bool {
        self.get(key)
            .iter()
    ...
    pub fn for_each_key_value(&self, mut f: impl FnMut(f32, &[f32])) {
        for (k, vals) in &self.inner {
...
impl Default for F32F32ListMultimap {
    fn default() -> Self {
        Self::new()
    }
}

impl PartialEq for F32F32ListMultimap {
...
impl fmt::Display for F32F32ListMultimap {
...
mod tests {
    ...
    #[test]
    fn test_put_get() {
        let mut m = F32F32ListMultimap::new();
        m.put(1.0f32, 1.0f32);
        m.put(1.0f32, 2.0f32);
        m.put(2.0f32, 3.0f32);
    ...
    fn test_contains_key_value() {
        ...
        m.put(1.0f32, 1.0f32);
        m.put(1.0f32, 2.0f32);
        assert!(m.contains_key_value(1.0f32, 1.0f32));
        assert!(m.contains_key_value(1.0f32, 2.0f32));
        assert!(!m.contains_key_value(1.0f32, 3.0f32));
    }
    ...
    #[test]
    fn test_equals() {
        let mut m1 = F32F32ListMultimap::new();
        m1.put(1.0f32, 1.0f32);
        m1.put(1.0f32, 2.0f32);
        let mut m2 = F32F32ListMultimap::new();
        m2.put(1.0f32, 1.0f32);
        m2.put(1.0f32, 2.0f32);
        assert_eq!(m1, m2);
    }
    ...
    fn test_with_key_value_fluent() {
        let m = F32F32ListMultimap::new()
            .with_key_value(1.0f32, 1.0f32)
            .with_key_value(1.0f32, 2.0f32)
            .with_key_value(2.0f32, 3.0f32);
        assert_eq!(m.size(), 3);
        assert_eq!(m.keys_count(), 2);
    }
