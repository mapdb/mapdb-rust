...
use crate::hashmap::f64_char_hash_bi_map::F64CharHashBiMap;
use std::fmt;

/// Bidirectional hash map from `char` keys to `f64` values.
...
#[derive(Debug, Clone)]
pub struct CharF64HashBiMap {
    forward: OpenHashMap<char, f64>,
    reverse: OpenHashMap<f64, char>,
}

impl CharF64HashBiMap {
    pub fn new() -> Self {
        CharF64HashBiMap {
    ...
    pub fn get_key(&self, value: f64) -> Option<char> {
    ...
    pub fn remove_value(&mut self, value: f64) -> Option<char> {
    ...
    pub fn inverse(&self) -> F64CharHashBiMap {
        F64CharHashBiMap::from_maps(self.reverse.clone(), self.forward.clone())
    }
    ...
    pub(crate) fn from_maps(
        ...
        CharF64HashBiMap { forward, reverse }
    }
}

impl Default for CharF64HashBiMap {
    fn default() -> Self {
        Self::new()
    }
}

impl PartialEq for CharF64HashBiMap {
...
impl fmt::Display for CharF64HashBiMap {
...
mod tests {
    ...
    #[test]
    fn test_insert_get() {
        let mut m = CharF64HashBiMap::new();
        m.insert('a', 1.0f64);
        m.insert('b', 2.0f64);
        assert_eq!(m.get('a'), Some(1.0f64));
        assert_eq!(m.get('b'), Some(2.0f64));
        assert_eq!(m.len(), 2);
    }
    ...
    fn test_get_key() {
        ...
        m.insert('a', 1.0f64);
        m.insert('b', 2.0f64);
        assert_eq!(m.get_key(1.0f64), Some('a'));
        assert_eq!(m.get_key(2.0f64), Some('b'));
    ...
    fn test_insert_overwrite() {
        ...
        assert_eq!(m.get_key(2.0f64), Some('a'));
    ...
    fn test_remove_value() {
        ...
        m.insert('a', 1.0f64);
        m.insert('b', 2.0f64);
        assert_eq!(m.remove_value(1.0f64), Some('a'));
        assert_eq!(m.len(), 1);
        assert!(!m.contains_key('a'));
        assert!(!m.contains_value(1.0f64));
    }
    ...
    fn test_inverse() {
        ...
        assert_eq!(inv.get(1.0f64), Some('a'));
        assert_eq!(inv.get(2.0f64), Some('b'));
        // inverse reverse lookup: old key -> old value
        assert_eq!(inv.get_key('a'), Some(1.0f64));
        assert_eq!(inv.len(), 2);
    }
...
impl crate::traits::char_f64_map::CharF64Map for CharF64HashBiMap {
    fn len(&self) -> usize {
        self.len()
    }
...
impl crate::traits::char_f64_map::CharF64MutableMap for CharF64HashBiMap {
