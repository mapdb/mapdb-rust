...
#[derive(Debug, Clone)]
pub struct BoolF32ListMultimap {
    inner: HashMap<bool, Vec<f32>>,
    total_size: usize,
}

impl BoolF32ListMultimap {
    pub fn new() -> Self {
        BoolF32ListMultimap {
            inner: HashMap::new(),
            total_size: 0,
    ...
    pub fn put(&mut self, key: bool, value: f32) {
    ...
    pub fn get_all(&self, key: bool) -> Vec<f32> {
        self.get(key).to_vec()
    }

    /// Removes all values for the given key. Returns the removed values.
    pub fn remove_all(&mut self, key: bool) -> Vec<f32> {
        let internal_key = key;
    ...
    pub fn contains_key_value(&self, key: bool, value: f32) -> bool {
        self.get(key)
            .iter()
    ...
    pub fn for_each_key_value(&self, mut f: impl FnMut(bool, &[f32])) {
        for (k, vals) in &self.inner {
            f(*k, vals);
        }
    }
    ...
    pub fn to_vec(&self) -> Vec<(bool, f32)> {
...
impl Default for BoolF32ListMultimap {
    fn default() -> Self {
        Self::new()
    }
}

impl PartialEq for BoolF32ListMultimap {
...
impl fmt::Display for BoolF32ListMultimap {
...
mod tests {
    ...
    #[test]
    fn test_put_get() {
        let mut m = BoolF32ListMultimap::new();
        m.put(true, 1.0f32);
        m.put(true, 2.0f32);
        m.put(false, 3.0f32);
    ...
    fn test_contains_key_value() {
        ...
        m.put(true, 1.0f32);
        m.put(true, 2.0f32);
        assert!(m.contains_key_value(true, 1.0f32));
        assert!(m.contains_key_value(true, 2.0f32));
        assert!(!m.contains_key_value(true, 3.0f32));
    }
    ...
    #[test]
    fn test_equals() {
        let mut m1 = BoolF32ListMultimap::new();
        m1.put(true, 1.0f32);
        m1.put(true, 2.0f32);
        let mut m2 = BoolF32ListMultimap::new();
        m2.put(true, 1.0f32);
        m2.put(true, 2.0f32);
        assert_eq!(m1, m2);
    }
    ...
    fn test_with_key_value_fluent() {
        let m = BoolF32ListMultimap::new()
            .with_key_value(true, 1.0f32)
            .with_key_value(true, 2.0f32)
            .with_key_value(false, 3.0f32);
        assert_eq!(m.size(), 3);
        assert_eq!(m.keys_count(), 2);
    }
