...
use crate::hashmap::bool_f64_hash_bi_map::BoolF64HashBiMap;
use std::fmt;
...
impl F64BoolHashBiMap {
    ...
    pub fn get_key(&self, value: bool) -> Option<f64> {
    ...
    pub fn remove_value(&mut self, value: bool) -> Option<f64> {
    ...
    pub fn inverse(&self) -> BoolF64HashBiMap {
        BoolF64HashBiMap::from_maps(self.reverse.clone(), self.forward.clone())
    }
    ...
    pub(crate) fn from_maps(
        ...
        F64BoolHashBiMap { forward, reverse }
    }
}

impl Default for F64BoolHashBiMap {
    fn default() -> Self {
        Self::new()
    }
}

impl PartialEq for F64BoolHashBiMap {
...
impl fmt::Display for F64BoolHashBiMap {
...
mod tests {
    ...
    fn test_get_key() {
        ...
        m.insert(1.0f64, true);
        m.insert(2.0f64, false);
        assert_eq!(m.get_key(true), Some(1.0f64));
        assert_eq!(m.get_key(false), Some(2.0f64));
    }
    ...
    fn test_insert_overwrite() {
        ...
        assert_eq!(m.get_key(false), Some(1.0f64));
    ...
    fn test_remove_value() {
        ...
        m.insert(1.0f64, true);
        m.insert(2.0f64, false);
        assert_eq!(m.remove_value(true), Some(1.0f64));
        assert_eq!(m.len(), 1);
        assert!(!m.contains_key(1.0f64));
        assert!(!m.contains_value(true));
    }
    ...
    fn test_inverse() {
        ...
        assert_eq!(inv.get(true), Some(1.0f64));
        assert_eq!(inv.get(false), Some(2.0f64));
        // inverse reverse lookup: old key -> old value
        assert_eq!(inv.get_key(1.0f64), Some(true));
        assert_eq!(inv.len(), 2);
    }
...
impl crate::traits::f64_bool_map::F64BoolMap for F64BoolHashBiMap {
    fn len(&self) -> usize {
        self.len()
    }
...
impl crate::traits::f64_bool_map::F64BoolMutableMap for F64BoolHashBiMap {
