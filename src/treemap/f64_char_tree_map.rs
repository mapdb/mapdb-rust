...
/// Sorted map from `f64` keys to `char` values.
#[derive(Debug, Clone)]
pub struct F64CharTreeMap {
    entries: Vec<(f64, char)>,
}
...
impl F64CharTreeMap {
    ...
    pub fn min(&self) -> Option<(f64, char)> {
        self.entries.first().copied()
    }

    pub fn max(&self) -> Option<(f64, char)> {
...
impl Default for F64CharTreeMap {
    fn default() -> Self {
        Self::new()
    }
}

impl PartialEq for F64CharTreeMap {
...
impl fmt::Display for F64CharTreeMap {
...
impl crate::traits::f64_char_map::F64CharMap for F64CharTreeMap {
    fn len(&self) -> usize {
        self.len()
    }
...
impl crate::traits::f64_char_map::F64CharMutableMap for F64CharTreeMap {
