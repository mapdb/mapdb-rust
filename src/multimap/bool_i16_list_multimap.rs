...
#[derive(Debug, Clone)]
pub struct BoolI16ListMultimap {
    inner: HashMap<bool, Vec<i16>>,
    total_size: usize,
}

impl BoolI16ListMultimap {
    pub fn new() -> Self {
        BoolI16ListMultimap {
            inner: HashMap::new(),
            total_size: 0,
    ...
    pub fn put(&mut self, key: bool, value: i16) {
    ...
    pub fn get_all(&self, key: bool) -> Vec<i16> {
        self.get(key).to_vec()
    }

    /// Removes all values for the given key. Returns the removed values.
    pub fn remove_all(&mut self, key: bool) -> Vec<i16> {
        let internal_key = key;
    ...
    pub fn contains_key_value(&self, key: bool, value: i16) -> bool {
    ...
    pub fn for_each_key_value(&self, mut f: impl FnMut(bool, &[i16])) {
        for (k, vals) in &self.inner {
            f(*k, vals);
        }
    }
    ...
    pub fn reject(&self, predicate: impl Fn(bool, i16) -> bool) -> Self {
        let mut result = Self::new();
        for (k, vals) in &self.inner {
            let key = *k;
            for &val in vals {
    ...
    pub fn to_vec(&self) -> Vec<(bool, i16)> {
    ...
    pub fn with_key_value(mut self, key: bool, value: i16) -> Self {
        self.put(key, value);
        self
    }
}

impl Default for BoolI16ListMultimap {
    fn default() -> Self {
        Self::new()
    }
}

impl PartialEq for BoolI16ListMultimap {
...
impl fmt::Display for BoolI16ListMultimap {
...
mod tests {
    ...
    #[test]
    fn test_put_get() {
        let mut m = BoolI16ListMultimap::new();
        m.put(true, 1);
        m.put(true, 2);
        m.put(false, 3);
    ...
    fn test_contains_key_value() {
        ...
        m.put(true, 1);
        m.put(true, 2);
        assert!(m.contains_key_value(true, 1));
        assert!(m.contains_key_value(true, 2));
        assert!(!m.contains_key_value(true, 3));
    }
    ...
    #[test]
    fn test_equals() {
        let mut m1 = BoolI16ListMultimap::new();
        m1.put(true, 1);
        m1.put(true, 2);
        let mut m2 = BoolI16ListMultimap::new();
        m2.put(true, 1);
        m2.put(true, 2);
        assert_eq!(m1, m2);
    }
    ...
    fn test_with_key_value_fluent() {
        let m = BoolI16ListMultimap::new()
            .with_key_value(true, 1)
            .with_key_value(true, 2)
            .with_key_value(false, 3);
        assert_eq!(m.size(), 3);
        assert_eq!(m.keys_count(), 2);
    }
