...
impl I16I32HashMap {
    ...
    pub fn insert(&mut self, key: i16, value: i32) -> Option<i32> {
        self.inner.insert(key, value)
    }

    /// Returns the value for the key, or None.
    pub fn get(&self, key: i16) -> Option<i32> {
        self.inner.get(key)
    }

    /// Returns the value for the key, or the default.
    pub fn get_or_default(&self, key: i16, default: i32) -> i32 {
        self.inner.get(key).unwrap_or(default)
    }

    /// Removes the key. Returns the old value if present.
    pub fn remove(&mut self, key: i16) -> Option<i32> {
    ...
    pub fn iter(&self) -> impl Iterator<Item = (i16, i32)> + '_ {
        self.inner.iter()
    }
    ...
    pub fn for_each(&self, mut f: impl FnMut(i16, i32)) {
        for (k, v) in self.iter() {
            f(k, v);
        }
    }

    pub fn select(&self, predicate: impl Fn(i16, i32) -> bool) -> Self {
    ...
    pub fn reject(&self, predicate: impl Fn(i16, i32) -> bool) -> Self {
        let mut result = Self::new();
        for (k, v) in self.iter() {
            if !predicate(k, v) {
                result.insert(k, v);
    ...
    pub fn detect(&self, predicate: impl Fn(i16, i32) -> bool) -> Option<(i16, i32)> {
        self.iter().find(|&(k, v)| predicate(k, v))
    }

    pub fn any_satisfy(&self, predicate: impl Fn(i16, i32) -> bool) -> bool {
        self.iter().any(|(k, v)| predicate(k, v))
    }

    pub fn all_satisfy(&self, predicate: impl Fn(i16, i32) -> bool) -> bool {
        self.iter().all(|(k, v)| predicate(k, v))
    }

    pub fn none_satisfy(&self, predicate: impl Fn(i16, i32) -> bool) -> bool {
        !self.iter().any(|(k, v)| predicate(k, v))
    }

    pub fn count(&self, predicate: impl Fn(i16, i32) -> bool) -> usize {
        self.iter().filter(|&(k, v)| predicate(k, v)).count()
    }

    pub fn inject_into<R>(&self, initial: R, mut f: impl FnMut(R, i16, i32) -> R) -> R {
        let mut acc = initial;
        for (k, v) in self.iter() {
            acc = f(acc, k, v);
        }
    ...
    pub fn with_key_value(mut self, key: i16, value: i32) -> Self {
...
impl Default for I16I32HashMap {
    fn default() -> Self {
        Self::new()
    }
}

impl PartialEq for I16I32HashMap {
...
impl fmt::Display for I16I32HashMap {
...
mod tests {
    ...
    #[test]
    fn test_equals() {
        let mut m1 = I16I32HashMap::new();
        m1.insert(1, 1);
        m1.insert(2, 2);
        let mut m2 = I16I32HashMap::new();
        m2.insert(2, 2);
        m2.insert(1, 1);
        assert_eq!(m1, m2);
    }
...
impl crate::traits::i16_i32_map::I16I32Map for I16I32HashMap {
    fn len(&self) -> usize {
        self.len()
    }
    ...
    fn get(&self, key: i16) -> Option<i32> {
        self.get(key)
    }
    fn iter(&self) -> impl Iterator<Item = (i16, i32)> + '_ {
        self.iter()
    }
}

impl crate::traits::i16_i32_map::I16I32MutableMap for I16I32HashMap {
    fn insert(&mut self, key: i16, value: i32) -> Option<i32> {
        self.insert(key, value)
    }
    fn remove(&mut self, key: i16) -> Option<i32> {
        self.remove(key)
    }
    fn clear(&mut self) {
        self.clear()
