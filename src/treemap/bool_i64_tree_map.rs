...
/// Sorted map from `bool` keys to `i64` values.
#[derive(Debug, Clone)]
pub struct BoolI64TreeMap {
    inner: BTreeMap<bool, i64>,
}
...
impl BoolI64TreeMap {
    ...
    pub fn min(&self) -> Option<(bool, i64)> {
        self.inner.iter().next().map(|(&k, &v)| (k, v))
    }

    pub fn max(&self) -> Option<(bool, i64)> {
...
impl Default for BoolI64TreeMap {
    fn default() -> Self {
        Self::new()
    }
}

impl PartialEq for BoolI64TreeMap {
...
impl fmt::Display for BoolI64TreeMap {
...
impl crate::traits::bool_i64_map::BoolI64Map for BoolI64TreeMap {
    fn len(&self) -> usize {
        self.len()
    }
...
impl crate::traits::bool_i64_map::BoolI64MutableMap for BoolI64TreeMap {
