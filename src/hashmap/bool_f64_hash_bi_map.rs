...
use crate::hashmap::f64_bool_hash_bi_map::F64BoolHashBiMap;
use std::fmt;
...
impl BoolF64HashBiMap {
    ...
    pub fn get_key(&self, value: f64) -> Option<bool> {
    ...
    pub fn remove_value(&mut self, value: f64) -> Option<bool> {
    ...
    pub fn inverse(&self) -> F64BoolHashBiMap {
        F64BoolHashBiMap::from_maps(self.reverse.clone(), self.forward.clone())
    }
    ...
    pub(crate) fn from_maps(
        ...
        BoolF64HashBiMap { forward, reverse }
    }
}

impl Default for BoolF64HashBiMap {
    fn default() -> Self {
        Self::new()
    }
}

impl PartialEq for BoolF64HashBiMap {
...
impl fmt::Display for BoolF64HashBiMap {
...
mod tests {
    ...
    fn test_get_key() {
        ...
        m.insert(true, 1.0f64);
        m.insert(false, 2.0f64);
        assert_eq!(m.get_key(1.0f64), Some(true));
        assert_eq!(m.get_key(2.0f64), Some(false));
        assert_eq!(m.get_key(99.0f64), None);
    }
    ...
    fn test_insert_overwrite() {
        ...
        assert_eq!(m.get_key(2.0f64), Some(true));
        // Old reverse mapping should be gone
        assert_eq!(m.get_key(1.0f64), None);
    }
    ...
    fn test_remove() {
        ...
        assert!(!m.contains_value(1.0f64));
    }
    ...
    fn test_remove_value() {
        ...
        m.insert(true, 1.0f64);
        m.insert(false, 2.0f64);
        assert_eq!(m.remove_value(1.0f64), Some(true));
        assert_eq!(m.len(), 1);
        assert!(!m.contains_key(true));
        assert!(!m.contains_value(1.0f64));
    }
    ...
    fn test_contains() {
        ...
        assert!(m.contains_value(1.0f64));
        assert!(!m.contains_value(99.0f64));
    }
    ...
    fn test_inverse() {
        ...
        assert_eq!(inv.get(1.0f64), Some(true));
        assert_eq!(inv.get(2.0f64), Some(false));
        // inverse reverse lookup: old key -> old value
        assert_eq!(inv.get_key(true), Some(1.0f64));
        assert_eq!(inv.len(), 2);
    }
...
impl crate::traits::bool_f64_map::BoolF64Map for BoolF64HashBiMap {
    fn len(&self) -> usize {
        self.len()
    }
...
impl crate::traits::bool_f64_map::BoolF64MutableMap for BoolF64HashBiMap {
