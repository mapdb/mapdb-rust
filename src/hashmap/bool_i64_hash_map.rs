...
impl BoolI64HashMap {
    ...
    pub fn insert(&mut self, key: bool, value: i64) -> Option<i64> {
    ...
    pub fn remove(&mut self, key: bool) -> Option<i64> {
    ...
    pub fn detect(&self, predicate: impl Fn(bool, i64) -> bool) -> Option<(bool, i64)> {
    ...
    pub fn none_satisfy(&self, predicate: impl Fn(bool, i64) -> bool) -> bool {
        !self.iter().any(|(k, v)| predicate(k, v))
    }

    pub fn count(&self, predicate: impl Fn(bool, i64) -> bool) -> usize {
        self.iter().filter(|&(k, v)| predicate(k, v)).count()
    }

    pub fn inject_into<R>(&self, initial: R, mut f: impl FnMut(R, bool, i64) -> R) -> R {
        let mut acc = initial;
        for (k, v) in self.iter() {
            acc = f(acc, k, v);
        }
...
impl Default for BoolI64HashMap {
    fn default() -> Self {
        Self::new()
    }
}

impl PartialEq for BoolI64HashMap {
...
impl fmt::Display for BoolI64HashMap {
...
mod tests {
    ...
    #[test]
    fn test_equals() {
        let mut m1 = BoolI64HashMap::new();
        m1.insert(true, 1);
        m1.insert(false, 2);
        let mut m2 = BoolI64HashMap::new();
        m2.insert(false, 2);
        m2.insert(true, 1);
        assert_eq!(m1, m2);
    }
...
impl crate::traits::bool_i64_map::BoolI64Map for BoolI64HashMap {
    fn len(&self) -> usize {
        self.len()
    }
...
impl crate::traits::bool_i64_map::BoolI64MutableMap for BoolI64HashMap {
