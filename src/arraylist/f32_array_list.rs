...
impl F32ArrayList {
    ...
    pub fn set(&mut self, index: usize, value: f32) -> f32 {
    ...
    /// # Panics
    /// Panics if index is out of bounds.
    pub fn remove_at_index(&mut self, index: usize) -> f32 {
    ...
    pub fn count(&self, predicate: impl Fn(f32) -> bool) -> usize {
    ...
    pub fn inject_into<R>(&self, initial: R, mut f: impl FnMut(R, f32) -> R) -> R {
        let mut acc = initial;
        for &v in &self.items {
            acc = f(acc, v);
        }
...
impl Default for F32ArrayList {
    fn default() -> Self {
        Self::new()
    }
}

impl PartialEq for F32ArrayList {
...
impl fmt::Display for F32ArrayList {
...
impl IntoIterator for F32ArrayList {
    type Item = f32;
...
impl<'a> IntoIterator for &'a F32ArrayList {
    type Item = &'a f32;
...
impl std::ops::Index<usize> for F32ArrayList {
    type Output = f32;
...
mod tests {
    ...
    fn test_push_and_get() {
        let mut l = F32ArrayList::new();
        l.push(1.0f32);
        l.push(2.0f32);
        l.push(3.0f32);
        assert_eq!(l.len(), 3);
        assert_eq!(l.get(0), Some(1.0f32));
        assert_eq!(l.get(2), Some(3.0f32));
        assert_eq!(l.get(99), None);
    }

    #[test]
    fn test_of() {
        let l = F32ArrayList::of(&[1.0f32, 2.0f32, 3.0f32]);
        assert_eq!(l.len(), 3);
    }

    #[test]
    fn test_set() {
        let mut l = F32ArrayList::of(&[1.0f32, 2.0f32]);
        let old = l.set(0, 3.0f32);
        assert_eq!(old, 1.0f32);
        assert_eq!(l.get(0), Some(3.0f32));
    }

    #[test]
    fn test_remove_at_index() {
        let mut l = F32ArrayList::of(&[1.0f32, 2.0f32, 3.0f32]);
        let removed = l.remove_at_index(1);
        assert_eq!(removed, 2.0f32);
        assert_eq!(l.len(), 2);
    }
    ...
    fn test_remove() {
        let mut l = F32ArrayList::of(&[1.0f32, 2.0f32, 3.0f32]);
        assert!(l.remove(2.0f32));
        assert!(!l.contains(2.0f32));
        assert!(!l.remove(99.0f32));
    }

    #[test]
    fn test_contains() {
        let l = F32ArrayList::of(&[1.0f32, 2.0f32]);
        assert!(l.contains(1.0f32));
        assert!(!l.contains(99.0f32));
    }
    ...
    fn test_select_reject() {
        let l = F32ArrayList::of(&[1.0f32, 2.0f32, 3.0f32, 4.0f32, 5.0f32]);
        let sel = l.select(|v| v > 3.0f32);
        assert_eq!(sel.len(), 2);
        let rej = l.reject(|v| v > 3.0f32);
        assert_eq!(rej.len(), 3);
    }
    ...
    fn test_detect() {
        let l = F32ArrayList::of(&[1.0f32, 2.0f32]);
        assert_eq!(l.detect(|v| v == 2.0f32), Some(2.0f32));
        assert_eq!(l.detect(|v| v == 99.0f32), None);
    }

    #[test]
    fn test_any_all_none_satisfy() {
        let l = F32ArrayList::of(&[1.0f32, 2.0f32, 3.0f32]);
        assert!(l.any_satisfy(|v| v == 2.0f32));
        assert!(!l.any_satisfy(|v| v == 99.0f32));
        assert!(l.all_satisfy(|v| v > 0.0));
        assert!(l.none_satisfy(|v| v == 99.0f32));
    }

    #[test]
    fn test_sum_min_max() {
        let l = F32ArrayList::of(&[3.0f32, 1.0f32, 2.0f32]);
        assert_eq!(l.min(), Some(1.0f32));
        assert_eq!(l.max(), Some(3.0f32));
    }

    #[test]
    fn test_sort() {
        let mut l = F32ArrayList::of(&[3.0f32, 1.0f32, 2.0f32]);
        l.sort();
        assert_eq!(l.to_vec(), vec![1.0f32, 2.0f32, 3.0f32]);
    }
    ...
    fn test_reversed() {
        let l = F32ArrayList::of(&[1.0f32, 2.0f32, 3.0f32]);
        let r = l.reversed();
        assert_eq!(r.get(0), Some(3.0f32));
        assert_eq!(r.get(2), Some(1.0f32));
    }

    #[test]
    fn test_equals() {
        let l1 = F32ArrayList::of(&[1.0f32, 2.0f32]);
        let l2 = F32ArrayList::of(&[1.0f32, 2.0f32]);
        let l3 = F32ArrayList::of(&[1.0f32]);
        assert_eq!(l1, l2);
        assert_ne!(l1, l3);
    }
    ...
    fn test_index() {
        let l = F32ArrayList::of(&[1.0f32, 2.0f32]);
        assert_eq!(l[0], 1.0f32);
        assert_eq!(l[1], 2.0f32);
    }

    #[test]
    fn test_into_iter() {
        let l = F32ArrayList::of(&[1.0f32, 2.0f32, 3.0f32]);
        let sum: f32 = l.into_iter().map(|v| v).sum();
        assert!(sum > 0.0);
    }
...
impl crate::traits::f32_collection::F32Collection for F32ArrayList {
    fn len(&self) -> usize {
        self.len()
    }
...
impl crate::traits::f32_collection::F32MutableCollection for F32ArrayList {
    fn clear(&mut self) {
        self.clear()
    }
}

impl crate::traits::f32_collection::F32List for F32ArrayList {
...
impl crate::traits::f32_collection::F32MutableList for F32ArrayList {
