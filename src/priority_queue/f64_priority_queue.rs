...
/// Primitive min-heap priority queue of `f64` values, backed by a `Vec`.
/// O(log n) push/pop, O(1) peek. Duplicates are allowed.
#[derive(Debug, Clone)]
pub struct F64PriorityQueue {
    items: Vec<f64>,
}

impl F64PriorityQueue {
    pub fn new() -> Self {
        F64PriorityQueue { items: Vec::new() }
    }
...
impl Default for F64PriorityQueue {
    fn default() -> Self {
        Self::new()
    }
}

impl fmt::Display for F64PriorityQueue {
...
mod tests {
    ...
    fn test_push_peek_pop_min() {
        let mut q = F64PriorityQueue::new();
        q.push(3.0f64);
        q.push(1.0f64);
        q.push(2.0f64);
        // v[0] should be the smallest
        assert_eq!(q.peek(), Some(1.0f64));
        assert_eq!(q.len(), 3);
    ...
    #[test]
    fn test_of_heapify() {
        let q = F64PriorityQueue::of(&[3.0f64, 1.0f64, 2.0f64]);
        assert_eq!(q.len(), 3);
        assert_eq!(q.peek(), Some(1.0f64));
    }
