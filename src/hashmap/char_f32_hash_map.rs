...
/// Hash map from `char` keys to `f32` values.
/// Open-addressing with linear probing and Robin Hood backward-shift deletion.
#[derive(Debug, Clone)]
pub struct CharF32HashMap {
    inner: OpenHashMap<char, f32>,
}
...
impl CharF32HashMap {
    ...
    pub fn reject(&self, predicate: impl Fn(char, f32) -> bool) -> Self {
        let mut result = Self::new();
        for (k, v) in self.iter() {
            if !predicate(k, v) {
                result.insert(k, v);
    ...
    pub fn detect(&self, predicate: impl Fn(char, f32) -> bool) -> Option<(char, f32)> {
    ...
    pub fn none_satisfy(&self, predicate: impl Fn(char, f32) -> bool) -> bool {
        !self.iter().any(|(k, v)| predicate(k, v))
    }

    pub fn count(&self, predicate: impl Fn(char, f32) -> bool) -> usize {
        self.iter().filter(|&(k, v)| predicate(k, v)).count()
    }

    pub fn inject_into<R>(&self, initial: R, mut f: impl FnMut(R, char, f32) -> R) -> R {
        let mut acc = initial;
        for (k, v) in self.iter() {
            acc = f(acc, k, v);
        }
    ...
    pub fn with_key_value(mut self, key: char, value: f32) -> Self {
...
impl Default for CharF32HashMap {
    fn default() -> Self {
        Self::new()
    }
}

impl PartialEq for CharF32HashMap {
...
impl fmt::Display for CharF32HashMap {
...
mod tests {
    ...
    fn test_get_or_default() {
        let mut m = CharF32HashMap::new();
        m.insert('a', 1.0f32);
        assert_eq!(m.get_or_default('a', 3.0f32), 1.0f32);
        assert_eq!(m.get_or_default('z', 3.0f32), 3.0f32);
    }
    ...
    #[test]
    fn test_equals() {
        let mut m1 = CharF32HashMap::new();
        m1.insert('a', 1.0f32);
        m1.insert('b', 2.0f32);
        let mut m2 = CharF32HashMap::new();
        m2.insert('b', 2.0f32);
        m2.insert('a', 1.0f32);
        assert_eq!(m1, m2);
    }
    ...
    fn test_resize() {
        let mut m = CharF32HashMap::new();
        for i in 0..100 {
            m.insert(char::from(b'a' + (i % 26) as u8), (i * 10) as f32);
        }
        assert!(m.len() > 0);
    }
...
impl crate::traits::char_f32_map::CharF32Map for CharF32HashMap {
    fn len(&self) -> usize {
        self.len()
    }
...
impl crate::traits::char_f32_map::CharF32MutableMap for CharF32HashMap {
