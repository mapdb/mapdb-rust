...
/// Each key maps to a `Vec<i8>` of values, preserving insertion order per key.
///
/// Uses `std::collections::HashMap` internally because values are `Vec<i8>` (not `Copy`),
...
#[derive(Debug, Clone)]
pub struct F32I8ListMultimap {
    inner: HashMap<u32, Vec<i8>>,
    total_size: usize,
}

impl F32I8ListMultimap {
    pub fn new() -> Self {
        F32I8ListMultimap {
            inner: HashMap::new(),
            total_size: 0,
    ...
    pub fn put(&mut self, key: f32, value: i8) {
    ...
    pub fn get_all(&self, key: f32) -> Vec<i8> {
        self.get(key).to_vec()
    }

    /// Removes all values for the given key. Returns the removed values.
    pub fn remove_all(&mut self, key: f32) -> Vec<i8> {
        let internal_key = f32::to_bits(key);
    ...
    pub fn contains_key_value(&self, key: f32, value: i8) -> bool {
    ...
    /// failure. Note that per-key value lists (`Vec<i8>`) grow
    ...
    pub fn for_each_key_value(&self, mut f: impl FnMut(f32, &[i8])) {
        for (k, vals) in &self.inner {
...
impl Default for F32I8ListMultimap {
    fn default() -> Self {
        Self::new()
    }
}

impl PartialEq for F32I8ListMultimap {
...
impl fmt::Display for F32I8ListMultimap {
...
mod tests {
    ...
    #[test]
    fn test_put_get() {
        let mut m = F32I8ListMultimap::new();
        m.put(1.0f32, 1);
        m.put(1.0f32, 2);
        m.put(2.0f32, 3);
    ...
    fn test_contains_key_value() {
        ...
        m.put(1.0f32, 1);
        m.put(1.0f32, 2);
        assert!(m.contains_key_value(1.0f32, 1));
        assert!(m.contains_key_value(1.0f32, 2));
        assert!(!m.contains_key_value(1.0f32, 3));
    }
    ...
    #[test]
    fn test_equals() {
        let mut m1 = F32I8ListMultimap::new();
        m1.put(1.0f32, 1);
        m1.put(1.0f32, 2);
        let mut m2 = F32I8ListMultimap::new();
        m2.put(1.0f32, 1);
        m2.put(1.0f32, 2);
        assert_eq!(m1, m2);
    }
    ...
    fn test_with_key_value_fluent() {
        let m = F32I8ListMultimap::new()
            .with_key_value(1.0f32, 1)
            .with_key_value(1.0f32, 2)
            .with_key_value(2.0f32, 3);
        assert_eq!(m.size(), 3);
        assert_eq!(m.keys_count(), 2);
    }
