...
/// List multimap from `i16` keys to `char` values.
...
#[derive(Debug, Clone)]
pub struct I16CharListMultimap {
    inner: HashMap<i16, Vec<char>>,
    total_size: usize,
}

impl I16CharListMultimap {
    pub fn new() -> Self {
        I16CharListMultimap {
            inner: HashMap::new(),
            total_size: 0,
    ...
    pub fn put(&mut self, key: i16, value: char) {
    ...
    pub fn get_all(&self, key: i16) -> Vec<char> {
        self.get(key).to_vec()
    }

    /// Removes all values for the given key. Returns the removed values.
    pub fn remove_all(&mut self, key: i16) -> Vec<char> {
        let internal_key = key;
    ...
    pub fn contains_key_value(&self, key: i16, value: char) -> bool {
    ...
    pub fn for_each_key_value(&self, mut f: impl FnMut(i16, &[char])) {
        for (k, vals) in &self.inner {
            f(*k, vals);
        }
    }
    ...
    pub fn to_vec(&self) -> Vec<(i16, char)> {
...
impl Default for I16CharListMultimap {
    fn default() -> Self {
        Self::new()
    }
}

impl PartialEq for I16CharListMultimap {
...
impl fmt::Display for I16CharListMultimap {
...
mod tests {
    ...
    #[test]
    fn test_put_get() {
        let mut m = I16CharListMultimap::new();
        m.put(1, 'a');
        m.put(1, 'b');
        m.put(2, 'c');
    ...
    #[test]
    fn test_equals() {
        let mut m1 = I16CharListMultimap::new();
        m1.put(1, 'a');
        m1.put(1, 'b');
        let mut m2 = I16CharListMultimap::new();
        m2.put(1, 'a');
        m2.put(1, 'b');
        assert_eq!(m1, m2);
    }
