// AUTO-GENERATED. DO NOT EDIT.
use crate::stack::i8_array_stack::I8ArrayStack;
use std::fmt;
use std::sync::Arc;

/// Immutable stack. Push/pop return new stacks (persistent data structure).
#[derive(Debug, Clone)]
pub struct ImmutableI8ArrayStack {
    items: Arc<[i8]>,
}

impl ImmutableI8ArrayStack {
    pub fn from_mutable(stack: &I8ArrayStack) -> Self {
        ImmutableI8ArrayStack {
            items: Arc::from(stack.to_vec().into_boxed_slice()),
        }
    }
    pub fn of(values: &[i8]) -> Self {
        ImmutableI8ArrayStack {
            items: Arc::from(values),
        }
    }
    pub fn peek(&self) -> Option<i8> {
        self.items.first().copied()
    }
    pub fn len(&self) -> usize {
        self.items.len()
    }
    pub fn is_empty(&self) -> bool {
        self.items.is_empty()
    }

    /// Returns (new stack without top, popped value).
    pub fn pop(&self) -> Option<(Self, i8)> {
        if self.items.is_empty() {
            return None;
        }
        let val = self.items[0];
        let rest: Vec<i8> = self.items[1..].to_vec();
        Some((
            ImmutableI8ArrayStack {
                items: Arc::from(rest.into_boxed_slice()),
            },
            val,
        ))
    }

    /// Returns a new stack with the value pushed on top.
    pub fn push(&self, value: i8) -> Self {
        let mut items = vec![value];
        items.extend_from_slice(&self.items);
        ImmutableI8ArrayStack {
            items: Arc::from(items.into_boxed_slice()),
        }
    }

    pub fn to_mutable(&self) -> I8ArrayStack {
        I8ArrayStack::of(&self.items)
    }
    pub fn contains(&self, value: i8) -> bool {
        self.items.contains(&value)
    }
    pub fn iter(&self) -> impl Iterator<Item = i8> + '_ {
        self.items.iter().copied()
    }
    pub fn to_vec(&self) -> Vec<i8> {
        self.items.to_vec()
    }
}

impl crate::traits::i8_collection::I8Collection for ImmutableI8ArrayStack {
    fn len(&self) -> usize {
        self.len()
    }
    fn contains(&self, value: i8) -> bool {
        self.contains(value)
    }
    fn iter(&self) -> impl Iterator<Item = i8> + '_ {
        self.iter()
    }
}

impl crate::traits::i8_collection::I8Stack for ImmutableI8ArrayStack {
    fn peek(&self) -> Option<i8> {
        self.peek()
    }
}

impl fmt::Display for ImmutableI8ArrayStack {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "[")?;
        for (i, v) in self.items.iter().enumerate() {
            if i > 0 {
                write!(f, ", ")?;
            }
            write!(f, "{}", v)?;
        }
        write!(f, "]")
    }
}

#[cfg(test)]
mod tests {
    use super::*;
    #[test]
    fn test_peek_len() {
        let im = ImmutableI8ArrayStack::of(&[1, 2]);
        assert_eq!(im.len(), 2);
        assert_eq!(im.peek(), Some(1));
    }
    #[test]
    fn test_push_immutable() {
        let im = ImmutableI8ArrayStack::of(&[1]);
        let im2 = im.push(2);
        assert_eq!(im.len(), 1);
        assert_eq!(im2.len(), 2);
    }
    #[test]
    fn test_pop_immutable() {
        let im = ImmutableI8ArrayStack::of(&[1, 2]);
        let (im2, val) = im.pop().unwrap();
        assert_eq!(val, 1);
        assert_eq!(im2.len(), 1);
        assert_eq!(im.len(), 2);
    }
    #[test]
    fn test_to_mutable() {
        let im = ImmutableI8ArrayStack::of(&[1]);
        let mut m = im.to_mutable();
        m.push(2);
        assert_eq!(im.len(), 1);
    }
    #[test]
    fn test_display() {
        assert!(!ImmutableI8ArrayStack::of(&[1]).to_string().is_empty());
    }
}
