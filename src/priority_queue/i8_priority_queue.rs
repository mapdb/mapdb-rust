...
/// Primitive min-heap priority queue of `i8` values, backed by a `Vec`.
/// O(log n) push/pop, O(1) peek. Duplicates are allowed.
#[derive(Debug, Clone)]
pub struct I8PriorityQueue {
    items: Vec<i8>,
}

impl I8PriorityQueue {
    pub fn new() -> Self {
        I8PriorityQueue { items: Vec::new() }
    }
    ...
    pub fn drain_sorted(mut self) -> Vec<i8> {
...
impl Default for I8PriorityQueue {
    fn default() -> Self {
        Self::new()
    }
}

impl fmt::Display for I8PriorityQueue {
...
mod tests {
    ...
    #[test]
    fn test_of_heapify() {
        let q = I8PriorityQueue::of(&[3, 1, 2]);
        assert_eq!(q.len(), 3);
        assert_eq!(q.peek(), Some(1));
    }
