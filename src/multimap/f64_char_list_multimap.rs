...
/// List multimap from `f64` keys to `char` values.
...
#[derive(Debug, Clone)]
pub struct F64CharListMultimap {
    inner: HashMap<u64, Vec<char>>,
    total_size: usize,
}

impl F64CharListMultimap {
    pub fn new() -> Self {
        F64CharListMultimap {
            inner: HashMap::new(),
            total_size: 0,
    ...
    pub fn put(&mut self, key: f64, value: char) {
    ...
    pub fn get_all(&self, key: f64) -> Vec<char> {
        self.get(key).to_vec()
    }

    /// Removes all values for the given key. Returns the removed values.
    pub fn remove_all(&mut self, key: f64) -> Vec<char> {
        let internal_key = f64::to_bits(key);
    ...
    pub fn contains_key_value(&self, key: f64, value: char) -> bool {
    ...
    pub fn for_each_key_value(&self, mut f: impl FnMut(f64, &[char])) {
        for (k, vals) in &self.inner {
    ...
    pub fn reject(&self, predicate: impl Fn(f64, char) -> bool) -> Self {
        let mut result = Self::new();
        for (k, vals) in &self.inner {
            let key = f64::from_bits(*k);
            for &val in vals {
    ...
    pub fn to_vec(&self) -> Vec<(f64, char)> {
    ...
    pub fn with_key_value(mut self, key: f64, value: char) -> Self {
        self.put(key, value);
        self
    }
}

impl Default for F64CharListMultimap {
    fn default() -> Self {
        Self::new()
    }
}

impl PartialEq for F64CharListMultimap {
...
impl fmt::Display for F64CharListMultimap {
...
mod tests {
    ...
    #[test]
    fn test_put_get() {
        let mut m = F64CharListMultimap::new();
        m.put(1.0f64, 'a');
        m.put(1.0f64, 'b');
        m.put(2.0f64, 'c');
    ...
    fn test_contains_key_value() {
        ...
        m.put(1.0f64, 'a');
        m.put(1.0f64, 'b');
        assert!(m.contains_key_value(1.0f64, 'a'));
        assert!(m.contains_key_value(1.0f64, 'b'));
        assert!(!m.contains_key_value(1.0f64, 'c'));
    }
    ...
    #[test]
    fn test_equals() {
        let mut m1 = F64CharListMultimap::new();
        m1.put(1.0f64, 'a');
        m1.put(1.0f64, 'b');
        let mut m2 = F64CharListMultimap::new();
        m2.put(1.0f64, 'a');
        m2.put(1.0f64, 'b');
        assert_eq!(m1, m2);
    }
    ...
    fn test_with_key_value_fluent() {
        let m = F64CharListMultimap::new()
            .with_key_value(1.0f64, 'a')
            .with_key_value(1.0f64, 'b')
            .with_key_value(2.0f64, 'c');
        assert_eq!(m.size(), 3);
        assert_eq!(m.keys_count(), 2);
    }
