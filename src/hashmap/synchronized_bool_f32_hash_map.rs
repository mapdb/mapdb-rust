...
use super::bool_f32_hash_map::BoolF32HashMap;

/// Thread-safe wrapper around [`BoolF32HashMap`]. Reads are concurrent;
/// mutations are exclusive. Lock poisoning panics (parity with Go).
#[derive(Debug, Default)]
pub struct SynchronizedBoolF32HashMap {
    inner: RwLock<BoolF32HashMap>,
}

impl SynchronizedBoolF32HashMap {
    pub fn new() -> Self {
        SynchronizedBoolF32HashMap {
            inner: RwLock::new(BoolF32HashMap::new()),
        }
    }

    pub fn with_capacity(capacity: usize) -> Self {
        SynchronizedBoolF32HashMap {
            inner: RwLock::new(BoolF32HashMap::with_capacity(capacity)),
        }
    }

    pub fn insert(&self, key: bool, value: f32) -> Option<f32> {
    ...
    pub fn remove(&self, key: bool) -> Option<f32> {
...
mod tests {
    ...
    #[test]
    fn test_basic_ops() {
        let m = SynchronizedBoolF32HashMap::new();
        assert!(m.is_empty());
        assert_eq!(m.insert(true, 1.0f32), None);
        assert_eq!(m.insert(false, 2.0f32), None);
        ...
        assert_eq!(m.remove(true), Some(2.0f32));
        assert!(!m.contains_key(true));
        m.clear();
        assert!(m.is_empty());
    }
    ...
    #[test]
    fn test_concurrent() {
        let m = Arc::new(SynchronizedBoolF32HashMap::new());
        let mut handles = Vec::new();
        for _ in 0..4 {
            let c = Arc::clone(&m);
            handles.push(thread::spawn(move || {
                for _ in 0..100 {
                    c.insert(true, 1.0f32);
                    c.get(true);
                }
            }));
        }
