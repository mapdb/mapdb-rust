...
/// Primitive min-heap priority queue of `i64` values, backed by a `Vec`.
/// O(log n) push/pop, O(1) peek. Duplicates are allowed.
#[derive(Debug, Clone)]
pub struct I64PriorityQueue {
    items: Vec<i64>,
}

impl I64PriorityQueue {
    pub fn new() -> Self {
        I64PriorityQueue { items: Vec::new() }
    }
    ...
    pub fn drain_sorted(mut self) -> Vec<i64> {
...
impl Default for I64PriorityQueue {
    fn default() -> Self {
        Self::new()
    }
}

impl fmt::Display for I64PriorityQueue {
...
mod tests {
    ...
    fn test_push_peek_pop_min() {
        let mut q = I64PriorityQueue::new();
        q.push(3);
        q.push(1);
        q.push(2);
    ...
    #[test]
    fn test_of_heapify() {
        let q = I64PriorityQueue::of(&[3, 1, 2]);
        assert_eq!(q.len(), 3);
        assert_eq!(q.peek(), Some(1));
    }
