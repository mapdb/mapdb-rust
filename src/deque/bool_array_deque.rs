// AUTO-GENERATED. DO NOT EDIT.

use std::collections::VecDeque;
use std::fmt;

/// Double-ended queue of `bool` values, backed by `VecDeque`.
/// O(1) amortized push/pop at both ends.
#[derive(Debug, Clone)]
pub struct BoolArrayDeque {
    inner: VecDeque<bool>,
}

impl BoolArrayDeque {
    pub fn new() -> Self {
        BoolArrayDeque {
            inner: VecDeque::new(),
        }
    }

    pub fn with_capacity(capacity: usize) -> Self {
        BoolArrayDeque {
            inner: VecDeque::with_capacity(capacity),
        }
    }

    pub fn of(values: &[bool]) -> Self {
        BoolArrayDeque {
            inner: values.iter().copied().collect(),
        }
    }

    /// Adds a value to the front of the deque.
    pub fn add_first(&mut self, value: bool) {
        self.inner.push_front(value);
    }

    /// Adds a value to the back of the deque.
    pub fn add_last(&mut self, value: bool) {
        self.inner.push_back(value);
    }

    /// Removes and returns the front element, or None if empty.
    pub fn remove_first(&mut self) -> Option<bool> {
        self.inner.pop_front()
    }

    /// Removes and returns the back element, or None if empty.
    pub fn remove_last(&mut self) -> Option<bool> {
        self.inner.pop_back()
    }

    /// Returns the front element without removing it, or None if empty.
    pub fn peek_first(&self) -> Option<bool> {
        self.inner.front().copied()
    }

    /// Returns the back element without removing it, or None if empty.
    pub fn peek_last(&self) -> Option<bool> {
        self.inner.back().copied()
    }

    pub fn len(&self) -> usize {
        self.inner.len()
    }
    pub fn is_empty(&self) -> bool {
        self.inner.is_empty()
    }
    pub fn clear(&mut self) {
        self.inner.clear();
    }

    /// Ensures the deque has capacity for `additional` more elements.
    /// Returns `TryReserveError` on allocator failure.
    pub fn try_reserve(
        &mut self,
        additional: usize,
    ) -> Result<(), std::collections::TryReserveError> {
        self.inner.try_reserve(additional)
    }

    pub fn contains(&self, value: bool) -> bool {
        self.inner.iter().any(|&v| v == value)
    }

    /// Iterates from front to back.
    pub fn iter(&self) -> impl Iterator<Item = bool> + '_ {
        self.inner.iter().copied()
    }

    pub fn for_each(&self, mut f: impl FnMut(bool)) {
        for v in self.iter() {
            f(v);
        }
    }

    pub fn any_satisfy(&self, predicate: impl Fn(bool) -> bool) -> bool {
        self.inner.iter().any(|&v| predicate(v))
    }

    pub fn all_satisfy(&self, predicate: impl Fn(bool) -> bool) -> bool {
        self.inner.iter().all(|&v| predicate(v))
    }

    pub fn to_vec(&self) -> Vec<bool> {
        self.iter().collect()
    }
}

impl Default for BoolArrayDeque {
    fn default() -> Self {
        Self::new()
    }
}

impl PartialEq for BoolArrayDeque {
    fn eq(&self, other: &Self) -> bool {
        if self.inner.len() != other.inner.len() {
            return false;
        }
        self.inner
            .iter()
            .zip(other.inner.iter())
            .all(|(a, b)| a == b)
    }
}

impl fmt::Display for BoolArrayDeque {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "[")?;
        for (i, v) in self.inner.iter().enumerate() {
            if i > 0 {
                write!(f, ", ")?;
            }
            write!(f, "{}", v)?;
        }
        write!(f, "]")
    }
}

#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn test_add_last_remove_first_fifo() {
        let mut d = BoolArrayDeque::new();
        d.add_last(true);
        d.add_last(false);
        d.add_last(true);
        assert_eq!(d.len(), 3);
        assert_eq!(d.remove_first(), Some(true));
        assert_eq!(d.remove_first(), Some(false));
        assert_eq!(d.remove_first(), Some(true));
        assert!(d.is_empty());
    }

    #[test]
    fn test_add_first_remove_last() {
        let mut d = BoolArrayDeque::new();
        d.add_first(true);
        d.add_first(false);
        d.add_first(true);
        // front: v[2], v[1], v[0]
        assert_eq!(d.peek_first(), Some(true));
        assert_eq!(d.peek_last(), Some(true));
        assert_eq!(d.remove_last(), Some(true));
        assert_eq!(d.remove_last(), Some(false));
        assert_eq!(d.remove_last(), Some(true));
    }

    #[test]
    fn test_remove_empty() {
        let mut d = BoolArrayDeque::new();
        assert_eq!(d.remove_first(), None);
        assert_eq!(d.remove_last(), None);
        assert_eq!(d.peek_first(), None);
        assert_eq!(d.peek_last(), None);
    }

    #[test]
    fn test_mixed_ops() {
        let mut d = BoolArrayDeque::new();
        d.add_last(false);
        d.add_first(true);
        d.add_last(true);
        // order: v[0], v[1], v[2]
        assert_eq!(d.remove_first(), Some(true));
        assert_eq!(d.remove_last(), Some(true));
        assert_eq!(d.remove_first(), Some(false));
    }

    #[test]
    fn test_contains() {
        let mut d = BoolArrayDeque::new();
        d.add_last(true);
        assert!(d.contains(true));
    }

    #[test]
    fn test_clear() {
        let mut d = BoolArrayDeque::of(&[true, false]);
        d.clear();
        assert!(d.is_empty());
    }

    #[test]
    fn test_iter_front_to_back() {
        let d = BoolArrayDeque::of(&[true, false, true]);
        let vals: Vec<_> = d.iter().collect();
        assert_eq!(vals, vec![true, false, true]);
    }

    #[test]
    fn test_display() {
        let mut d = BoolArrayDeque::new();
        d.add_last(true);
        assert!(!d.to_string().is_empty());
    }

    #[test]
    fn test_try_reserve_happy_path() {
        let mut d = BoolArrayDeque::new();
        d.try_reserve(10).unwrap();
    }

    #[test]
    fn test_try_reserve_propagates_capacity_overflow() {
        let mut d = BoolArrayDeque::new();
        assert!(d.try_reserve(usize::MAX / 2).is_err());
    }
}
