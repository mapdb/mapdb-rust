...
#[derive(Debug, Clone)]
pub struct I64I64ListMultimap {
    inner: HashMap<i64, Vec<i64>>,
    total_size: usize,
}

impl I64I64ListMultimap {
    pub fn new() -> Self {
        I64I64ListMultimap {
            inner: HashMap::new(),
            total_size: 0,
    ...
    pub fn get_all(&self, key: i64) -> Vec<i64> {
        self.get(key).to_vec()
    }

    /// Removes all values for the given key. Returns the removed values.
    pub fn remove_all(&mut self, key: i64) -> Vec<i64> {
        let internal_key = key;
    ...
    pub fn contains_key_value(&self, key: i64, value: i64) -> bool {
    ...
    pub fn for_each_key_value(&self, mut f: impl FnMut(i64, &[i64])) {
        for (k, vals) in &self.inner {
            f(*k, vals);
        }
    }
...
impl Default for I64I64ListMultimap {
    fn default() -> Self {
        Self::new()
    }
}

impl PartialEq for I64I64ListMultimap {
...
impl fmt::Display for I64I64ListMultimap {
...
mod tests {
    ...
    #[test]
    fn test_put_get() {
        let mut m = I64I64ListMultimap::new();
        m.put(1, 1);
        m.put(1, 2);
        m.put(2, 3);
    ...
    #[test]
    fn test_equals() {
        let mut m1 = I64I64ListMultimap::new();
        m1.put(1, 1);
        m1.put(1, 2);
        let mut m2 = I64I64ListMultimap::new();
        m2.put(1, 1);
        m2.put(1, 2);
        assert_eq!(m1, m2);
    }
