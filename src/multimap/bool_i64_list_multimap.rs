...
#[derive(Debug, Clone)]
pub struct BoolI64ListMultimap {
    inner: HashMap<bool, Vec<i64>>,
    total_size: usize,
}

impl BoolI64ListMultimap {
    pub fn new() -> Self {
        BoolI64ListMultimap {
            inner: HashMap::new(),
            total_size: 0,
    ...
    pub fn put(&mut self, key: bool, value: i64) {
    ...
    pub fn get_all(&self, key: bool) -> Vec<i64> {
        self.get(key).to_vec()
    }

    /// Removes all values for the given key. Returns the removed values.
    pub fn remove_all(&mut self, key: bool) -> Vec<i64> {
        let internal_key = key;
    ...
    pub fn contains_key_value(&self, key: bool, value: i64) -> bool {
    ...
    pub fn for_each_key_value(&self, mut f: impl FnMut(bool, &[i64])) {
        for (k, vals) in &self.inner {
            f(*k, vals);
        }
    }
    ...
    pub fn reject(&self, predicate: impl Fn(bool, i64) -> bool) -> Self {
        let mut result = Self::new();
        for (k, vals) in &self.inner {
            let key = *k;
            for &val in vals {
    ...
    pub fn to_vec(&self) -> Vec<(bool, i64)> {
    ...
    pub fn with_key_value(mut self, key: bool, value: i64) -> Self {
        self.put(key, value);
        self
    }
}

impl Default for BoolI64ListMultimap {
    fn default() -> Self {
        Self::new()
    }
}

impl PartialEq for BoolI64ListMultimap {
...
impl fmt::Display for BoolI64ListMultimap {
...
mod tests {
    ...
    #[test]
    fn test_put_get() {
        let mut m = BoolI64ListMultimap::new();
        m.put(true, 1);
        m.put(true, 2);
        m.put(false, 3);
    ...
    #[test]
    fn test_equals() {
        let mut m1 = BoolI64ListMultimap::new();
        m1.put(true, 1);
        m1.put(true, 2);
        let mut m2 = BoolI64ListMultimap::new();
        m2.put(true, 1);
        m2.put(true, 2);
        assert_eq!(m1, m2);
    }
