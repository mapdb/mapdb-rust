...
use crate::hashmap::i32_bool_hash_map::I32BoolHashMap;
use std::collections::HashMap;
use std::fmt;

/// Immutable hash map from `i32` keys to `bool` values. Clone is cheap (shared data).
#[derive(Debug, Clone)]
pub struct ImmutableI32BoolHashMap {
    inner: HashMap<i32, bool>,
}

impl ImmutableI32BoolHashMap {
    pub fn from_mutable(m: &I32BoolHashMap) -> Self {
        let mut inner = HashMap::new();
        m.for_each(|k, v| {
            inner.insert(k, v);
        });
        ImmutableI32BoolHashMap { inner }
    }
    ...
    pub fn to_mutable(&self) -> I32BoolHashMap {
...
impl PartialEq for ImmutableI32BoolHashMap {
...
impl fmt::Display for ImmutableI32BoolHashMap {
...
mod tests {
    ...
    fn test_get() {
        ...
        m.insert(1, true);
        m.insert(2, false);
        let im = ImmutableI32BoolHashMap::from_mutable(&m);
    ...
    fn test_display() {
        let mut m = I32BoolHashMap::new();
        m.insert(1, true);
        assert!(!ImmutableI32BoolHashMap::from_mutable(&m)
            .to_string()
            .is_empty());
    }
}

impl crate::traits::i32_bool_map::I32BoolMap for ImmutableI32BoolHashMap {
    fn len(&self) -> usize {
        self.len()
    }
