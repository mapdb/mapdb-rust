...
/// List multimap from `char` keys to `i64` values.
...
#[derive(Debug, Clone)]
pub struct CharI64ListMultimap {
    inner: HashMap<char, Vec<i64>>,
    total_size: usize,
}

impl CharI64ListMultimap {
    pub fn new() -> Self {
        CharI64ListMultimap {
            inner: HashMap::new(),
            total_size: 0,
    ...
    pub fn put(&mut self, key: char, value: i64) {
    ...
    pub fn get_all(&self, key: char) -> Vec<i64> {
        self.get(key).to_vec()
    }

    /// Removes all values for the given key. Returns the removed values.
    pub fn remove_all(&mut self, key: char) -> Vec<i64> {
        let internal_key = key;
    ...
    pub fn contains_key_value(&self, key: char, value: i64) -> bool {
    ...
    pub fn for_each_key_value(&self, mut f: impl FnMut(char, &[i64])) {
        for (k, vals) in &self.inner {
            f(*k, vals);
        }
    }
...
impl Default for CharI64ListMultimap {
    fn default() -> Self {
        Self::new()
    }
}

impl PartialEq for CharI64ListMultimap {
...
impl fmt::Display for CharI64ListMultimap {
...
mod tests {
    ...
    #[test]
    fn test_put_get() {
        let mut m = CharI64ListMultimap::new();
        m.put('a', 1);
        m.put('a', 2);
        m.put('b', 3);
    ...
    #[test]
    fn test_equals() {
        let mut m1 = CharI64ListMultimap::new();
        m1.put('a', 1);
        m1.put('a', 2);
        let mut m2 = CharI64ListMultimap::new();
        m2.put('a', 1);
        m2.put('a', 2);
        assert_eq!(m1, m2);
    }
