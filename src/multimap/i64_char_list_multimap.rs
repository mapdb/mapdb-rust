...
/// List multimap from `i64` keys to `char` values.
...
#[derive(Debug, Clone)]
pub struct I64CharListMultimap {
    inner: HashMap<i64, Vec<char>>,
    total_size: usize,
}

impl I64CharListMultimap {
    pub fn new() -> Self {
        I64CharListMultimap {
            inner: HashMap::new(),
            total_size: 0,
    ...
    pub fn put(&mut self, key: i64, value: char) {
    ...
    pub fn get_all(&self, key: i64) -> Vec<char> {
        self.get(key).to_vec()
    }

    /// Removes all values for the given key. Returns the removed values.
    pub fn remove_all(&mut self, key: i64) -> Vec<char> {
        let internal_key = key;
    ...
    pub fn contains_key_value(&self, key: i64, value: char) -> bool {
    ...
    pub fn for_each_key_value(&self, mut f: impl FnMut(i64, &[char])) {
        for (k, vals) in &self.inner {
            f(*k, vals);
        }
    }
    ...
    pub fn to_vec(&self) -> Vec<(i64, char)> {
...
impl Default for I64CharListMultimap {
    fn default() -> Self {
        Self::new()
    }
}

impl PartialEq for I64CharListMultimap {
...
impl fmt::Display for I64CharListMultimap {
...
mod tests {
    ...
    #[test]
    fn test_put_get() {
        let mut m = I64CharListMultimap::new();
        m.put(1, 'a');
        m.put(1, 'b');
        m.put(2, 'c');
    ...
    #[test]
    fn test_equals() {
        let mut m1 = I64CharListMultimap::new();
        m1.put(1, 'a');
        m1.put(1, 'b');
        let mut m2 = I64CharListMultimap::new();
        m2.put(1, 'a');
        m2.put(1, 'b');
        assert_eq!(m1, m2);
    }
