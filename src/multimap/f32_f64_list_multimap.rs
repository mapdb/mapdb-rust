...
#[derive(Debug, Clone)]
pub struct F32F64ListMultimap {
    inner: HashMap<u32, Vec<f64>>,
    total_size: usize,
}

impl F32F64ListMultimap {
    pub fn new() -> Self {
        F32F64ListMultimap {
            inner: HashMap::new(),
            total_size: 0,
    ...
    pub fn put(&mut self, key: f32, value: f64) {
    ...
    pub fn get_all(&self, key: f32) -> Vec<f64> {
        self.get(key).to_vec()
    }

    /// Removes all values for the given key. Returns the removed values.
    pub fn remove_all(&mut self, key: f32) -> Vec<f64> {
        let internal_key = f32::to_bits(key);
    ...
    pub fn contains_key_value(&self, key: f32, value: f64) -> bool {
        self.get(key)
            .iter()
    ...
    pub fn for_each_key_value(&self, mut f: impl FnMut(f32, &[f64])) {
        for (k, vals) in &self.inner {
...
impl Default for F32F64ListMultimap {
    fn default() -> Self {
        Self::new()
    }
}

impl PartialEq for F32F64ListMultimap {
...
impl fmt::Display for F32F64ListMultimap {
...
mod tests {
    ...
    #[test]
    fn test_put_get() {
        let mut m = F32F64ListMultimap::new();
        m.put(1.0f32, 1.0f64);
        m.put(1.0f32, 2.0f64);
        m.put(2.0f32, 3.0f64);
    ...
    fn test_contains_key_value() {
        ...
        m.put(1.0f32, 1.0f64);
        m.put(1.0f32, 2.0f64);
        assert!(m.contains_key_value(1.0f32, 1.0f64));
        assert!(m.contains_key_value(1.0f32, 2.0f64));
        assert!(!m.contains_key_value(1.0f32, 3.0f64));
    }
    ...
    #[test]
    fn test_equals() {
        let mut m1 = F32F64ListMultimap::new();
        m1.put(1.0f32, 1.0f64);
        m1.put(1.0f32, 2.0f64);
        let mut m2 = F32F64ListMultimap::new();
        m2.put(1.0f32, 1.0f64);
        m2.put(1.0f32, 2.0f64);
        assert_eq!(m1, m2);
    }
    ...
    fn test_with_key_value_fluent() {
        let m = F32F64ListMultimap::new()
            .with_key_value(1.0f32, 1.0f64)
            .with_key_value(1.0f32, 2.0f64)
            .with_key_value(2.0f32, 3.0f64);
        assert_eq!(m.size(), 3);
        assert_eq!(m.keys_count(), 2);
    }
