...
impl F32I64HashMap {
    ...
    pub fn insert(&mut self, key: f32, value: i64) -> Option<i64> {
        self.inner.insert(key, value)
    }

    /// Returns the value for the key, or None.
    pub fn get(&self, key: f32) -> Option<i64> {
        self.inner.get(key)
    }

    /// Returns the value for the key, or the default.
    pub fn get_or_default(&self, key: f32, default: i64) -> i64 {
        self.inner.get(key).unwrap_or(default)
    }

    /// Removes the key. Returns the old value if present.
    pub fn remove(&mut self, key: f32) -> Option<i64> {
    ...
    pub fn for_each(&self, mut f: impl FnMut(f32, i64)) {
        for (k, v) in self.iter() {
            f(k, v);
        }
    }

    pub fn select(&self, predicate: impl Fn(f32, i64) -> bool) -> Self {
    ...
    pub fn reject(&self, predicate: impl Fn(f32, i64) -> bool) -> Self {
        let mut result = Self::new();
        for (k, v) in self.iter() {
            if !predicate(k, v) {
                result.insert(k, v);
    ...
    pub fn detect(&self, predicate: impl Fn(f32, i64) -> bool) -> Option<(f32, i64)> {
        self.iter().find(|&(k, v)| predicate(k, v))
    }

    pub fn any_satisfy(&self, predicate: impl Fn(f32, i64) -> bool) -> bool {
        self.iter().any(|(k, v)| predicate(k, v))
    }

    pub fn all_satisfy(&self, predicate: impl Fn(f32, i64) -> bool) -> bool {
        self.iter().all(|(k, v)| predicate(k, v))
    }

    pub fn none_satisfy(&self, predicate: impl Fn(f32, i64) -> bool) -> bool {
        !self.iter().any(|(k, v)| predicate(k, v))
    }

    pub fn count(&self, predicate: impl Fn(f32, i64) -> bool) -> usize {
        self.iter().filter(|&(k, v)| predicate(k, v)).count()
    }

    pub fn inject_into<R>(&self, initial: R, mut f: impl FnMut(R, f32, i64) -> R) -> R {
        let mut acc = initial;
        for (k, v) in self.iter() {
            acc = f(acc, k, v);
        }
    ...
    pub fn with_key_value(mut self, key: f32, value: i64) -> Self {
...
impl Default for F32I64HashMap {
    fn default() -> Self {
        Self::new()
    }
}

impl PartialEq for F32I64HashMap {
...
impl fmt::Display for F32I64HashMap {
...
mod tests {
    ...
    #[test]
    fn test_equals() {
        let mut m1 = F32I64HashMap::new();
        m1.insert(1.0f32, 1);
        m1.insert(2.0f32, 2);
        let mut m2 = F32I64HashMap::new();
        m2.insert(2.0f32, 2);
        m2.insert(1.0f32, 1);
        assert_eq!(m1, m2);
    }
...
impl crate::traits::f32_i64_map::F32I64Map for F32I64HashMap {
    fn len(&self) -> usize {
        self.len()
    }
    ...
    fn get(&self, key: f32) -> Option<i64> {
        self.get(key)
    }
    fn iter(&self) -> impl Iterator<Item = (f32, i64)> + '_ {
        self.iter()
    }
}

impl crate::traits::f32_i64_map::F32I64MutableMap for F32I64HashMap {
    fn insert(&mut self, key: f32, value: i64) -> Option<i64> {
        self.insert(key, value)
    }
    fn remove(&mut self, key: f32) -> Option<i64> {
        self.remove(key)
    }
    fn clear(&mut self) {
        self.clear()
