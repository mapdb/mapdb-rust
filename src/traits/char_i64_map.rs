...
/// Read-only trait for any map from `char` keys to `i64` values.
///
...
pub trait CharI64Map {
    ...
    fn for_each_key_value(&self, mut f: impl FnMut(char, i64)) {
...
/// Mutable map trait extending CharI64Map.
pub trait CharI64MutableMap: CharI64Map {
...
mod verify {
    use super::*;
    fn _assert_map<T: CharI64Map>() {}
    fn _assert_mut<T: CharI64MutableMap>() {}

    /// Compile-time verification: every concrete `char` → `i64` map type
    /// implements the read-only and (where applicable) mutable map trait.
    #[test]
    fn types_implement_traits() {
        _assert_map::<crate::hashmap::char_i64_hash_map::CharI64HashMap>();
        _assert_mut::<crate::hashmap::char_i64_hash_map::CharI64HashMap>();
        _assert_map::<crate::treemap::char_i64_tree_map::CharI64TreeMap>();
        _assert_mut::<crate::treemap::char_i64_tree_map::CharI64TreeMap>();
        _assert_map::<crate::hashmap::char_i64_hash_bi_map::CharI64HashBiMap>();
        _assert_mut::<crate::hashmap::char_i64_hash_bi_map::CharI64HashBiMap>();
        _assert_map::<crate::immutable::immutable_char_i64_hash_map::ImmutableCharI64HashMap>();
    }
}
