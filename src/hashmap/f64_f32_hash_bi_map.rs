...
use crate::hashmap::f32_f64_hash_bi_map::F32F64HashBiMap;
use std::fmt;
...
impl F64F32HashBiMap {
    ...
    pub fn get_key(&self, value: f32) -> Option<f64> {
    ...
    pub fn remove_value(&mut self, value: f32) -> Option<f64> {
    ...
    pub fn inverse(&self) -> F32F64HashBiMap {
        F32F64HashBiMap::from_maps(self.reverse.clone(), self.forward.clone())
    }
    ...
    pub(crate) fn from_maps(
        ...
        F64F32HashBiMap { forward, reverse }
    }
}

impl Default for F64F32HashBiMap {
    fn default() -> Self {
        Self::new()
    }
}

impl PartialEq for F64F32HashBiMap {
...
impl fmt::Display for F64F32HashBiMap {
...
mod tests {
    ...
    fn test_get_key() {
        ...
        m.insert(1.0f64, 1.0f32);
        m.insert(2.0f64, 2.0f32);
        assert_eq!(m.get_key(1.0f32), Some(1.0f64));
        assert_eq!(m.get_key(2.0f32), Some(2.0f64));
    ...
    fn test_insert_overwrite() {
        ...
        assert_eq!(m.get_key(2.0f32), Some(1.0f64));
    ...
    fn test_remove_value() {
        ...
        m.insert(1.0f64, 1.0f32);
        m.insert(2.0f64, 2.0f32);
        assert_eq!(m.remove_value(1.0f32), Some(1.0f64));
        assert_eq!(m.len(), 1);
        assert!(!m.contains_key(1.0f64));
        assert!(!m.contains_value(1.0f32));
    }
    ...
    fn test_inverse() {
        ...
        assert_eq!(inv.get(1.0f32), Some(1.0f64));
        assert_eq!(inv.get(2.0f32), Some(2.0f64));
        // inverse reverse lookup: old key -> old value
        assert_eq!(inv.get_key(1.0f64), Some(1.0f32));
        assert_eq!(inv.len(), 2);
    }
...
impl crate::traits::f64_f32_map::F64F32Map for F64F32HashBiMap {
    fn len(&self) -> usize {
        self.len()
    }
...
impl crate::traits::f64_f32_map::F64F32MutableMap for F64F32HashBiMap {
