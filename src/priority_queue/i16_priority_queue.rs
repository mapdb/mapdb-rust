...
/// Primitive min-heap priority queue of `i16` values, backed by a `Vec`.
/// O(log n) push/pop, O(1) peek. Duplicates are allowed.
#[derive(Debug, Clone)]
pub struct I16PriorityQueue {
    items: Vec<i16>,
}

impl I16PriorityQueue {
    pub fn new() -> Self {
        I16PriorityQueue { items: Vec::new() }
    }
    ...
    pub fn drain_sorted(mut self) -> Vec<i16> {
...
impl Default for I16PriorityQueue {
    fn default() -> Self {
        Self::new()
    }
}

impl fmt::Display for I16PriorityQueue {
...
mod tests {
    ...
    fn test_push_peek_pop_min() {
        let mut q = I16PriorityQueue::new();
        q.push(3);
        q.push(1);
        q.push(2);
    ...
    #[test]
    fn test_of_heapify() {
        let q = I16PriorityQueue::of(&[3, 1, 2]);
        assert_eq!(q.len(), 3);
        assert_eq!(q.peek(), Some(1));
    }
