// AUTO-GENERATED. DO NOT EDIT.

use crate::hash_table::OpenHashMap;
use crate::hashmap::f32_char_hash_bi_map::F32CharHashBiMap;
use std::fmt;

/// Bidirectional hash map from `char` keys to `f32` values.
/// Both key->value and value->key lookups are O(1).
/// Internally uses two `OpenHashMap` instances: forward (key->value) and reverse (value->key).
#[derive(Debug, Clone)]
pub struct CharF32HashBiMap {
    forward: OpenHashMap<char, f32>,
    reverse: OpenHashMap<f32, char>,
}

impl CharF32HashBiMap {
    pub fn new() -> Self {
        CharF32HashBiMap {
            forward: OpenHashMap::new(),
            reverse: OpenHashMap::new(),
        }
    }

    pub fn with_capacity(capacity: usize) -> Self {
        CharF32HashBiMap {
            forward: OpenHashMap::with_capacity(capacity),
            reverse: OpenHashMap::with_capacity(capacity),
        }
    }

    /// Inserts a key-value pair into the bi-map.
    /// If the key already existed, the old value is removed from the reverse map
    /// and returned. The new value->key mapping is then inserted.
    pub fn insert(&mut self, key: char, value: f32) -> Option<f32> {
        // If this key already maps to an old value, remove old_value->key from reverse
        let old = self.forward.insert(key, value);
        if let Some(old_val) = old {
            self.reverse.remove(old_val);
        }
        // If this value already maps to an old key, remove old_key->value from forward
        if let Some(old_key) = self.reverse.insert(value, key) {
            if old_key != key {
                self.forward.remove(old_key);
            }
        }
        old
    }

    /// Forward lookup: returns the value for the given key.
    pub fn get(&self, key: char) -> Option<f32> {
        self.forward.get(key)
    }

    /// Reverse lookup: returns the key for the given value.
    pub fn get_key(&self, value: f32) -> Option<char> {
        self.reverse.get(value)
    }

    /// Removes by key. Returns the old value if present, and also removes
    /// the corresponding reverse mapping.
    pub fn remove(&mut self, key: char) -> Option<f32> {
        if let Some(val) = self.forward.remove(key) {
            self.reverse.remove(val);
            Some(val)
        } else {
            None
        }
    }

    /// Removes by value. Returns the old key if present, and also removes
    /// the corresponding forward mapping.
    pub fn remove_value(&mut self, value: f32) -> Option<char> {
        if let Some(key) = self.reverse.remove(value) {
            self.forward.remove(key);
            Some(key)
        } else {
            None
        }
    }

    pub fn contains_key(&self, key: char) -> bool {
        self.forward.contains_key(key)
    }

    pub fn contains_value(&self, value: f32) -> bool {
        self.reverse.contains_key(value)
    }

    pub fn len(&self) -> usize {
        self.forward.len()
    }
    pub fn is_empty(&self) -> bool {
        self.forward.is_empty()
    }

    pub fn clear(&mut self) {
        self.forward.clear();
        self.reverse.clear();
    }

    /// Ensures both the forward and reverse maps can accept `additional`
    /// more entries without a rehash. Returns `TryReserveError` on
    /// allocator failure. See `docs/rust/error-handling.md`.
    pub fn try_reserve(
        &mut self,
        additional: usize,
    ) -> Result<(), std::collections::TryReserveError> {
        self.forward.try_reserve(additional)?;
        self.reverse.try_reserve(additional)?;
        Ok(())
    }

    /// Iterates over (key, value) pairs via the forward map.
    pub fn iter(&self) -> impl Iterator<Item = (char, f32)> + '_ {
        self.forward.iter()
    }

    pub fn keys(&self) -> impl Iterator<Item = char> + '_ {
        self.forward.iter().map(|(k, _)| k)
    }

    pub fn values(&self) -> impl Iterator<Item = f32> + '_ {
        self.forward.iter().map(|(_, v)| v)
    }

    pub fn for_each(&self, mut f: impl FnMut(char, f32)) {
        for (k, v) in self.iter() {
            f(k, v);
        }
    }

    /// Returns a new BiMap with forward and reverse swapped.
    /// The returned map has value->key as forward and key->value as reverse.
    pub fn inverse(&self) -> F32CharHashBiMap {
        F32CharHashBiMap::from_maps(self.reverse.clone(), self.forward.clone())
    }

    /// Internal constructor from pre-built maps (used by the inverse type's `inverse()` call).
    pub(crate) fn from_maps(
        forward: OpenHashMap<char, f32>,
        reverse: OpenHashMap<f32, char>,
    ) -> Self {
        CharF32HashBiMap { forward, reverse }
    }
}

impl Default for CharF32HashBiMap {
    fn default() -> Self {
        Self::new()
    }
}

impl PartialEq for CharF32HashBiMap {
    fn eq(&self, other: &Self) -> bool {
        if self.len() != other.len() {
            return false;
        }
        self.iter()
            .all(|(k, v)| other.get(k).is_some_and(|ov| v.to_bits() == ov.to_bits()))
    }
}

impl fmt::Display for CharF32HashBiMap {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "{{")?;
        let mut first = true;
        for (k, v) in self.iter() {
            if !first {
                write!(f, ", ")?;
            }
            write!(f, "{}={}", k, v)?;
            first = false;
        }
        write!(f, "}}")
    }
}

#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn test_insert_get() {
        let mut m = CharF32HashBiMap::new();
        m.insert('a', 1.0f32);
        m.insert('b', 2.0f32);
        assert_eq!(m.get('a'), Some(1.0f32));
        assert_eq!(m.get('b'), Some(2.0f32));
        assert_eq!(m.len(), 2);
    }

    #[test]
    fn test_get_key() {
        let mut m = CharF32HashBiMap::new();
        m.insert('a', 1.0f32);
        m.insert('b', 2.0f32);
        assert_eq!(m.get_key(1.0f32), Some('a'));
        assert_eq!(m.get_key(2.0f32), Some('b'));
        assert_eq!(m.get_key(99.0f32), None);
    }

    #[test]
    fn test_insert_overwrite() {
        let mut m = CharF32HashBiMap::new();
        m.insert('a', 1.0f32);
        let old = m.insert('a', 2.0f32);
        assert_eq!(old, Some(1.0f32));
        assert_eq!(m.get('a'), Some(2.0f32));
        // Reverse should reflect the new mapping
        assert_eq!(m.get_key(2.0f32), Some('a'));
        // Old reverse mapping should be gone
        assert_eq!(m.get_key(1.0f32), None);
    }

    #[test]
    fn test_remove() {
        let mut m = CharF32HashBiMap::new();
        m.insert('a', 1.0f32);
        m.insert('b', 2.0f32);
        assert_eq!(m.remove('a'), Some(1.0f32));
        assert_eq!(m.len(), 1);
        assert!(!m.contains_key('a'));
        assert!(!m.contains_value(1.0f32));
    }

    #[test]
    fn test_remove_value() {
        let mut m = CharF32HashBiMap::new();
        m.insert('a', 1.0f32);
        m.insert('b', 2.0f32);
        assert_eq!(m.remove_value(1.0f32), Some('a'));
        assert_eq!(m.len(), 1);
        assert!(!m.contains_key('a'));
        assert!(!m.contains_value(1.0f32));
    }

    #[test]
    fn test_contains() {
        let mut m = CharF32HashBiMap::new();
        m.insert('a', 1.0f32);
        assert!(m.contains_key('a'));
        assert!(!m.contains_key('z'));
        assert!(m.contains_value(1.0f32));
        assert!(!m.contains_value(99.0f32));
    }

    #[test]
    fn test_inverse() {
        let mut m = CharF32HashBiMap::new();
        m.insert('a', 1.0f32);
        m.insert('b', 2.0f32);
        let inv = m.inverse();
        // inverse forward lookup: old value -> old key
        assert_eq!(inv.get(1.0f32), Some('a'));
        assert_eq!(inv.get(2.0f32), Some('b'));
        // inverse reverse lookup: old key -> old value
        assert_eq!(inv.get_key('a'), Some(1.0f32));
        assert_eq!(inv.len(), 2);
    }

    #[test]
    fn test_clear() {
        let mut m = CharF32HashBiMap::new();
        m.insert('a', 1.0f32);
        m.clear();
        assert!(m.is_empty());
        assert_eq!(m.len(), 0);
    }

    #[test]
    fn test_display() {
        let mut m = CharF32HashBiMap::new();
        m.insert('a', 1.0f32);
        let s = m.to_string();
        assert!(!s.is_empty());
        assert!(s.starts_with('{'));
        assert!(s.ends_with('}'));
    }

    #[test]
    fn test_for_each() {
        let mut m = CharF32HashBiMap::new();
        m.insert('a', 1.0f32);
        m.insert('b', 2.0f32);
        let mut count = 0usize;
        m.for_each(|_k, _v| {
            count += 1;
        });
        assert_eq!(count, 2);
    }

    #[test]
    fn test_try_reserve_happy_path() {
        let mut m = CharF32HashBiMap::new();
        m.try_reserve(100).unwrap();
    }

    #[test]
    fn test_try_reserve_propagates_capacity_overflow() {
        let mut m = CharF32HashBiMap::new();
        assert!(m.try_reserve(usize::MAX / 2).is_err());
    }
}

impl crate::traits::char_f32_map::CharF32Map for CharF32HashBiMap {
    fn len(&self) -> usize {
        self.len()
    }
    fn contains_key(&self, key: char) -> bool {
        self.contains_key(key)
    }
    fn get(&self, key: char) -> Option<f32> {
        self.get(key)
    }
    fn iter(&self) -> impl Iterator<Item = (char, f32)> + '_ {
        self.iter()
    }
}

impl crate::traits::char_f32_map::CharF32MutableMap for CharF32HashBiMap {
    fn insert(&mut self, key: char, value: f32) -> Option<f32> {
        self.insert(key, value)
    }
    fn remove(&mut self, key: char) -> Option<f32> {
        self.remove(key)
    }
    fn clear(&mut self) {
        self.clear()
    }
}
