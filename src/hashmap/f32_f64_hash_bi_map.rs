...
use crate::hashmap::f64_f32_hash_bi_map::F64F32HashBiMap;
use std::fmt;
...
impl F32F64HashBiMap {
    ...
    pub fn get_key(&self, value: f64) -> Option<f32> {
    ...
    pub fn remove_value(&mut self, value: f64) -> Option<f32> {
    ...
    pub fn inverse(&self) -> F64F32HashBiMap {
        F64F32HashBiMap::from_maps(self.reverse.clone(), self.forward.clone())
    }
    ...
    pub(crate) fn from_maps(
        ...
        F32F64HashBiMap { forward, reverse }
    }
}

impl Default for F32F64HashBiMap {
    fn default() -> Self {
        Self::new()
    }
}

impl PartialEq for F32F64HashBiMap {
...
impl fmt::Display for F32F64HashBiMap {
...
mod tests {
    ...
    fn test_get_key() {
        ...
        m.insert(1.0f32, 1.0f64);
        m.insert(2.0f32, 2.0f64);
        assert_eq!(m.get_key(1.0f64), Some(1.0f32));
        assert_eq!(m.get_key(2.0f64), Some(2.0f32));
    ...
    fn test_insert_overwrite() {
        ...
        assert_eq!(m.get_key(2.0f64), Some(1.0f32));
    ...
    fn test_remove_value() {
        ...
        m.insert(1.0f32, 1.0f64);
        m.insert(2.0f32, 2.0f64);
        assert_eq!(m.remove_value(1.0f64), Some(1.0f32));
        assert_eq!(m.len(), 1);
        assert!(!m.contains_key(1.0f32));
        assert!(!m.contains_value(1.0f64));
    }
    ...
    fn test_inverse() {
        ...
        assert_eq!(inv.get(1.0f64), Some(1.0f32));
        assert_eq!(inv.get(2.0f64), Some(2.0f32));
        // inverse reverse lookup: old key -> old value
        assert_eq!(inv.get_key(1.0f32), Some(1.0f64));
        assert_eq!(inv.len(), 2);
    }
...
impl crate::traits::f32_f64_map::F32F64Map for F32F64HashBiMap {
    fn len(&self) -> usize {
        self.len()
    }
...
impl crate::traits::f32_f64_map::F32F64MutableMap for F32F64HashBiMap {
