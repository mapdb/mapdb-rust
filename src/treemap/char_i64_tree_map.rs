...
/// Sorted map from `char` keys to `i64` values.
#[derive(Debug, Clone)]
pub struct CharI64TreeMap {
    inner: BTreeMap<char, i64>,
}
...
impl CharI64TreeMap {
    ...
    pub fn min(&self) -> Option<(char, i64)> {
        self.inner.iter().next().map(|(&k, &v)| (k, v))
    }

    pub fn max(&self) -> Option<(char, i64)> {
...
impl Default for CharI64TreeMap {
    fn default() -> Self {
        Self::new()
    }
}

impl PartialEq for CharI64TreeMap {
...
impl fmt::Display for CharI64TreeMap {
...
impl crate::traits::char_i64_map::CharI64Map for CharI64TreeMap {
    fn len(&self) -> usize {
        self.len()
    }
...
impl crate::traits::char_i64_map::CharI64MutableMap for CharI64TreeMap {
