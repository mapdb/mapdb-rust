...
impl F64F32HashMap {
    ...
    pub fn reject(&self, predicate: impl Fn(f64, f32) -> bool) -> Self {
        let mut result = Self::new();
        for (k, v) in self.iter() {
            if !predicate(k, v) {
                result.insert(k, v);
    ...
    pub fn detect(&self, predicate: impl Fn(f64, f32) -> bool) -> Option<(f64, f32)> {
    ...
    pub fn none_satisfy(&self, predicate: impl Fn(f64, f32) -> bool) -> bool {
        !self.iter().any(|(k, v)| predicate(k, v))
    }

    pub fn count(&self, predicate: impl Fn(f64, f32) -> bool) -> usize {
        self.iter().filter(|&(k, v)| predicate(k, v)).count()
    }

    pub fn inject_into<R>(&self, initial: R, mut f: impl FnMut(R, f64, f32) -> R) -> R {
        let mut acc = initial;
        for (k, v) in self.iter() {
            acc = f(acc, k, v);
        }
    ...
    pub fn with_key_value(mut self, key: f64, value: f32) -> Self {
...
impl Default for F64F32HashMap {
    fn default() -> Self {
        Self::new()
    }
}

impl PartialEq for F64F32HashMap {
...
impl fmt::Display for F64F32HashMap {
...
mod tests {
    ...
    fn test_insert_overwrite() {
        ...
        assert_eq!(m.get(1.0f64), Some(2.0f32));
    }
    ...
    #[test]
    fn test_equals() {
        let mut m1 = F64F32HashMap::new();
        m1.insert(1.0f64, 1.0f32);
        m1.insert(2.0f64, 2.0f32);
        let mut m2 = F64F32HashMap::new();
        m2.insert(2.0f64, 2.0f32);
        m2.insert(1.0f64, 1.0f32);
        assert_eq!(m1, m2);
    }
    ...
    fn test_nan_key_findable() {
        let mut m = F64F32HashMap::new();
        m.insert(f64::NAN, 1.0f32);
        assert!(m.contains_key(f64::NAN));
        assert_eq!(m.get(f64::NAN), Some(1.0f32));
    }
    ...
    fn test_nan_key_replaces_does_not_duplicate() {
        let mut m = F64F32HashMap::new();
        m.insert(f64::NAN, 1.0f32);
        m.insert(f64::NAN, 2.0f32);
        m.insert(f64::NAN, 3.0f32);
        assert_eq!(m.len(), 1);
        assert_eq!(m.get(f64::NAN), Some(3.0f32));
    }
    ...
    fn test_negative_zero_distinct_from_positive_zero() {
        let mut m = F64F32HashMap::new();
        m.insert(0.0f64, 1.0f32);
        m.insert(-0.0f64, 2.0f32);
        assert_eq!(m.len(), 2);
        assert_eq!(m.get(0.0f64), Some(1.0f32));
        assert_eq!(m.get(-0.0f64), Some(2.0f32));
    }

    #[test]
    fn test_infinity_keys() {
        let mut m = F64F32HashMap::new();
        m.insert(f64::INFINITY, 1.0f32);
        m.insert(f64::NEG_INFINITY, 2.0f32);
        assert_eq!(m.len(), 2);
        assert_eq!(m.get(f64::INFINITY), Some(1.0f32));
        assert_eq!(m.get(f64::NEG_INFINITY), Some(2.0f32));
        assert!(m.contains_key(f64::INFINITY));
        assert!(m.contains_key(f64::NEG_INFINITY));
    }
}

impl crate::traits::f64_f32_map::F64F32Map for F64F32HashMap {
    fn len(&self) -> usize {
        self.len()
    }
...
impl crate::traits::f64_f32_map::F64F32MutableMap for F64F32HashMap {
