...
/// List multimap from `char` keys to `bool` values.
...
#[derive(Debug, Clone)]
pub struct CharBoolListMultimap {
    inner: HashMap<char, Vec<bool>>,
    total_size: usize,
}

impl CharBoolListMultimap {
    pub fn new() -> Self {
        CharBoolListMultimap {
            inner: HashMap::new(),
            total_size: 0,
        }
    }
    ...
    pub fn put(&mut self, key: char, value: bool) {
    ...
    pub fn get_all(&self, key: char) -> Vec<bool> {
        self.get(key).to_vec()
    }

    /// Removes all values for the given key. Returns the removed values.
    pub fn remove_all(&mut self, key: char) -> Vec<bool> {
        let internal_key = key;
    ...
    pub fn contains_key_value(&self, key: char, value: bool) -> bool {
    ...
    pub fn for_each_key_value(&self, mut f: impl FnMut(char, &[bool])) {
        for (k, vals) in &self.inner {
            f(*k, vals);
        }
    }
...
impl Default for CharBoolListMultimap {
    fn default() -> Self {
        Self::new()
    }
}

impl PartialEq for CharBoolListMultimap {
...
impl fmt::Display for CharBoolListMultimap {
...
mod tests {
    ...
    #[test]
    fn test_put_get() {
        let mut m = CharBoolListMultimap::new();
        m.put('a', true);
        m.put('a', false);
        m.put('b', true);
    ...
    fn test_contains_key_value() {
        ...
        m.put('a', true);
        m.put('a', false);
        assert!(m.contains_key_value('a', true));
        assert!(m.contains_key_value('a', false));
    }
    ...
    #[test]
    fn test_equals() {
        let mut m1 = CharBoolListMultimap::new();
        m1.put('a', true);
        m1.put('a', false);
        let mut m2 = CharBoolListMultimap::new();
        m2.put('a', true);
        m2.put('a', false);
        assert_eq!(m1, m2);
    }
    ...
    fn test_with_key_value_fluent() {
        let m = CharBoolListMultimap::new()
            .with_key_value('a', true)
            .with_key_value('a', false)
            .with_key_value('b', true);
        assert_eq!(m.size(), 3);
        assert_eq!(m.keys_count(), 2);
    }
