...
impl I32I16HashMap {
    ...
    pub fn insert(&mut self, key: i32, value: i16) -> Option<i16> {
    ...
    pub fn remove(&mut self, key: i32) -> Option<i16> {
    ...
    pub fn reject(&self, predicate: impl Fn(i32, i16) -> bool) -> Self {
        let mut result = Self::new();
        for (k, v) in self.iter() {
            if !predicate(k, v) {
                result.insert(k, v);
    ...
    pub fn detect(&self, predicate: impl Fn(i32, i16) -> bool) -> Option<(i32, i16)> {
    ...
    pub fn none_satisfy(&self, predicate: impl Fn(i32, i16) -> bool) -> bool {
        !self.iter().any(|(k, v)| predicate(k, v))
    }

    pub fn count(&self, predicate: impl Fn(i32, i16) -> bool) -> usize {
        self.iter().filter(|&(k, v)| predicate(k, v)).count()
    }

    pub fn inject_into<R>(&self, initial: R, mut f: impl FnMut(R, i32, i16) -> R) -> R {
        let mut acc = initial;
        for (k, v) in self.iter() {
            acc = f(acc, k, v);
        }
    ...
    pub fn with_key_value(mut self, key: i32, value: i16) -> Self {
...
impl Default for I32I16HashMap {
    fn default() -> Self {
        Self::new()
    }
}

impl PartialEq for I32I16HashMap {
...
impl fmt::Display for I32I16HashMap {
...
mod tests {
    ...
    #[test]
    fn test_equals() {
        let mut m1 = I32I16HashMap::new();
        m1.insert(1, 1);
        m1.insert(2, 2);
        let mut m2 = I32I16HashMap::new();
        m2.insert(2, 2);
        m2.insert(1, 1);
        assert_eq!(m1, m2);
    }
...
impl crate::traits::i32_i16_map::I32I16Map for I32I16HashMap {
    fn len(&self) -> usize {
        self.len()
    }
...
impl crate::traits::i32_i16_map::I32I16MutableMap for I32I16HashMap {
