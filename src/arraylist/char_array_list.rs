...
/// Resizable array-backed list of `char` values.
///
/// Specialized for `char` — no boxing, contiguous memory layout.
#[derive(Debug, Clone)]
pub struct CharArrayList {
    items: Vec<char>,
}
...
impl CharArrayList {
    ...
    pub fn push_all(&mut self, values: &[char]) {
    ...
    pub fn set(&mut self, index: usize, value: char) -> char {
    ...
    /// # Panics
    /// Panics if index is out of bounds.
    pub fn remove_at_index(&mut self, index: usize) -> char {
    ...
    pub fn count(&self, predicate: impl Fn(char) -> bool) -> usize {
    ...
    pub fn inject_into<R>(&self, initial: R, mut f: impl FnMut(R, char) -> R) -> R {
        let mut acc = initial;
        for &v in &self.items {
            acc = f(acc, v);
        }
...
impl Default for CharArrayList {
    fn default() -> Self {
        Self::new()
    }
}

impl PartialEq for CharArrayList {
...
impl Eq for CharArrayList {}

impl fmt::Display for CharArrayList {
...
impl IntoIterator for CharArrayList {
    type Item = char;
...
impl<'a> IntoIterator for &'a CharArrayList {
    type Item = &'a char;
...
impl std::ops::Index<usize> for CharArrayList {
    type Output = char;
...
mod tests {
    ...
    #[test]
    fn test_of() {
        let l = CharArrayList::of(&['a', 'b', 'c']);
        assert_eq!(l.len(), 3);
    }

    #[test]
    fn test_set() {
        let mut l = CharArrayList::of(&['a', 'b']);
        let old = l.set(0, 'c');
        assert_eq!(old, 'a');
        assert_eq!(l.get(0), Some('c'));
    }

    #[test]
    fn test_remove_at_index() {
        let mut l = CharArrayList::of(&['a', 'b', 'c']);
    ...
    fn test_select_reject() {
        let l = CharArrayList::of(&['a', 'b', 'c', 'd', 'e']);
        let sel = l.select(|v| v > 'c');
        assert_eq!(sel.len(), 2);
        let rej = l.reject(|v| v > 'c');
        assert_eq!(rej.len(), 3);
    }
    ...
    fn test_detect() {
        let l = CharArrayList::of(&['a', 'b']);
        assert_eq!(l.detect(|v| v == 'b'), Some('b'));
        assert_eq!(l.detect(|v| v == 'z'), None);
    }

    #[test]
    fn test_any_all_none_satisfy() {
        let l = CharArrayList::of(&['a', 'b', 'c']);
        assert!(l.any_satisfy(|v| v == 'b'));
        assert!(!l.any_satisfy(|v| v == 'z'));
        assert!(l.all_satisfy(|v| v > '\0'));
        assert!(l.none_satisfy(|v| v == 'z'));
    }

    #[test]
    fn test_sum_min_max() {
        let l = CharArrayList::of(&['c', 'a', 'b']);
        assert_eq!(l.min(), Some('a'));
        assert_eq!(l.max(), Some('c'));
    }

    #[test]
    fn test_sort() {
        let mut l = CharArrayList::of(&['c', 'a', 'b']);
        l.sort();
    ...
    fn test_reversed() {
        let l = CharArrayList::of(&['a', 'b', 'c']);
        let r = l.reversed();
        assert_eq!(r.get(0), Some('c'));
        assert_eq!(r.get(2), Some('a'));
    }

    #[test]
    fn test_equals() {
        let l1 = CharArrayList::of(&['a', 'b']);
        let l2 = CharArrayList::of(&['a', 'b']);
        let l3 = CharArrayList::of(&['a']);
        assert_eq!(l1, l2);
        assert_ne!(l1, l3);
    }
    ...
    fn test_resize() {
        let mut l = CharArrayList::new();
        for i in 0..100 {
            l.push(char::from(b'a' + (i % 26) as u8));
        }
        assert_eq!(l.len(), 100);
    }
...
impl crate::traits::char_collection::CharCollection for CharArrayList {
    fn len(&self) -> usize {
        self.len()
    }
...
impl crate::traits::char_collection::CharMutableCollection for CharArrayList {
    fn clear(&mut self) {
        self.clear()
    }
}

impl crate::traits::char_collection::CharList for CharArrayList {
...
impl crate::traits::char_collection::CharMutableList for CharArrayList {
