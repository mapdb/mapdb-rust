// AUTO-GENERATED. DO NOT EDIT.

use std::collections::VecDeque;
use std::fmt;

/// Double-ended queue of `f64` values, backed by `VecDeque`.
/// O(1) amortized push/pop at both ends.
#[derive(Debug, Clone)]
pub struct F64ArrayDeque {
    inner: VecDeque<f64>,
}

impl F64ArrayDeque {
    pub fn new() -> Self {
        F64ArrayDeque {
            inner: VecDeque::new(),
        }
    }

    pub fn with_capacity(capacity: usize) -> Self {
        F64ArrayDeque {
            inner: VecDeque::with_capacity(capacity),
        }
    }

    pub fn of(values: &[f64]) -> Self {
        F64ArrayDeque {
            inner: values.iter().copied().collect(),
        }
    }

    /// Adds a value to the front of the deque.
    pub fn add_first(&mut self, value: f64) {
        self.inner.push_front(value);
    }

    /// Adds a value to the back of the deque.
    pub fn add_last(&mut self, value: f64) {
        self.inner.push_back(value);
    }

    /// Removes and returns the front element, or None if empty.
    pub fn remove_first(&mut self) -> Option<f64> {
        self.inner.pop_front()
    }

    /// Removes and returns the back element, or None if empty.
    pub fn remove_last(&mut self) -> Option<f64> {
        self.inner.pop_back()
    }

    /// Returns the front element without removing it, or None if empty.
    pub fn peek_first(&self) -> Option<f64> {
        self.inner.front().copied()
    }

    /// Returns the back element without removing it, or None if empty.
    pub fn peek_last(&self) -> Option<f64> {
        self.inner.back().copied()
    }

    pub fn len(&self) -> usize {
        self.inner.len()
    }
    pub fn is_empty(&self) -> bool {
        self.inner.is_empty()
    }
    pub fn clear(&mut self) {
        self.inner.clear();
    }

    /// Ensures the deque has capacity for `additional` more elements.
    /// Returns `TryReserveError` on allocator failure.
    pub fn try_reserve(
        &mut self,
        additional: usize,
    ) -> Result<(), std::collections::TryReserveError> {
        self.inner.try_reserve(additional)
    }

    pub fn contains(&self, value: f64) -> bool {
        self.inner.iter().any(|&v| v.to_bits() == value.to_bits())
    }

    /// Iterates from front to back.
    pub fn iter(&self) -> impl Iterator<Item = f64> + '_ {
        self.inner.iter().copied()
    }

    pub fn for_each(&self, mut f: impl FnMut(f64)) {
        for v in self.iter() {
            f(v);
        }
    }

    pub fn any_satisfy(&self, predicate: impl Fn(f64) -> bool) -> bool {
        self.inner.iter().any(|&v| predicate(v))
    }

    pub fn all_satisfy(&self, predicate: impl Fn(f64) -> bool) -> bool {
        self.inner.iter().all(|&v| predicate(v))
    }

    pub fn to_vec(&self) -> Vec<f64> {
        self.iter().collect()
    }
}

impl Default for F64ArrayDeque {
    fn default() -> Self {
        Self::new()
    }
}

impl PartialEq for F64ArrayDeque {
    fn eq(&self, other: &Self) -> bool {
        if self.inner.len() != other.inner.len() {
            return false;
        }
        self.inner
            .iter()
            .zip(other.inner.iter())
            .all(|(a, b)| a.to_bits() == b.to_bits())
    }
}

impl fmt::Display for F64ArrayDeque {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "[")?;
        for (i, v) in self.inner.iter().enumerate() {
            if i > 0 {
                write!(f, ", ")?;
            }
            write!(f, "{}", v)?;
        }
        write!(f, "]")
    }
}

#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn test_add_last_remove_first_fifo() {
        let mut d = F64ArrayDeque::new();
        d.add_last(1.0f64);
        d.add_last(2.0f64);
        d.add_last(3.0f64);
        assert_eq!(d.len(), 3);
        assert_eq!(d.remove_first(), Some(1.0f64));
        assert_eq!(d.remove_first(), Some(2.0f64));
        assert_eq!(d.remove_first(), Some(3.0f64));
        assert!(d.is_empty());
    }

    #[test]
    fn test_add_first_remove_last() {
        let mut d = F64ArrayDeque::new();
        d.add_first(1.0f64);
        d.add_first(2.0f64);
        d.add_first(3.0f64);
        // front: v[2], v[1], v[0]
        assert_eq!(d.peek_first(), Some(3.0f64));
        assert_eq!(d.peek_last(), Some(1.0f64));
        assert_eq!(d.remove_last(), Some(1.0f64));
        assert_eq!(d.remove_last(), Some(2.0f64));
        assert_eq!(d.remove_last(), Some(3.0f64));
    }

    #[test]
    fn test_remove_empty() {
        let mut d = F64ArrayDeque::new();
        assert_eq!(d.remove_first(), None);
        assert_eq!(d.remove_last(), None);
        assert_eq!(d.peek_first(), None);
        assert_eq!(d.peek_last(), None);
    }

    #[test]
    fn test_mixed_ops() {
        let mut d = F64ArrayDeque::new();
        d.add_last(2.0f64);
        d.add_first(1.0f64);
        d.add_last(3.0f64);
        // order: v[0], v[1], v[2]
        assert_eq!(d.remove_first(), Some(1.0f64));
        assert_eq!(d.remove_last(), Some(3.0f64));
        assert_eq!(d.remove_first(), Some(2.0f64));
    }

    #[test]
    fn test_contains() {
        let mut d = F64ArrayDeque::new();
        d.add_last(1.0f64);
        assert!(d.contains(1.0f64));
        assert!(!d.contains(3.0f64));
    }

    #[test]
    fn test_clear() {
        let mut d = F64ArrayDeque::of(&[1.0f64, 2.0f64]);
        d.clear();
        assert!(d.is_empty());
    }

    #[test]
    fn test_iter_front_to_back() {
        let d = F64ArrayDeque::of(&[1.0f64, 2.0f64, 3.0f64]);
        let vals: Vec<_> = d.iter().collect();
        assert_eq!(vals, vec![1.0f64, 2.0f64, 3.0f64]);
    }

    #[test]
    fn test_display() {
        let mut d = F64ArrayDeque::new();
        d.add_last(1.0f64);
        assert!(!d.to_string().is_empty());
    }

    #[test]
    fn test_try_reserve_happy_path() {
        let mut d = F64ArrayDeque::new();
        d.try_reserve(10).unwrap();
    }

    #[test]
    fn test_try_reserve_propagates_capacity_overflow() {
        let mut d = F64ArrayDeque::new();
        assert!(d.try_reserve(usize::MAX / 2).is_err());
    }
}
