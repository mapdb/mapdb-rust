...
impl BoolI16HashMap {
    ...
    pub fn detect(&self, predicate: impl Fn(bool, i16) -> bool) -> Option<(bool, i16)> {
    ...
    pub fn none_satisfy(&self, predicate: impl Fn(bool, i16) -> bool) -> bool {
        !self.iter().any(|(k, v)| predicate(k, v))
    }

    pub fn count(&self, predicate: impl Fn(bool, i16) -> bool) -> usize {
        self.iter().filter(|&(k, v)| predicate(k, v)).count()
    }

    pub fn inject_into<R>(&self, initial: R, mut f: impl FnMut(R, bool, i16) -> R) -> R {
        let mut acc = initial;
        for (k, v) in self.iter() {
            acc = f(acc, k, v);
        }
...
impl Default for BoolI16HashMap {
    fn default() -> Self {
        Self::new()
    }
}

impl PartialEq for BoolI16HashMap {
...
impl fmt::Display for BoolI16HashMap {
...
mod tests {
    ...
    #[test]
    fn test_equals() {
        let mut m1 = BoolI16HashMap::new();
        m1.insert(true, 1);
        m1.insert(false, 2);
        let mut m2 = BoolI16HashMap::new();
        m2.insert(false, 2);
        m2.insert(true, 1);
        assert_eq!(m1, m2);
    }
...
impl crate::traits::bool_i16_map::BoolI16Map for BoolI16HashMap {
    fn len(&self) -> usize {
        self.len()
    }
...
impl crate::traits::bool_i16_map::BoolI16MutableMap for BoolI16HashMap {
