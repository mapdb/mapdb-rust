...
use super::char_f32_hash_map::CharF32HashMap;

/// Thread-safe wrapper around [`CharF32HashMap`]. Reads are concurrent;
/// mutations are exclusive. Lock poisoning panics (parity with Go).
#[derive(Debug, Default)]
pub struct SynchronizedCharF32HashMap {
    inner: RwLock<CharF32HashMap>,
}

impl SynchronizedCharF32HashMap {
    pub fn new() -> Self {
        SynchronizedCharF32HashMap {
            inner: RwLock::new(CharF32HashMap::new()),
        }
    }

    pub fn with_capacity(capacity: usize) -> Self {
        SynchronizedCharF32HashMap {
            inner: RwLock::new(CharF32HashMap::with_capacity(capacity)),
        }
    }

    pub fn insert(&self, key: char, value: f32) -> Option<f32> {
    ...
    pub fn remove(&self, key: char) -> Option<f32> {
...
mod tests {
    ...
    #[test]
    fn test_basic_ops() {
        let m = SynchronizedCharF32HashMap::new();
        assert!(m.is_empty());
        assert_eq!(m.insert('a', 1.0f32), None);
        assert_eq!(m.insert('b', 2.0f32), None);
        assert_eq!(m.insert('c', 3.0f32), None);
        assert_eq!(m.get('a'), Some(1.0f32));
        assert!(m.contains_key('a'));
        assert!(m.contains_value(2.0f32));
        let old = m.insert('a', 2.0f32);
        assert_eq!(old, Some(1.0f32));
        assert_eq!(m.remove('a'), Some(2.0f32));
        assert!(!m.contains_key('a'));
        m.clear();
        assert!(m.is_empty());
    }
    ...
    #[test]
    fn test_concurrent() {
        let m = Arc::new(SynchronizedCharF32HashMap::new());
