...
/// Set multimap from `bool` keys to `char` values.
...
#[derive(Debug, Clone)]
pub struct BoolCharSetMultimap {
    inner: HashMap<bool, Vec<char>>,
    total_size: usize,
}

impl BoolCharSetMultimap {
    pub fn new() -> Self {
        BoolCharSetMultimap {
            inner: HashMap::new(),
            total_size: 0,
...
impl Default for BoolCharSetMultimap {
    fn default() -> Self {
        Self::new()
    }
}

impl PartialEq for BoolCharSetMultimap {
...
impl fmt::Display for BoolCharSetMultimap {
...
mod tests {
    ...
    #[test]
    fn test_put_get() {
        let mut m = BoolCharSetMultimap::new();
        m.put(true, 'a');
        m.put(true, 'b');
        m.put(false, 'c');
    ...
    #[test]
    fn test_equals() {
        let mut m1 = BoolCharSetMultimap::new();
        m1.put(true, 'a');
        m1.put(true, 'b');
        let mut m2 = BoolCharSetMultimap::new();
        m2.put(true, 'a');
        m2.put(true, 'b');
        assert_eq!(m1, m2);
    }
