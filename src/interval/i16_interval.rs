...
impl I16Interval {
    ...
    pub fn from_to_by(from: i16, to: i16, step: i16) -> Self {
        assert!(step != 0, "I16Interval: step must not be zero");
        if from < to {
            assert!(
                step > 0,
                "I16Interval: step must be positive when from < to"
            );
        } else if from > to {
            assert!(
                step < 0,
                "I16Interval: step must be negative when from > to"
            );
        }
        I16Interval { from, to, step }
    }

    /// Creates an interval from `from` to `to` (inclusive) with step 1 or -1.
    pub fn from_to(from: i16, to: i16) -> Self {
        let step: i16 = if from <= to { 1 } else { -1 };
        I16Interval { from, to, step }
    }

    /// Creates an interval from 1 to `to` (inclusive).
    pub fn one_to(to: i16) -> Self {
        Self::from_to(1, to)
    }

    /// Creates an interval from 0 to `to - 1` (inclusive).
    pub fn zero_to(to: i16) -> Self {
        Self::from_to(0, to)
    }
    ...
    pub fn get(&self, index: usize) -> Option<i16> {
        if index >= self.len() {
            ...
            Some(self.from + self.step * index as i16)
        }
    }

    /// Returns an iterator over the elements.
    pub fn iter(&self) -> I16IntervalIter {
        I16IntervalIter {
            current: self.from,
            to: self.to,
            step: self.step,
...
/// Iterator for `I16Interval`.
pub struct I16IntervalIter {
    current: i16,
    to: i16,
    step: i16,
    done: bool,
...
impl Iterator for I16IntervalIter {
    type Item = i16;
...
impl ExactSizeIterator for I16IntervalIter {}

impl fmt::Display for I16Interval {
...
mod tests {
    ...
    fn test_from_to_ascending() {
        let iv = I16Interval::from_to(1, 5);
        assert_eq!(iv.len(), 5);
    ...
    fn test_from_to_descending() {
        let iv = I16Interval::from_to(5, 1);
        assert_eq!(iv.len(), 5);
    ...
    #[test]
    fn test_from_to_by() {
        let iv = I16Interval::from_to_by(0, 10, 2);
        assert_eq!(iv.len(), 6);
    ...
    fn test_from_to_by_negative_step() {
        let iv = I16Interval::from_to_by(10, 0, -3);
        assert_eq!(iv.to_vec(), vec![10, 7, 4, 1]);
    }

    #[test]
    fn test_single_element() {
        let iv = I16Interval::from_to(3, 3);
        assert_eq!(iv.len(), 1);
    ...
    #[test]
    fn test_one_to() {
        let iv = I16Interval::one_to(3);
        assert_eq!(iv.to_vec(), vec![1, 2, 3]);
    }

    #[test]
    fn test_zero_to() {
        let iv = I16Interval::zero_to(3);
...
impl crate::traits::i16_collection::I16Collection for I16Interval {
    fn len(&self) -> usize {
        self.len()
    }
