...
/// List multimap from `char` keys to `f32` values.
...
#[derive(Debug, Clone)]
pub struct CharF32ListMultimap {
    inner: HashMap<char, Vec<f32>>,
    total_size: usize,
}

impl CharF32ListMultimap {
    pub fn new() -> Self {
        CharF32ListMultimap {
            inner: HashMap::new(),
            total_size: 0,
    ...
    pub fn put(&mut self, key: char, value: f32) {
    ...
    pub fn get_all(&self, key: char) -> Vec<f32> {
        self.get(key).to_vec()
    }

    /// Removes all values for the given key. Returns the removed values.
    pub fn remove_all(&mut self, key: char) -> Vec<f32> {
        let internal_key = key;
    ...
    pub fn contains_key_value(&self, key: char, value: f32) -> bool {
        self.get(key)
            .iter()
    ...
    pub fn for_each_key_value(&self, mut f: impl FnMut(char, &[f32])) {
        for (k, vals) in &self.inner {
            f(*k, vals);
        }
    }
    ...
    pub fn to_vec(&self) -> Vec<(char, f32)> {
...
impl Default for CharF32ListMultimap {
    fn default() -> Self {
        Self::new()
    }
}

impl PartialEq for CharF32ListMultimap {
...
impl fmt::Display for CharF32ListMultimap {
...
mod tests {
    ...
    #[test]
    fn test_put_get() {
        let mut m = CharF32ListMultimap::new();
        m.put('a', 1.0f32);
        m.put('a', 2.0f32);
        m.put('b', 3.0f32);
    ...
    fn test_contains_key_value() {
        ...
        m.put('a', 1.0f32);
        m.put('a', 2.0f32);
        assert!(m.contains_key_value('a', 1.0f32));
        assert!(m.contains_key_value('a', 2.0f32));
        assert!(!m.contains_key_value('a', 3.0f32));
    }
    ...
    #[test]
    fn test_equals() {
        let mut m1 = CharF32ListMultimap::new();
        m1.put('a', 1.0f32);
        m1.put('a', 2.0f32);
        let mut m2 = CharF32ListMultimap::new();
        m2.put('a', 1.0f32);
        m2.put('a', 2.0f32);
        assert_eq!(m1, m2);
    }
    ...
    fn test_with_key_value_fluent() {
        let m = CharF32ListMultimap::new()
            .with_key_value('a', 1.0f32)
            .with_key_value('a', 2.0f32)
            .with_key_value('b', 3.0f32);
        assert_eq!(m.size(), 3);
        assert_eq!(m.keys_count(), 2);
    }
