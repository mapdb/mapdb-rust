...
use super::char_f64_hash_map::CharF64HashMap;

/// Thread-safe wrapper around [`CharF64HashMap`]. Reads are concurrent;
/// mutations are exclusive. Lock poisoning panics (parity with Go).
#[derive(Debug, Default)]
pub struct SynchronizedCharF64HashMap {
    inner: RwLock<CharF64HashMap>,
}

impl SynchronizedCharF64HashMap {
    pub fn new() -> Self {
        SynchronizedCharF64HashMap {
            inner: RwLock::new(CharF64HashMap::new()),
        }
    }

    pub fn with_capacity(capacity: usize) -> Self {
        SynchronizedCharF64HashMap {
            inner: RwLock::new(CharF64HashMap::with_capacity(capacity)),
        }
    }

    pub fn insert(&self, key: char, value: f64) -> Option<f64> {
    ...
    pub fn remove(&self, key: char) -> Option<f64> {
...
mod tests {
    ...
    #[test]
    fn test_basic_ops() {
        let m = SynchronizedCharF64HashMap::new();
        assert!(m.is_empty());
        assert_eq!(m.insert('a', 1.0f64), None);
        assert_eq!(m.insert('b', 2.0f64), None);
        assert_eq!(m.insert('c', 3.0f64), None);
        assert_eq!(m.get('a'), Some(1.0f64));
        assert!(m.contains_key('a'));
        assert!(m.contains_value(2.0f64));
        let old = m.insert('a', 2.0f64);
        assert_eq!(old, Some(1.0f64));
        assert_eq!(m.remove('a'), Some(2.0f64));
        assert!(!m.contains_key('a'));
        m.clear();
        assert!(m.is_empty());
    }
    ...
    #[test]
    fn test_concurrent() {
        let m = Arc::new(SynchronizedCharF64HashMap::new());
