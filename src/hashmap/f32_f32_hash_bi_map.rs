...
impl F32F32HashBiMap {
    ...
    pub fn get_key(&self, value: f32) -> Option<f32> {
    ...
    pub fn inverse(&self) -> F32F32HashBiMap {
        F32F32HashBiMap::from_maps(self.reverse.clone(), self.forward.clone())
    }
    ...
    pub(crate) fn from_maps(
        ...
        F32F32HashBiMap { forward, reverse }
    }
}

impl Default for F32F32HashBiMap {
    fn default() -> Self {
        Self::new()
    }
}

impl PartialEq for F32F32HashBiMap {
...
impl fmt::Display for F32F32HashBiMap {
...
mod tests {
    ...
    fn test_get_key() {
        ...
        m.insert(1.0f32, 1.0f32);
        m.insert(2.0f32, 2.0f32);
        assert_eq!(m.get_key(1.0f32), Some(1.0f32));
        assert_eq!(m.get_key(2.0f32), Some(2.0f32));
        assert_eq!(m.get_key(99.0f32), None);
    }
    ...
    fn test_insert_overwrite() {
        ...
        assert_eq!(m.get_key(2.0f32), Some(1.0f32));
        // Old reverse mapping should be gone
        assert_eq!(m.get_key(1.0f32), None);
    }
    ...
    fn test_remove() {
        ...
        assert!(!m.contains_value(1.0f32));
    }
    ...
    fn test_remove_value() {
        ...
        m.insert(1.0f32, 1.0f32);
        m.insert(2.0f32, 2.0f32);
        assert_eq!(m.remove_value(1.0f32), Some(1.0f32));
        assert_eq!(m.len(), 1);
        assert!(!m.contains_key(1.0f32));
        assert!(!m.contains_value(1.0f32));
    }
    ...
    fn test_contains() {
        ...
        assert!(m.contains_value(1.0f32));
        assert!(!m.contains_value(99.0f32));
    }
    ...
    fn test_inverse() {
        ...
        assert_eq!(inv.get(1.0f32), Some(1.0f32));
        assert_eq!(inv.get(2.0f32), Some(2.0f32));
        // inverse reverse lookup: old key -> old value
        assert_eq!(inv.get_key(1.0f32), Some(1.0f32));
        assert_eq!(inv.len(), 2);
    }
...
impl crate::traits::f32_f32_map::F32F32Map for F32F32HashBiMap {
    fn len(&self) -> usize {
        self.len()
    }
...
impl crate::traits::f32_f32_map::F32F32MutableMap for F32F32HashBiMap {
